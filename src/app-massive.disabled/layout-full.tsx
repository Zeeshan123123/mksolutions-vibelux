...
async function getIsDesignRoute() {
  const headersList = await headers();
  const pathname = headersList.get('x-pathname') || '';
  return pathname.startsWith('/design') || pathname.startsWith('/greenhouse-designer');
}
...
export default async function RootLayout({
  ...
  const isDesignRoute = await getIsDesignRoute();
    ...
    <ClerkProvider>
      <html lang="en" suppressHydrationWarning>
        <head>
          ...
          {/* Scripts are now safely loaded via React components */}
        </head>
