...
export default function FacilityOnboarding() {
  ...
  const handleSubmit = async () => {
    ...
    if (!profile.totalSquareFeet || profile.totalSquareFeet <= 0) {
      validationErrors.push('Valid facility size is required');
    }
    
    // Financial validation
    if (profile.currentMonthlyRevenue < 0 || profile.currentMonthlyRevenue > 10000000) {
      validationErrors.push('Monthly revenue must be between $0 - $10M');
    }
      
    if (profile.currentMonthlyExpenses < 0 || profile.currentMonthlyExpenses > profile.currentMonthlyRevenue * 2) {
      validationErrors.push('Monthly expenses appear unrealistic');
    }
      
    if (profile.energyCostPerMonth < 0 || profile.energyCostPerMonth > profile.currentMonthlyExpenses) {
      validationErrors.push('Energy costs cannot exceed total expenses');
    }
    
    // Investment validation
    if (profile.fundingAmount <= 0 || profile.fundingAmount > 50000000) {
      validationErrors.push('Target investment amount must be between $1 - $50M');
    }
    
    // Operations validation
    if (profile.currentStaffCount < 0 || profile.currentStaffCount > 1000) {
