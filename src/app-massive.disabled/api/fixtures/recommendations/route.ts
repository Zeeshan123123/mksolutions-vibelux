...
async function handler(request: AuthenticatedRequest) {
  try {
    ...
    if (request.method === 'GET') {
      ...
      const recommendationResult = await fixtureSearchEngine.getRecommendations({
        cropName,
        growthStage,
        ...additionalFilters
      })
      ...
      const insights = generateCropInsights(cropName, recommendationResult.recommendations, {
        facilityType,
        coverageArea,
        budget,
        priority
      ...
      return NextResponse.json({
        ...
        alternativeOptions: recommendationResult.alternativeOptions,
        insights,
        meta: {
          userId: user.userId,
          timestamp: Date.now(),
      ...
      const insights = generateCropInsights(body.cropName, recommendationResult.recommendations, body)
    ...
    logger.error('api', 'Fixture recommendations error:', error )
    return NextResponse.json(
