...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      logger.error('api', 'PDF parsing error:', pdfError);
      return NextResponse.json(
    ...
    try {
      ...
      logger.error('api', 'Failed to parse Claude response:', new Error(responseText));
      return NextResponse.json(
    ...
    logger.error('api', 'PDF processing error:', error );
    return NextResponse.json(
...
async function updateClientBaseline(clientId: string, billData: any) {
  try {
    ...
    logger.error('api', 'Failed to update baseline:', error );
  }
}