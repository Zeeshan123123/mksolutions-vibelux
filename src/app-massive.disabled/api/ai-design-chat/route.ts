...
import { createClaudeClient, CLAUDE_CONFIG } from '@/lib/claude-config';
...
function getClaude() {
  if (!claude) {
    try {
      ...
      logger.error('api', 'Failed to initialize Claude client:', error );
      return null;
    }
  }
  return claude;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!claude) {
      logger.error('api', 'Claude client not initialized');
      logger.error('api', 'API Key exists: ' + !!process.env.CLAUDE_API_KEY);
      logger.error('api', 'API Key length: ' + (process.env.CLAUDE_API_KEY?.length || 0));
...
`;
    ...
    // Use Claude 4 Opus for all design tasks - it's the most powerful model available
    const model = CLAUDE_CONFIG.models.design;
      ...
      try {
        ...
        if (apiError?.status === 429 || apiError?.error?.type === 'rate_limit_error') {
          if (retries < maxRetries) {
            ...
            logger.warn('api', 'Max retries reached after rate limiting, using enhanced fallback designer');
    ...
    try {
      ...
      logger.error('api', 'Failed to parse AI response:', responseContent);
      ...
      aiResponse.design.zones = aiResponse.design.zones.map((zone: any) => {
        ...
        if (zone.fixtures) {
          zone.fixtures = zone.fixtures.map((fixture: any, index: number) => ({
            ...fixture,
            id: fixture.id || `fixture_${Date.now()}_${index}_${Math.random().toString(36).substr(2, 9)}`,
        ...
        if (zone.racks) {
          zone.racks = zone.racks.map((rack: any, index: number) => ({
            ...rack,
            id: rack.id || `rack_${Date.now()}_${index}_${Math.random().toString(36).substr(2, 9)}`,
    ...
    logger.error('api', 'AI Design Chat Error:', error );
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('401') || error.message.includes('authentication')) {
        ...
        logger.warn('api', 'Claude rate limit hit, using fallback designer');
        ...
        try {
          ...
          logger.error('api', 'Fallback designer error:', fallbackError);
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    model: CLAUDE_CONFIG.models.design,
    capabilities: [
