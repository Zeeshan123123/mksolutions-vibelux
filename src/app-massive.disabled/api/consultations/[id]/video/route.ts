...
export async function POST(
  ...
  try {
    ...
    const isExpert = consultation.expert.id === userId;
    const participantType = isExpert ? 'expert' : 'client';

    switch (action) {
      case 'join':
        return await handleJoinVideo(consultation, userId, participantType, isExpert);
      
      case 'leave':
        return await handleLeaveVideo(consultation, userId);
    ...
    logger.error('api', 'Error handling video conference:', error );
    return NextResponse.json(
...
async function handleJoinVideo(
  ...
  try {
    ...
    logger.error('api', 'Error joining video:', error );
    throw error;
  }
}
...
async function handleLeaveVideo(consultation: any, userId: string) {
  try {
    ...
    logger.error('api', 'Error leaving video:', error );
    throw error;
  }
}
...
async function handleEndVideo(consultation: any) {
  try {
    ...
    logger.error('api', 'Error ending video:', error );
    throw error;
  }
}