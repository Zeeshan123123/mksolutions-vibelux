...
// TODO: Fix database layer references - commenting out temporarily
export async function POST_DISABLED(request: NextRequest) {
  return NextResponse.json({ error: 'Affiliate conversion tracking temporarily disabled' }, { status: 503 });
  /*
  try {
    const { userId } = auth();
    ...
    logger.error('api', 'Failed to track conversion:', error );
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('api', 'Failed to check conversion tracking:', error );
    return NextResponse.json(
