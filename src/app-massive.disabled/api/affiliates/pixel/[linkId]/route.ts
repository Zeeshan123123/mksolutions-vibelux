...
// TODO: Fix affiliate system missing methods
export async function GET_DISABLED(
  request: NextRequest,
  { params }: { params: { linkId: string } }
) {
  // Return pixel immediately while logging the attempt
  const buffer = Buffer.from(PIXEL_DATA, 'base64');
  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        logger.error('api', 'Pixel tracking error:', error )
      }
    })
    ...
    logger.error('api', 'Tracking pixel error:', error )
