...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('api', 'Error fetching analytics metrics:', error );
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (type) {
      case 'harvest':
        // TODO: Fix harvestBatch database schema
        const harvest = {
          id: 'temp-' + Date.now(),
          facilityId,
      ...
      case 'energy':
        ...
        const powerReading = await prisma.sensorReading.create({
          data: {
            facilityId,
            sensorId: `manual-power-${Date.now()}`, // Generate a manual sensor ID
            sensorType: 'POWER',
            value: data.powerKw,
            unit: 'kW',
      ...
      case 'facility':
        ...
        const facility = await prisma.facility.update({
          where: { id: facilityId },
          data: {
            size: data.totalSquareFeet, // Map totalSquareFeet to size field
            // Note: cultivationSquareFeet doesn't exist in schema, using just size
          },
        });
      ...
      case 'expense':
        ...
        const expense = await prisma.expense.create({
          data: {
            facilityId,
            categoryId: `default-category-${Date.now()}`, // Generate default category ID since it's required
            amount: data.amount,
            description: data.description,
            expenseDate: new Date(data.date || Date.now()),
            recordedBy: userId,
          },
        });
    ...
    logger.error('api', 'Error saving manual data:', error );
    return NextResponse.json(
