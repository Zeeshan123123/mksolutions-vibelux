...
export async function POST(request: NextRequest) {
  try {
    // Claude client will be initialized later if needed
    let claude: any = null;
...
5. "interpretedSchema": Your understanding of what this dataset represents`;
    ...
    // Initialize Claude client now
    try {
      const { createClaudeClient } = await import('@/lib/claude-config');
    ...
    if (usage) {
      ...
      // TODO: Fix aiUsage database schema
      const { userId } = auth();
      if (userId) {
    ...
    logger.error('api', 'AI interpretation error:', error );
