...
export async function GET(request: NextRequest) {
  try {
    ...
    // Only admins can access this endpoint - for now we'll skip role check since authenticateRequest doesn't return role
    ...
    logger.error('api', 'Admin facilities error:', error );
    return NextResponse.json(
...
async function getFacilityMetrics(facilityId: string) {
  ...
  try {
    ...
    logger.error('api', `Error fetching metrics for facility ${facilityId}:`, error);
