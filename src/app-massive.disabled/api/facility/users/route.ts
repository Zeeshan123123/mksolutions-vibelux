...
export async function GET(req: NextRequest) {
  try {
    ...
    const pendingInvites = await prisma.facilityInvite.findMany({
      where: {
    ...
    logger.error('api', 'Error fetching facility users:', error );
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    const facilityOwner = await prisma.facilityUser.findFirst({
      where: {
        facilityId: facility.id,
        role: 'OWNER'
      },
    ...
    const tierConfig = SUBSCRIPTION_TIERS_15[ownerSubscriptionTier];
    ...
    const currentTeamMemberCount = await prisma.facilityUser.count({
      where: { facilityId: facility.id }
    });

    // Count pending invitations
    const pendingInviteCount = await prisma.facilityInvite.count({
      where: {
        facilityId: facility.id,
        status: 'PENDING'
      }
    ...
    const teamMemberLimit = (tierConfig as any)?.limits?.teamMembers || 5; // Default fallback
    ...
    if (existingUser) {
      const existingFacilityUser = await prisma.facilityUser.findFirst({
        where: {
    ...
    logger.error('api', 'Error inviting user:', error );
    return NextResponse.json(
...
export async function PUT(req: NextRequest) {
  try {
    ...
    // Get target user
    const targetFacilityUser = await prisma.facilityUser.findFirst({
      where: {
        userId: targetUserId,
        facilityId: facility.id
      }
    ...
    // Update role
    const updated = await prisma.facilityUser.update({
    ...
    logger.error('api', 'Error updating user role:', error );
    return NextResponse.json(
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // Remove user
    await prisma.facilityUser.delete({
      where: { id: targetFacilityUser.id }
    });
    
    return NextResponse.json({ message: 'User removed from facility' });
  } catch (error) {
    logger.error('api', 'Error removing user:', error );
    return NextResponse.json(
