...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!bucketResponse.ok && bucketResponse.status !== 409) { // 409 means bucket exists
      ...
      logger.error('api', 'Bucket creation error:', new Error(error) )
    ...
    if (!uploadResponse.ok) {
      ...
      logger.error('api', 'Upload error:', new Error(error) )
    ...
    if (!translationResponse.ok) {
      ...
      logger.error('api', 'Translation error:', new Error(error) )
    ...
    logger.error('api', 'Conversion error:', error )
    return NextResponse.json(
