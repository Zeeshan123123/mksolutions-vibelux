...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    const upcomingPayments: PaymentRecord[] = [];
    ...
    generatedData.investments.forEach(investment => {
      ...
      for (let month = 0; month < monthsSinceStart; month++) {
        ...
        let amount = 0;
        let paymentType: PaymentRecord['paymentType'] = 'service_fee';
        ...
        const payment: PaymentRecord = {
          ...
          amount,
          currency: 'USD',
          date: month < monthsSinceStart - 1 ? paymentDate : paymentDate,
          status: month < monthsSinceStart - 1 ? 'paid' : 'pending'
        };
      ...
      for (let month = 0; month < 3; month++) {
        ...
        const upcomingPayment: PaymentRecord = {
    ...
    setData({
      ...
      upcomingPayments: upcomingPayments.sort((a, b) => a.date.getTime() - b.date.getTime())
    });
    setLoading(false);
  }, []);
  ...
  const monthlyRevenue = payments
    .filter(p => p.status === 'paid' && p.date > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))
    .reduce((sum, payment) => sum + payment.amount, 0);
  const projectedMonthlyRevenue = upcomingPayments
    .filter(p => p.date < new Date(Date.now() + 30 * 24 * 60 * 60 * 1000))
    .reduce((sum, payment) => sum + payment.amount, 0);
                  ...
                  {investments
                    .sort((a, b) => {
                      const aRevenue = payments
                        .filter(p => p.investmentId === a.id && p.status === 'paid' && 
                                p.date > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))
                        .reduce((sum, p) => sum + p.amount, 0);
                      const bRevenue = payments
                        .filter(p => p.investmentId === b.id && p.status === 'paid' && 
                                p.date > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))
                        .reduce((sum, p) => sum + p.amount, 0);
                  ...
                  {payments.slice(0, 20).map((payment) => {
                    ...
                    return (
                      <TableRow key={payment.id}>
                        ...
                        <TableCell className="text-xs font-mono">{payment.id}</TableCell>
                        <TableCell>
                          <Button 
                            size="sm" 
                            variant="outline"
                ...
                <TableBody>
                  {upcomingPayments.map((payment) => {
                    ...
                    const daysUntilDue = Math.floor(
                      (payment.date.getTime() - new Date().getTime()) / (24 * 60 * 60 * 1000)
                    );
