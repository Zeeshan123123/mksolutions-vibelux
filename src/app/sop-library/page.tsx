...
export default function SOPLibraryPage() {
  ...
  const loadSOPs = async () => {
    ...
    try {
      // Fetch from database
      const dbSOPs = await SOPService.getSOPs({
        category: selectedCategory === 'all' ? undefined : selectedCategory,
        search: searchQuery || undefined
      })
      
      // Transform database SOPs to frontend format
      const transformedSOPs: SOPTemplate[] = dbSOPs.map(sop => ({
        id: sop.id,
        title: sop.title,
        category: sop.category as any,
        description: sop.description,
        difficulty: sop.difficulty as any,
        estimatedTime: `${sop.estimatedTime} minutes`,
        lastUpdated: new Date(sop.updatedAt).toISOString().split('T')[0],
        isCustom: sop.createdBy === user?.id,
        isFavorite: false,
        usage: sop.completions,
        tags: sop.tags,
        steps: sop.content?.sections?.map((section: any) => ({
          id: section.id,
          title: section.title,
          description: section.steps?.join('\n') || '',
          duration: '5 minutes',
          critical: true
        })) || [],
      ...
      setFilteredSOPs(transformedSOPs)
      
      // If no SOPs exist, generate defaults
      if (transformedSOPs.length === 0 && user?.id) {
        await generateDefaultSOPs()
      }
    } catch (error) {
      console.error('Failed to load SOPs:', error)
      toast({
        title: 'Error',
        description: 'Failed to load SOPs. Using demo data.',
        variant: 'destructive'
      })
  ...
  const generateDefaultSOPs = async () => {
    if (!user?.id) return
    
    try {
      // This would call an API to generate default SOPs
      const response = await fetch('/api/sop/generate-defaults', {
        method: 'POST',
      ...
      if (response.ok) {
        toast({
          title: 'Success',
          description: 'Default SOPs have been created for you.'
        })
        await loadSOPs()
      }
    } catch (error) {
  ...
  const saveSOP = async (updatedSOP: SOPTemplate) => {
    if (!user?.id) return
    ...
    try {
      const sopData = {
        ...
        content: {
          sections: updatedSOP.steps.map(step => ({
            id: step.id,
            title: step.title,
            steps: step.description.split('\n').filter(s => s.trim())
          }))
        },
        materials: updatedSOP.materials,
        safetyNotes: updatedSOP.safetyNotes,
        tags: updatedSOP.tags,
        estimatedTime: parseInt(updatedSOP.estimatedTime),
        difficulty: updatedSOP.difficulty,
        frequency: 'as-needed'
      }
      
      let result
      if (updatedSOP.id.startsWith('temp_')) {
        // Create new SOP
        result = await SOPService.createSOP(sopData, user.id)
      } else {
        // Update existing SOP
        result = await SOPService.updateSOP(updatedSOP.id, sopData, user.id)
      }
