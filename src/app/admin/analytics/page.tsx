...
interface AnalyticsData {
  ...
  recipeAnalytics?: {
    ...
    cropPerformance: Array<{
      ...
      implementations: number;
      avgYieldIncrease: number;
      topSpectrum: { blue: number; green: number; red: number; farRed: number };
      bestDLI: number;
      successRate: number;
    }>;
  ...
  growerInsights?: {
    ...
    facilitySizeDistribution: Array<{
      range: string;
      count: number;
      avgSuccess: number;
    }>;
...
function AdminAnalyticsContent() {
  ...
  const fetchAnalytics = async () => {
    try {
      logger.info('system', 'Fetching analytics with dateRange:', { data: dateRange });
      const response = await fetch(`/api/admin/analytics?range=${dateRange}`);
      logger.info('system', 'Response status:', { data: response.status });
      
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        logger.error('system', 'API Error:', errorData );
        throw new Error(`Failed to fetch analytics: ${response.status} ${response.statusText}`);
      }
      
      const data = await response.json();
      logger.info('system', 'Analytics data received:', { data: data });
      setAnalytics(data);
    } catch (error) {
      logger.error('system', 'Error fetching analytics:', error );
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="container mx-auto px-4">
          <div className="flex gap-1 overflow-x-auto">
            {[
              ...
              { id: 'recipes', label: 'Recipe Analytics', icon: Lightbulb },
              { id: 'correlations', label: 'Light Correlations', icon: TrendingUp },
              { id: 'growers', label: 'Grower Insights', icon: Users },
      ...
      <div className="container mx-auto px-4 py-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                ...
                <h3 className="text-2xl font-bold text-white">
                  {analytics.recipeAnalytics?.totalRecipes || 0}
                </h3>
                <p className="text-gray-400 text-sm mt-1">Total Recipes</p>
              </div>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                ...
                <h3 className="text-2xl font-bold text-white">
                  {analytics.recipeAnalytics?.totalImplementations.toLocaleString() || 0}
                </h3>
                <p className="text-gray-400 text-sm mt-1">Total Implementations</p>
              </div>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                ...
                <h3 className="text-2xl font-bold text-white">
                  {analytics.recipeAnalytics?.avgComplianceScore.toFixed(1) || 0}%
                </h3>
                <p className="text-gray-400 text-sm mt-1">Avg Compliance Score</p>
              </div>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                ...
                <h3 className="text-2xl font-bold text-white">
                  {analytics.recipeAnalytics?.activeRecipes || 0}
                </h3>
                <p className="text-gray-400 text-sm mt-1">Active Recipes</p>
              </div>
            </div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-800">
                      <th className="text-left py-3 px-4 text-gray-400 font-medium">Recipe</th>
                      <th className="text-left py-3 px-4 text-gray-400 font-medium">Crop</th>
                      <th className="text-right py-3 px-4 text-gray-400 font-medium">Implementations</th>
                      <th className="text-right py-3 px-4 text-gray-400 font-medium">Avg Yield Increase</th>
                      <th className="text-right py-3 px-4 text-gray-400 font-medium">Success Rate</th>
                      <th className="text-right py-3 px-4 text-gray-400 font-medium">Revenue</th>
                    </tr>
                  </thead>
                  <tbody>
                    {analytics.recipeAnalytics?.topPerformingRecipes.map((recipe, index) => (
                      <tr key={index} className="border-b border-gray-800/50">
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                        ...
                        <td className="py-3 px-4">
                          <span className="text-gray-300">{recipe.crop}</span>
                        </td>
                        <td className="text-right py-3 px-4 text-gray-300">
                          {recipe.implementations.toLocaleString()}
                        </td>
                        <td className="text-right py-3 px-4">
                          <span className="text-green-400 font-medium">+{recipe.avgYieldIncrease}%</span>
                        </td>
                        <td className="text-right py-3 px-4">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h3 className="text-lg font-semibold text-white mb-4">Crop Performance Summary</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {analytics.recipeAnalytics?.cropPerformance.map((crop, index) => (
                  <div key={index} className="bg-gray-800/50 rounded-lg p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="text-white">{crop.recipes}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Avg Yield Increase:</span>
                        <span className="text-green-400">+{crop.avgYieldIncrease}%</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Best DLI:</span>
                        <span className="text-yellow-400">{crop.bestDLI}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Success Rate:</span>
                        <span className="text-blue-400">{crop.successRate}%</span>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {selectedView === 'correlations' && (
          <div className="space-y-6">
            {/* Correlation Results */}
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h3 className="text-lg font-semibold text-white mb-4">Statistical Correlations</h3>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-800">
                      <th className="text-left py-3 px-4 text-gray-400 font-medium">Variable 1</th>
                      <th className="text-left py-3 px-4 text-gray-400 font-medium">Variable 2</th>
                      <th className="text-right py-3 px-4 text-gray-400 font-medium">Correlation</th>
                      <th className="text-right py-3 px-4 text-gray-400 font-medium">P-Value</th>
                      <th className="text-right py-3 px-4 text-gray-400 font-medium">Sample Size</th>
                      <th className="text-left py-3 px-4 text-gray-400 font-medium">Strength</th>
                    </tr>
                  </thead>
                  <tbody>
                    {analytics.recipeAnalytics?.correlationResults.map((corr, index) => (
                      <tr key={index} className="border-b border-gray-800/50">
                        <td className="py-3 px-4 text-white font-medium">{corr.variable1}</td>
                        <td className="py-3 px-4 text-gray-300">{corr.variable2}</td>
                        <td className="text-right py-3 px-4">
                          <span className={`font-medium ${
                            Math.abs(corr.correlation) > 0.7 ? 'text-green-400' :
                            Math.abs(corr.correlation) > 0.5 ? 'text-yellow-400' : 'text-gray-400'
                          }`}>
                        ...
                        <td className="text-right py-3 px-4">
                          <span className={`text-xs ${
                            corr.pValue < 0.001 ? 'text-green-400' :
                            corr.pValue < 0.01 ? 'text-yellow-400' : 'text-gray-400'
                          }`}>
                            {corr.pValue.toFixed(4)}
                          </span>
                        </td>
                        <td className="text-right py-3 px-4 text-gray-300">{corr.sampleSize}</td>
                        <td className="py-3 px-4">
                          <span className={`text-xs px-2 py-1 rounded ${
                            corr.strength === 'very_strong' ? 'bg-green-900/20 text-green-400' :
                            corr.strength === 'strong' ? 'bg-blue-900/20 text-blue-400' :
                            corr.strength === 'moderate' ? 'bg-yellow-900/20 text-yellow-400' :
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h3 className="text-lg font-semibold text-white mb-4">Environmental Factor Impact</h3>
              <div className="space-y-4">
                {analytics.recipeAnalytics?.environmentalCorrelations.map((env, index) => (
                  <div key={index} className="bg-gray-800/50 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <FlaskConical className="w-5 h-5 text-blue-400" />
                        <h4 className="text-white font-medium">{env.factor}</h4>
                      </div>
                      <span className="text-sm text-gray-400">Correlation: {env.correlation.toFixed(3)}</span>
                    </div>
                    <div className="space-y-1">
                      <p className="text-sm text-gray-300">
                        <span className="text-gray-400">Optimal Range:</span> {env.optimalRange}
                      </p>
                      <p className="text-sm text-green-300">{env.impact}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                ...
                <h3 className="text-2xl font-bold text-white">
                  {analytics.growerInsights?.totalGrowers.toLocaleString() || 0}
                </h3>
                <p className="text-gray-400 text-sm mt-1">Total Growers</p>
              </div>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                ...
                <h3 className="text-2xl font-bold text-white">
                  {analytics.growerInsights?.activeGrowers.toLocaleString() || 0}
                </h3>
                <p className="text-gray-400 text-sm mt-1">Active Growers</p>
              </div>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                ...
                <h3 className="text-2xl font-bold text-white">
                  {analytics.growerInsights?.avgFacilitySize.toLocaleString() || 0}
                </h3>
                <p className="text-gray-400 text-sm mt-1">Avg Facility Size (sq ft)</p>
              </div>
            </div>

            {/* Facility Size Distribution */}
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h3 className="text-lg font-semibold text-white mb-4">Facility Size Distribution</h3>
              <div className="space-y-4">
                {analytics.growerInsights?.facilitySizeDistribution.map((size, index) => (
                  <div key={index} className="bg-gray-800/50 rounded-lg p-4">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-white font-medium">{size.range}</span>
                      <span className="text-green-400 font-medium">{size.avgSuccess}% avg success</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-400">{size.count.toLocaleString()} facilities</span>
                      <div className="w-32 bg-gray-700 rounded-full h-2">
                        <div
                          className="bg-purple-500 h-2 rounded-full"
                          style={{ width: `${analytics.growerInsights ? (size.count / analytics.growerInsights.totalGrowers) * 100 : 0}%` }}
                        />
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              <h3 className="text-lg font-semibold text-white mb-4">Grower Experience Analysis</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {analytics.growerInsights?.experienceLevel.map((exp, index) => (
                  <div key={index} className="bg-gray-800/50 rounded-lg p-4">
                    <h4 className="text-white font-medium mb-3">{exp.level}</h4>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-gray-400">Growers:</span>
                        <span className="text-white">{exp.count.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Recipe Adoption:</span>
                        <span className="text-blue-400">{exp.recipeAdoption}%</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-400">Avg Success:</span>
                        <span className="text-green-400">{exp.avgSuccess}%</span>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-800">
                      ...
                      <th className="text-right py-3 px-4 text-gray-400 font-medium">Growers</th>
                      <th className="text-left py-3 px-4 text-gray-400 font-medium">Avg Yield</th>
                      <th className="text-right py-3 px-4 text-gray-400 font-medium">Recipe Usage</th>
                    </tr>
                  </thead>
                  <tbody>
                    {analytics.growerInsights?.topPerformingRegions.map((region, index) => (
                      <tr key={index} className="border-b border-gray-800/50">
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-2">
                        ...
                        <td className="text-right py-3 px-4 text-gray-300">
                          {region.growers.toLocaleString()}
                        </td>
                        <td className="py-3 px-4">
                          <span className={`text-xs px-2 py-1 rounded ${
                            region.avgYield === 'Excellent' ? 'bg-green-900/20 text-green-400' :
                            region.avgYield === 'Above Average' ? 'bg-blue-900/20 text-blue-400' :
                            'bg-gray-900/20 text-gray-400'
                          }`}>
                            {region.avgYield}
                          </span>
                        ...
                        <td className="text-right py-3 px-4">
                          <span className="text-purple-400 font-medium">{region.recipeUsage}%</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {selectedView === 'builder' && (
          <div className="space-y-6">
            <DashboardBuilder 
              className="w-full" 
              onSave={(dashboard) => {
                logger.info('system', 'Dashboard saved:', { data: dashboard });
              }}
            />
          </div>
        )}
