...
import { Search, Filter, Download, Heart, BarChart3, Eye, X, ChevronDown, Zap, Lightbulb, Ruler, Shield, Award, Loader2, ChevronLeft, ChevronRight, Calculator, Menu, Home, ArrowRight, Target } from "lucide-react"
  ...
  useEffect(() => {
    const fetchFixtures = async () => {
      ...
      try {
        ...
        logger.error('system', 'Failed to fetch fixtures:', error )
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="relative z-10">
        ...
        <div className="bg-gradient-to-r from-blue-900/80 to-green-900/80 backdrop-blur-xl border-b border-blue-700/50">
          <div className="container mx-auto px-4 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white">Professional Lighting Technician Tools</h3>
                  <p className="text-blue-200 text-sm">Advanced DLC analysis with spectrum sliders, electrical filters, and professional-grade search</p>
                </div>
              </div>
              <Link 
                href="/lighting-technician"
            ...
            <>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {fixtures.map(fixture => {
                  // Calculate responsive sizing based on actual DLC dimensions
                  const getFixtureDisplaySize = (fixture: any) => {
                    const width = fixture.width || 24; // inches
                    const length = fixture.length || 48; // inches
                    const aspectRatio = length / width;
                    
                    // Scale fixture dimensions to reasonable card sizes
                    let cardHeight, iconSize;
                    
                    if (width <= 12) {
                      // Small fixtures (0-12"): compact display
                      cardHeight = "h-32"; // 128px
                      iconSize = "w-12 h-12"; // 48px
                    } else if (width <= 24) {
                      // Medium fixtures (12-24"): standard display
                      cardHeight = "h-40"; // 160px
                      iconSize = "w-16 h-16"; // 64px
                    } else {
                      // Large fixtures (24"+ width): larger display
                      cardHeight = "h-48"; // 192px (current)
                      iconSize = "w-20 h-20"; // 80px (current)
                    }
                    
                    return { cardHeight, iconSize, aspectRatio };
                  };
                  
                  const { cardHeight, iconSize } = getFixtureDisplaySize(fixture);
                  ...
                  <div key={fixture.id} className="bg-gray-800/50 backdrop-blur-xl rounded-xl border border-gray-700 hover:border-gray-600 transition-all group">
                    {/* Gradient Placeholder Image - Scaled by actual DLC dimensions */}
                    <div className={`relative ${cardHeight} bg-gradient-to-br from-purple-900/50 via-purple-800/30 to-blue-900/50 rounded-t-xl overflow-hidden`}>
                      <div className="absolute inset-0 flex items-center justify-center">
                        <Lightbulb className={`${iconSize} text-purple-300/70`} />
                      </div>
                      ...
                      {/* Category Badge and Size Indicator */}
                      <div className="absolute bottom-2 left-2 flex gap-2">
                        ...
                        {(fixture.width || fixture.length) && (
                          <span className="bg-blue-800/90 backdrop-blur text-xs font-medium px-2 py-1 rounded-full text-blue-200 border border-blue-600/50">
                            {fixture.width ? Math.round(fixture.width) : '?'}" × {fixture.length ? Math.round(fixture.length) : '?'}"
                          </span>
                        )}
                      </div>
                    </div>
                        ...
                        {(fixture.width || fixture.length) && (
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-medium text-white">
                              {fixture.width ? Math.round(fixture.width) : '?'}" × {fixture.length ? Math.round(fixture.length) : '?'}"
                              {fixture.height && ` × ${Math.round(fixture.height)}"`}
                            </span>
                          </div>
                        )}
                        <div className="flex justify-between items-center">
