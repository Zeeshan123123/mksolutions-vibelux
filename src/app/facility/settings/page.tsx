...
export default function FacilitySettings() {
  ...
  const [weather, setWeather] = useState<{ provider: 'NOAA' | 'OPEN_METEO' | 'METEOSTAT'; stationId?: string; latitude?: number; longitude?: number } | null>(null);
  const [weatherLoading, setWeatherLoading] = useState(false);
  const [weatherResult, setWeatherResult] = useState<{ provider: string; days: number; totals: { hdd: number; cdd: number } } | null>(null);
  const [settings, setSettings] = useState({
    ...
    apiAccess: true,
    webhookURL: '',
    apiKey: process.env.NEXT_PUBLIC_VIBELUX_API_KEY || ''
  });
  ...
  const handleSave = async () => {
    try {
      ...
      logger.error('system', 'Error saving settings:', error );
      // Show error notification
    }
  };

  const loadWeatherBinding = async () => {
    try {
      const res = await fetch('/api/facility/weather');
      if (res.ok) {
        const data = await res.json();
        setWeather(data.facilityWeather || { provider: 'NOAA' });
      }
    } catch {}
  };

  const saveWeatherBinding = async () => {
    if (!weather) return;
    await fetch('/api/facility/weather', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(weather) });
  };

  React.useEffect(() => { loadWeatherBinding(); }, []);
        ...
        <TabsContent value="integration" className="space-y-6">
          <Card className="bg-gray-800 border-gray-700">
            ...
            <CardContent className="space-y-6">
              ...
              {/* Weather Provider Binding */}
              <div className="space-y-4">
                <h3 className="text-lg font-medium text-white flex items-center gap-2"><MapPin className="w-4 h-4"/> Weather Station</h3>
                <div className="grid md:grid-cols-4 gap-4">
                  <div>
                    <Label>Provider</Label>
                    <Select value={weather?.provider || 'NOAA'} onValueChange={(v) => setWeather(w => ({ ...(w || { provider: 'NOAA' }), provider: v as any }))}>
                      <SelectTrigger className="mt-1"><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="NOAA">NOAA (US)</SelectItem>
                        <SelectItem value="OPEN_METEO">Open‑Meteo</SelectItem>
                  ...
                  <div>
                    ...
                    <Input value={weather?.stationId || ''} onChange={(e) => setWeather(w => ({ ...(w || { provider: 'NOAA' }), stationId: e.target.value }))} className="mt-1" placeholder="e.g., GHCND:USW00023174"/>
                  </div>
                  <div>
                    <Label>Latitude</Label>
                    <Input type="number" value={weather?.latitude ?? ''} onChange={(e) => setWeather(w => ({ ...(w || { provider: 'NOAA' }), latitude: e.target.value ? parseFloat(e.target.value) : undefined }))} className="mt-1"/>
                  </div>
                  <div>
                    <Label>Longitude</Label>
                    <Input type="number" value={weather?.longitude ?? ''} onChange={(e) => setWeather(w => ({ ...(w || { provider: 'NOAA' }), longitude: e.target.value ? parseFloat(e.target.value) : undefined }))} className="mt-1"/>
                  </div>
                </div>
                <div className="flex gap-3">
                  <Button variant="outline" onClick={loadWeatherBinding}>Test</Button>
                  <Button className="bg-blue-600 hover:bg-blue-700" onClick={saveWeatherBinding}>Save Weather Binding</Button>
                </div>

                {/* Quick Weather Check */}
                <div className="p-4 border border-gray-700 rounded-lg bg-gray-900">
                  <div className="flex items-center justify-between mb-3">
                    <div className="text-sm text-gray-300">Last 30-day HDD/CDD (based on binding)</div>
                    <Button
                      variant="outline"
                      onClick={async () => {
                        try {
                          setWeatherLoading(true);
                          const res = await fetch('/api/facility/weather/refresh?days=30');
                          if (res.ok) {
                            const data = await res.json();
                            setWeatherResult({ provider: data.provider, days: data.days, totals: data.totals });
                          }
                        } finally {
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div className="bg-gray-800 rounded p-3">
                      <div className="text-gray-400">Heating Degree Days</div>
                      <div className="text-white text-lg font-semibold">{weatherResult?.totals.hdd ?? '—'}</div>
                    </div>
                    <div className="bg-gray-800 rounded p-3">
                      <div className="text-gray-400">Cooling Degree Days</div>
                      <div className="text-white text-lg font-semibold">{weatherResult?.totals.cdd ?? '—'}</div>
                    </div>
                  </div>
                  {weatherResult && (
                    <div className="text-xs text-gray-500 mt-2">Provider: {weatherResult.provider} • Period: last {weatherResult.days} days</div>
                  )}
                </div>
              </div>
            </CardContent>
