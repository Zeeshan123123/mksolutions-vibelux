...
export default function ConsolidatedAdvancedDesignerPage() {
  ...
  // Project management (SSR-safe localStorage replacement)
  ...
  useEffect(() => {
    ...
    try {
      const cpRaw = window.localStorage.getItem('vibelux_current_project');
      const psRaw = window.localStorage.getItem('vibelux_projects');
      if (cpRaw) {
        const parsed = JSON.parse(cpRaw);
        // revive dates if present
        if (parsed?.createdAt) parsed.createdAt = new Date(parsed.createdAt);
        if (parsed?.updatedAt) parsed.updatedAt = new Date(parsed.updatedAt);
        setCurrentProject(parsed);
      }
      if (psRaw) {
        const parsedArr: any[] = JSON.parse(psRaw);
        setProjects(
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      window.localStorage.setItem('vibelux_current_project', JSON.stringify(currentProject));
    } catch {}
  }, [currentProject]);
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    try {
      window.localStorage.setItem('vibelux_projects', JSON.stringify(projects));
    } catch {}
  }, [projects]);
  ...
  useEffect(() => {
    if (isLoaded && userId) {
      ...
      if (hasExistingProjects) {
        ...
        // Set default design config if not already set
        if (!designConfig) {
          setDesignConfig({
            spaceType: 'indoor',
            dimensions: { length: 40, width: 20, height: 10 },
  ...
  }, [isLoaded, userId, projects, currentProject, designConfig]);
  ...
  const handleGenerateFullSet = async () => {
    try {
      const response = await fetch('/api/ai-designer', {
        ...
        body: JSON.stringify({
          mode: 'build-greenhouse',
          currentState: { room: designState.room, objects: [] }
        })
      });
      if (!response.ok) throw new Error('Failed to generate');
      const data = await response.json();

      // Apply returned actions to designer (broadcast to AIDesignAssistant handler)
      if (Array.isArray(data.actions)) {
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('designer.applyActions', { detail: data.actions }));
        }
      }

      // Open generated multi-sheet PDF if provided
      const pdfUrl = data?.deliverables?.drawings?.dataUrl as string | undefined;
      if (pdfUrl && typeof window !== 'undefined') {
        window.open(pdfUrl, '_blank', 'noopener,noreferrer');
      }

      // Optionally download a JSON summary deliverable for review
      const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
      saveAs(blob, 'greenhouse-plan-summary.json');

      const projectMsg = data.projectId ? ` Project created: ${data.projectId}` : '';
      toast({ title: 'A/S/MEP set generated', description: `Deliverables prepared.${projectMsg}` });
    } catch (e: any) {
  ...
  // Show onboarding - disabled for now to ensure designer loads
  if (false && showOnboarding && !designConfig) {
    return (
  ...
  return (
    <ErrorBoundaryAdvanced level="page" resetKeys={[designState.fixtures.length]}>
      <div className="min-h-screen bg-gray-900 text-white flex overflow-hidden" style={{ minHeight: '900px' }}>
        {/* Sidebar */}
        <CollapsibleSidebar
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="bg-gray-800 border-b border-gray-700 px-4 py-2 flex-shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 min-w-0">
                <h1 className="text-lg font-bold truncate">Advanced Designer</h1>
                <button
                  onClick={() => setUseSimpleDesigner(!useSimpleDesigner)}
                  className="px-2 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs"
                >
                  {useSimpleDesigner ? 'Advanced' : 'Simple'}
                </button>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => typeof window !== 'undefined' && window.location.reload()}>
                      <Plus className="w-4 h-4 mr-2" /> New Project
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => typeof document !== 'undefined' && document.getElementById('file-input')?.click()}>
          ...
          <div className="flex-1 relative overflow-hidden">
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as AnalysisTab)} className="h-full flex flex-col">
              <TabsList className="flex w-full bg-gray-800 rounded-none border-b border-gray-700 flex-shrink-0 h-12">
                {/* Show only Design tab by default */}
                <TabsTrigger
                  value="design"
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button 
                      variant="ghost" 
                      className="flex items-center gap-2 h-12 px-4 border-l border-gray-700 rounded-none hover:bg-gray-700"
                    >
                  ...
                  <DropdownMenuContent align="end" className="w-48">
                    {tabConfig.slice(1).map((tab) => {
                      ...
                      return (
                        <DropdownMenuItem
                          key={tab.value}
                          onClick={() => hasAccess && setActiveTab(tab.value as AnalysisTab)}
                          disabled={!hasAccess}
                          className="flex items-center gap-2"
                        >
                          <Icon className="w-4 h-4" />
                          {tab.label}
                          {!hasAccess && <Crown className="w-3 h-3 ml-auto text-yellow-500" />}
                        </DropdownMenuItem>
                      );
                    })}
                  </DropdownMenuContent>
              ...
              {/* Design & Layout Tab */}
              <TabsContent value="design" className="flex-1 mt-0 overflow-auto">
              ...
              {/* Light Analysis Tab */}
              <TabsContent value="light" className="flex-1 mt-0 overflow-auto">
                <div className="h-full bg-gray-850 p-2 md:p-4">
                  <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 h-full auto-rows-min">
