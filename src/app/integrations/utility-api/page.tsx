...
export default function UtilityAPIPage() {
  ...
  const [facilityId, setFacilityId] = useState<string>(searchParams.get('facilityId') || '');
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isSignedIn || !userId) {
      setParseError('Please sign in to upload utility bills');
      return;
    }
    try {
      ...
      const resp = await fetch('/api/claude/process-pdf', { method: 'POST', body: form });
      if (!resp.ok) {
  ...
  const useAsBaseline = async () => {
    setBaselineMsg(null);
    if (!parsed?.data) {
      setBaselineMsg('No parsed data available');
      return;
    }
    if (!facilityId) {
      setBaselineMsg('Please provide a facilityId (use the Energy Setup wizard)');
      return;
    }
    // Derive baseline fields
    const totalKwh = Number(parsed.data.totalKwh ?? 0);
    const totalCharges = Number(parsed.data.totalCharges ?? 0);
    const peakDemand = Number(parsed.data.peakDemand ?? 0);
    if (!totalKwh || !totalCharges) {
      setBaselineMsg('Parsed data missing kWh/charges; cannot compute baseline');
      return;
    }
    const ratePerKwh = totalCharges && totalKwh ? totalCharges / totalKwh : 0.12;
    const averageMonthlyKwh = totalKwh; // for a monthly bill
    const demandKw = peakDemand || 0;
    try {
      ...
      setBaselineMsg('Baseline saved successfully');
    } catch (e: any) {
      setBaselineMsg(e?.message || 'Failed to save baseline');
    } finally {
      setSavingBaseline(false);
    }
  };

  // Load adjustments when facility changes and on automation tab
  useEffect(() => {
    const load = async () => {
      setAdjMsg(null);
      if (!facilityId || activeTab !== 'automation') return;
      try {
        const resp = await fetch(`/api/energy/adjustments?facilityId=${encodeURIComponent(facilityId)}`);
        const data = await resp.json();
        setAdjustments(data.adjustments || []);
      } catch {
        setAdjMsg('Failed to load adjustments');
      }
    };
    load();
  ...
  const submitAdjustment = async () => {
    setAdjMsg(null);
    if (!facilityId) { setAdjMsg('Facility ID required'); return; }
    try {
      const payload = {
        facilityId,
        effectiveStart: adjForm.effectiveStart || new Date().toISOString(),
        effectiveEnd: adjForm.effectiveEnd || null,
        kwhDelta: adjForm.kwhDelta ? Number(adjForm.kwhDelta) : 0,
        demandKwDelta: adjForm.demandKwDelta ? Number(adjForm.demandKwDelta) : 0,
        ratePerKwh: adjForm.ratePerKwh ? Number(adjForm.ratePerKwh) : null,
        demandChargeRate: adjForm.demandChargeRate ? Number(adjForm.demandChargeRate) : null,
        reason: adjForm.reason || null,
        notes: adjForm.notes || null
      };
      const resp = await fetch('/api/energy/adjustments', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload)
      });
      if (!resp.ok) throw new Error('Failed');
      setAdjMsg('Adjustment saved');
      // refresh list
      const list = await fetch(`/api/energy/adjustments?facilityId=${encodeURIComponent(facilityId)}`).then(r=>r.json());
      setAdjustments(list.adjustments || []);
    } catch {
      setAdjMsg('Failed to save adjustment');
    }
  };

  useEffect(() => {
    const tab = (searchParams?.get('tab') || '').toLowerCase();
    if (['overview', 'connections', 'pdf-processing', 'automation'].includes(tab)) {
      setActiveTab(tab);
    }
  }, [searchParams]);
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {activeTab === 'pdf-processing' && (
          <div className="space-y-8">
            ...
            <div className="mb-4">
              <label className="block text-sm text-gray-400 mb-2">Facility ID (from Energy Setup)</label>
              <input
                value={facilityId}
                onChange={(e) => setFacilityId(e.target.value)}
                placeholder="facility_..."
              ...
              {parsed?.data && (
                <div className="bg-gray-900/50 rounded-xl p-6 border border-white/10 mt-6">
                  <h3 className="text-lg font-semibold text-white mb-4">Extracted Bill Data</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    <div>
                      <h4 className="text-gray-300 font-medium mb-2">Account</h4>
                      <div className="space-y-1 text-gray-400">
                        <div className="flex justify-between"><span>Account #</span><span className="text-white">{parsed.data.accountNumber || '—'}</span></div>
                        <div className="flex justify-between"><span>Service Address</span><span className="text-white">{parsed.data.serviceAddress || '—'}</span></div>
                        <div className="flex justify-between"><span>Billing Period</span><span className="text-white">{parsed.data.billingPeriodStart || '—'} → {parsed.data.billingPeriodEnd || '—'}</span></div>
                      </div>
                    </div>
                    <div>
                      <h4 className="text-gray-300 font-medium mb-2">Usage & Charges</h4>
                      <div className="space-y-1 text-gray-400">
                        <div className="flex justify-between"><span>Total kWh</span><span className="text-white">{parsed.data.totalKwh ?? '—'}</span></div>
                        <div className="flex justify-between"><span>Peak Demand (kW)</span><span className="text-white">{parsed.data.peakDemand ?? '—'}</span></div>
                        <div className="flex justify-between"><span>Total Charges</span><span className="text-white">{parsed.data.totalCharges ?? '—'}</span></div>
                        <div className="flex justify-between"><span>Rate Schedule</span><span className="text-white">{parsed.data.rateSchedule || '—'}</span></div>
                      </div>
                    </div>
                  </div>
                  <div className="mt-4 text-xs text-gray-500">Baseline will update automatically as enough history accumulates.</div>
                </div>
              )}
            </div>
        ...
        {activeTab === 'automation' && (
          <div className="space-y-8">
            ...
            <div className="bg-gray-900/50 rounded-xl p-6 border border-white/10">
              <h3 className="text-lg font-semibold text-white mb-4">Operational Adjustments</h3>
              <p className="text-sm text-gray-400 mb-4">Capture factors not reflected in the bill (e.g., partial HPS→LED retrofit, closed rooms) that affect savings for specific periods.</p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Facility ID</label>
                  <input value={facilityId} onChange={(e)=>setFacilityId(e.target.value)} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Reason</label>
                  <input value={adjForm.reason} onChange={(e)=>setAdjForm({...adjForm, reason: e.target.value})} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white" placeholder="HPS retrofit in Zone A" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Effective Start</label>
                  <input type="date" value={adjForm.effectiveStart} onChange={(e)=>setAdjForm({...adjForm, effectiveStart: e.target.value})} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Effective End (optional)</label>
                  <input type="date" value={adjForm.effectiveEnd} onChange={(e)=>setAdjForm({...adjForm, effectiveEnd: e.target.value})} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">kWh Delta (per period)</label>
                  <input type="number" value={adjForm.kwhDelta} onChange={(e)=>setAdjForm({...adjForm, kwhDelta: e.target.value})} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white" placeholder="e.g., -12000" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Demand kW Delta</label>
                  <input type="number" value={adjForm.demandKwDelta} onChange={(e)=>setAdjForm({...adjForm, demandKwDelta: e.target.value})} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white" placeholder="e.g., -15" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Rate $/kWh (optional)</label>
                  <input type="number" step="0.0001" value={adjForm.ratePerKwh} onChange={(e)=>setAdjForm({...adjForm, ratePerKwh: e.target.value})} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white" placeholder="e.g., 0.12" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Demand Charge $/kW (optional)</label>
                  <input type="number" step="0.01" value={adjForm.demandChargeRate} onChange={(e)=>setAdjForm({...adjForm, demandChargeRate: e.target.value})} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white" placeholder="e.g., 15" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm text-gray-400 mb-1">Notes</label>
                  <textarea value={adjForm.notes} onChange={(e)=>setAdjForm({...adjForm, notes: e.target.value})} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white h-20" placeholder="Details about operational differences this period" />
                </div>
              </div>
              <div className="flex items-center gap-3 mb-6">
                <button onClick={submitAdjustment} className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">Save Adjustment</button>
                {adjMsg && <span className={`text-sm ${adjMsg.includes('saved') ? 'text-green-400' : 'text-red-400'}`}>{adjMsg}</span>}
              </div>

              <h4 className="text-white font-medium mb-2">Existing Adjustments</h4>
                ...
                <table className="w-full text-sm text-gray-300">
                  <thead>
                    <tr className="text-left border-b border-gray-800">
                      ...
                      <th className="py-2 pr-4">kWh Δ</th>
                      <th className="py-2 pr-4">kW Δ</th>
                    </tr>
                  </thead>
                  <tbody>
                    {adjustments.map((a) => (
                      <tr key={a.id} className="border-b border-gray-900/50">
                        ...
                        <td className="py-2 pr-4">{a.effectiveStart ? new Date(a.effectiveStart).toLocaleDateString() : '—'}</td>
                        <td className="py-2 pr-4">{a.effectiveEnd ? new Date(a.effectiveEnd).toLocaleDateString() : '—'}</td>
                        <td className="py-2 pr-4">{a.kwhDelta ?? 0}</td>
                        <td className="py-2 pr-4">{a.demandKwDelta ?? 0}</td>
                      </tr>
                    ))}
                    {adjustments.length === 0 && (
                      <tr><td className="py-3 text-gray-500" colSpan={5}>No adjustments yet</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              {parsed?.data && (
                <div className="bg-gray-900/50 rounded-xl p-6 border border-white/10 mt-6">
                  ...
                  <div className="mt-4 flex items-center gap-3">
                    ...
                    >
                      {savingBaseline ? 'Saving…' : 'Use as Baseline'}
                    </button>
                    {baselineMsg && (
                      <span className={`text-sm ${baselineMsg.includes('successfully') ? 'text-green-400' : 'text-red-400'}`}>{baselineMsg}</span>
                    )}
                  </div>
                  <div className="mt-2 text-xs text-gray-500">Baseline will update automatically as enough history accumulates.</div>
                </div>
              )}
      </div>
    </div>
