...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('api', 'Failed to get alert preferences:', error );
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('api', 'Failed to save alert preferences:', error );
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
async function updateUserContactInfo(userId: string, contact: { phone?: string; email?: string }) {
  try {
    ...
    logger.error('api', 'Failed to update user contact info:', error );
  }
}