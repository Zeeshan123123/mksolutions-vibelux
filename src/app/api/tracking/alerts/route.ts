...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === 'SOS' || severity === 'CRITICAL') {
      ...
      const nearbyUsers = await findNearbyUsers(location, radius, facilityId, userId);
    ...
    logger.error('api', 'Alert creation error:', error );
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('api', 'Alert fetch error:', error );
    return NextResponse.json(
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    logger.error('api', 'Alert acknowledge error:', error );
    return NextResponse.json(
