...
export async function POST(request: NextRequest) {
  try {
    ...
    if (apiKeyHeader && cronApiKey && crypto.timingSafeEqual(Buffer.from(apiKeyHeader), Buffer.from(cronApiKey))) {
    ...
    logger.error('api', 'Data retention cleanup error:', error );
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error('api', 'Data retention policy fetch error:', error );
    return NextResponse.json(
