...
export async function GET(request: NextRequest) {
  try {
    ...
    energyReadings.forEach(reading => {
      ...
      day.totalKwh += reading.value; // Value is already in the correct unit
      day.peakDemand = Math.max(day.peakDemand, reading.value);
      day.readings++;
    });
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      ...
      if (dayData) {
        ...
        let baselineKwh = baselineData?.value || 1200;
    ...
    logger.error('api', 'Error fetching energy trends:', error );
