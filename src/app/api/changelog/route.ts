...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedUpdates = updates.map(update => ({
      ...
      features: {
        added: (update.features as any)?.added || [],
        changed: (update.features as any)?.changed || [],
        deprecated: (update.features as any)?.deprecated || [],
        removed: (update.features as any)?.removed || []
      }
    }));
    ...
    logger.error('api', 'Changelog fetch error:', error );
    return NextResponse.json(
