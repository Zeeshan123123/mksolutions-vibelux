  ...
  try {
    ...
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        // Optionally persist session metadata
        break
      }
      case 'invoice.payment_succeeded': {
        const invoice = event.data.object as Stripe.Invoice
        logger.info('api', `Invoice paid: ${invoice.id}`)
        // Minimal example: mark internal invoice paid if we track it
        // await prisma.invoice.update({ where: { externalId: invoice.id }, data: { status: 'paid', paidAt: new Date() } }).catch(() => {})
        break
      }
      ...
      case 'customer.subscription.deleted': {
        ...
        // Optionally sync subscription status to user
        break
      }
      default: {
