...
export async function GET(request: NextRequest) {
  try {
    ...
    const pageAnalytics: PageAnalytics[] = Object.entries(pageGroups).map(([pagePath, views]) => {
      ...
      const dailyData: Array<{date: string, views: number, uniqueUsers: number}> = [];
      for (let i = 6; i >= 0; i--) {
    ...
    logger.error('api', 'Error fetching page analytics:', error );
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('api', 'Error tracking page view:', error );
    return NextResponse.json(
