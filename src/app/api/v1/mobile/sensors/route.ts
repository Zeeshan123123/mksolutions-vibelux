...
export async function GET(request: NextRequest) {
  try {
    ...
    const sensorData = {
      ...
      history: generateSensorHistory(startTime, now, sensorType || undefined),
      statistics: {
        temperature: {
          min: 72.1,
          max: 78.3,
    ...
    logger.error('api', 'Get sensors error:', error );
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('api', 'Sensor subscription error:', error );
    return NextResponse.json(
...
function generateSensorHistory(startTime: Date, endTime: Date, sensorType?: string) {
  ...
  types.forEach(type => {
    ...
    for (let i = 0; i < points; i++) {
      ...
      const timeNoise = Math.sin((timestamp.getTime() + i * 300000) / (1000 * 60 * 60));
