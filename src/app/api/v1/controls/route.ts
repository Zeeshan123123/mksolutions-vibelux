...
export async function POST(request: Request) {
  try {
    ...
    logger.error('api', 'Control command error:', error )
    return NextResponse.json(
...
export async function GET(request: Request) {
  try {
    ...
    logger.error('api', 'Get device states error:', error )
    return NextResponse.json(
...
function validateControlValue(deviceType: string, action: string, value: any): { valid: boolean; error?: string } {
  ...
  if ('ranges' in capabilities) {
    const ranges = (capabilities as any).ranges;
    if (action === 'setTemperature' && ranges.temperature) {
    ...
    if (action === 'setHumidity' && ranges.humidity) {
    ...
    if (action === 'setIntensity' && ranges.intensity) {
    ...
    if (action === 'setTarget' && ranges.target) {
