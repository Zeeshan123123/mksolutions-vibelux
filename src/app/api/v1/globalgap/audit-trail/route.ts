...
export async function GET(req: NextRequest) {
  try {
    ...
    const where: any = {
      userId: user.id,
      eventType: 'globalgap-compliance'
    };

    if (params.category !== 'all') {
      where.eventType = `check-${params.category}`;
    }
    ...
    const [records, total] = await Promise.all([
      prisma.usageRecord.findMany({
        where,
        orderBy: { timestamp: params.sortOrder },
    ...
    const auditTrail = records.map(record => {
      const eventData = record.eventData as any;
      return {
        id: record.id,
        timestamp: record.timestamp,
        category: record.eventType.replace('check-', ''),
        checkType: eventData.checkType,
        result: eventData.result,
        data: params.includeDocuments ? eventData.data : undefined,
