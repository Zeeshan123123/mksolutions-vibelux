...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get facility stats
    const stats = {
      totalProjects: await prisma.facilityProject.count({
        where: { facilityId: facility.id }
      }),
      activeInvestments: await prisma.investmentOpportunity.count({
        where: { 
          facilityId: facility.id,
          status: 'ACTIVE'
        }
      }),
      totalInvestors: await prisma.investment.count({
        where: {
          opportunity: {
            facilityId: facility.id
          },
      ...
      totalFunding: await prisma.investmentOpportunity.aggregate({
        where: { facilityId: facility.id },
    ...
    logger.error('api', 'Error fetching facility:', error );
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    logger.error('api', 'Error creating facility:', error );
    return NextResponse.json(
...
export async function PUT(req: NextRequest) {
  try {
    ...
    logger.error('api', 'Error updating facility:', error );
    return NextResponse.json(
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const activeInvestments = await prisma.investmentOpportunity.count({
      where: {
        facilityId: facility.id,
    ...
    logger.error('api', 'Error deleting facility:', error );
    return NextResponse.json(
