...
// import { plantVisionAI } from '@/lib/plant-vision-ai'; // TODO: Implement plant vision AI module
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('api', 'Plant health analysis error:', error );
    return NextResponse.json(
      { 
...
function calculateHealthMetrics(phenotype: any, diseases: any[], pests: any[]): any {
  ...
  if (phenotype.advancedPhenotyping) {
    Object.entries(phenotype.advancedPhenotyping.nutrientDeficiencyScore).forEach(([nutrient, defScore]) => {
      const defScoreNum = defScore as number;
      if (defScoreNum > 0.3) {
        score -= defScoreNum * 10;
        if (defScoreNum > 0.6) {
    ...
    Object.entries(phenotype.advancedPhenotyping.nutrientDeficiencyScore).forEach(([nutrient, score]) => {
      ...
      if (scoreNum > 0.2) {
        deficiencies.push({
          ...
          severity: scoreNum > 0.6 ? 'severe' : scoreNum > 0.4 ? 'moderate' : 'mild',
          score: scoreNum,
...
async function logHealthAnalysis(report: any): Promise<void> {
  try {
    ...
    logger.error('api', 'Failed to log health analysis:', error );
  }
}