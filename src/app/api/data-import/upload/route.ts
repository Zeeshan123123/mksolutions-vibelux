...
import { safeParseExcel } from '@/lib/security/safe-xlsx';
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error('api', 'File upload error:', error );
    return NextResponse.json(
...
async function processImportFile(importId: string, filepath: string, fileType: string) {
  try {
    ...
    if (fileType === 'csv') {
      ...
      // Use safe Excel parser to prevent prototype pollution
      const fileBuffer = await require('fs').promises.readFile(filepath);
      data = safeParseExcel(fileBuffer);
      headers = Object.keys(data[0] || {});
    ...
    logger.error('api', 'Import processing error:', error );
    await prisma.dataImport.update({
      where: { id: importId },
      data: {
        status: 'FAILED',
...
async function analyzeAndImportData(importId: string, data: any[]) {
  try {
    ...
    // Skip insight generation since HistoricalData doesn't track importId
    ...
    logger.error('api', 'Data analysis error:', error );
    throw error;
  }
}
...
async function generateDataInsights(importId: string) {
  ...
  // Skip data query since HistoricalData doesn't have importId
