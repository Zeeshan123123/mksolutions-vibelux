...
export async function POST(request: NextRequest) {
  ...
  // Enforce photoperiod safety at API boundary as well
  try {
    const { getZoneInfo, isFloweringZone } = await import('@/lib/energy/zones-registry')
    const info = getZoneInfo(validatedData.facilityId, validatedData.zoneId)
    if (isFloweringZone(info)) {
      const minutes = Math.max(0, Math.round((validatedData.endTime.getTime() - validatedData.startTime.getTime()) / 60000))
      if (minutes > 120) {
        return NextResponse.json({ error: 'Photoperiod protection: max 2h shed allowed during lights-on for flowering zones' }, { status: 400 })
      }
      if (validatedData.priority === 1) {
        return NextResponse.json({ error: 'Photoperiod protection: full shedding blocked for flowering zones' }, { status: 400 })
      }
    }
  } catch (e) {
    // If safety check fails parsing, proceed to service-level guard which will also catch
  }
