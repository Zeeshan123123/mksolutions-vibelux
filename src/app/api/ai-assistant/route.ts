...
import { rateLimit, rateLimitHeaders, redact } from '@/lib/security/rate-limit'
...
// Claude client will be initialized lazily
...
export async function POST(request: NextRequest) {
  try {
    ...
    // TODO: Fix usage tracking when schema is available
    // const currentMonth = new Date().toISOString().slice(0, 7) // YYYY-MM
    const monthlyUsage = 0; // Temporary - disable usage tracking
    ...
    // Per-user rate limit: 60 req / 10 min
    const key = `ai-assistant:${userId}`
    const rl = await rateLimit(key, 60, 10 * 60 * 1000)
    if (!rl.ok) {
      return new NextResponse(JSON.stringify({ error: 'Rate limit exceeded. Please wait and try again.' }), {
        status: 429,
    ...
    if (!response) {
      logger.error('api', 'Claude API error: No response')
    ...
    const content = (response.content[0] as any)?.text || 'Sorry, I could not generate a response.'
    ...
    try {
      ...
      logger.error('ai', 'AI Assistant error', {
        userId,
