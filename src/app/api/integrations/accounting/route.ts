...
// Stub imports
const createAccountingAdapter = (...args: any[]) => ({ 
  connect: async () => ({ connected: false }),
  getCustomers: async () => [],
  getItems: async () => [],
  getInvoices: async (...args: any[]) => [],
  getExpenses: async (...args: any[]) => [],
  getProfitAndLoss: async (...args: any[]) => ({ revenue: 0, expenses: 0, profit: 0 }),
  createInvoice: async () => ({ id: 'mock-invoice' }),
  syncTransactions: async () => ({ synced: 0 }),
  getBalanceSheet: async (...args: any[]) => ({ assets: 0, liabilities: 0, equity: 0 }),
  getCashFlow: async (...args: any[]) => ({ operating: 0, investing: 0, financing: 0 }),
  getRevenue: async (...args: any[]) => ({ total: 0, breakdown: [] }),
  getAssets: async () => [],
  getProviderInfo: () => ({ name: 'mock', capabilities: [] })
});
const getSupportedProviders = (): any[] => [];
const validateProviderConfig = (...args: any[]) => ({ valid: false, errors: [] });
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const adapter = createAccountingAdapter(integration.provider, {
      ...
      companyId: (integration.config as any)?.companyId,
      accessToken: integration.accessToken,
      refreshToken: integration.refreshToken,
      sandbox: (integration.config as any)?.sandbox || false
    });
    ...
    logger.error('api', 'Accounting API error:', error );
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.error('api', 'Integration setup error:', error );
    return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const whereClause = integrationId 
      ...
      : { userId, facilityId: facilityId || '', provider: provider || '' };
    ...
    logger.error('api', 'Integration deletion error:', error );
    return NextResponse.json(
