...
import { ProfessionalDrawingGenerator } from '@/lib/reports/professionalConstructionDrawings';
import type { ConstructionConfig } from '@/lib/reports/constructionDrawingGenerator';
...
export type DesignerAction = 
  ...
  | { type: 'GENERATE_SHEET_SET'; params: { disciplines: Array<'architectural'|'electrical'|'mechanical'|'plumbing'|'structural'> } }
  | { type: 'ADD_ELECTRICAL_PLAN'; params: { includePanelSchedules?: boolean } }
  | { type: 'ADD_HVAC_PLAN'; params: { method?: 'load-based'|'rule-of-thumb' } }
  | { type: 'ADD_PLUMBING_RISER'; params: { zones?: number, designFlowGpm?: number } }
  | { type: 'CREATE_WBS'; params: { phases?: string[] } };
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Mode: build-greenhouse - end-to-end plan + sheet set + WBS
    if (mode === 'build-greenhouse') {
      // 1) Create a suggested action sequence for A/S/MEP
      const actions: DesignerAction[] = [
        { type: 'GENERATE_SHEET_SET', params: { disciplines: ['architectural','electrical','mechanical','plumbing','structural'] } },
        { type: 'ADD_ELECTRICAL_PLAN', params: { includePanelSchedules: true } },
        { type: 'ADD_HVAC_PLAN', params: { method: 'load-based' } },
        { type: 'ADD_PLUMBING_RISER', params: { zones: 4, designFlowGpm: 20 } },
        { type: 'CREATE_WBS', params: { phases: ['Design','Permitting','Procurement','Installation','Commissioning'] } }
      ];

      // 2) Generate plumbing riser now as a deliverable example
      const { generatePlumbingRiser } = await import('@/lib/plumbing/PlumbingRiserGenerator');
      const riser = generatePlumbingRiser({
        room: {
          width: currentState?.room?.width || 20,
          length: currentState?.room?.length || 40,
          height: currentState?.room?.height || 12
        },
        irrigationZones: 4,
        designFlowGpm: 20
      });

      // 3) Suggest WBS structure for PM
      const wbs = [
        { id: 'PH1', name: 'Design', tasks: ['Architectural Plans','Electrical Plans','Mechanical Plans','Plumbing Riser','Structural Notes'] },
        { id: 'PH2', name: 'Permitting', tasks: ['Submittals','AHJ Coordination','Revisions'] },
        { id: 'PH3', name: 'Procurement', tasks: ['Long-Lead Items','Vendor POs','Logistics'] },
        { id: 'PH4', name: 'Installation', tasks: ['Site Prep','Electrical Install','HVAC Install','Plumbing Install','Controls'] },
        { id: 'PH5', name: 'Commissioning', tasks: ['Functional Tests','As-Builts','Training','Turnover'] }
      ];

      // 4) Try to create a Project and tasks (best effort)
      ...
      try {
        ...
        if (owner) {
          const project = await prisma.project.create({
            data: {
              name: `Greenhouse Project - ${new Date().toISOString().slice(0,10)}`,
              description: 'AI-generated greenhouse project (A/S/MEP + PM WBS)',
              ownerId: owner.id,
            }
          });
          projectId = project.id;

          // Flatten WBS to tasks
          const taskCreates = wbs.flatMap((phase, idx) =>
          ...
          // Create tasks if model exists
          for (const t of taskCreates) {
            try {
              // @ts-ignore - model shape may vary
              await prisma.projectTask.create({ data: t });
            } catch (e) {
              // Ignore if model/fields differ in this environment
              logger.warn('api', 'ProjectTask create failed (skipped)', e as Error);
              break;
            }
          }
        }
      } catch (e) {
        logger.warn('api', 'Project/WBS creation skipped', e as Error);
      }

      // 4) Generate basic A/E sheets (floor plan + electrical plan) as a single PDF (best effort)
      let drawings: { dataUrl?: string } = {};
      try {
        const cfg: ConstructionConfig = {
          project: {
            name: 'AI Greenhouse',
            client: 'VibeLux Client',
            location: 'TBD',
            number: 'PRJ-AI-001',
            date: new Date()
          },
          dimensions: {
            width: currentState?.room?.width || 40,
            length: currentState?.room?.length || 60
          },
          zones: 3,
          fixtures: (currentState?.objects || []).filter((o: any) => o.type === 'fixture'),
        } as any;

        const generator = new ProfessionalDrawingGenerator(cfg, 4);
        generator.addProfessionalTitleBlock('A-101', 'FLOOR PLAN', '1/8" = 1\' - 0"');
        generator.drawFloorPlan(cfg);
        // E sheet
        const pdf = generator.getPDF();
        pdf.addPage('landscape', [36, 24]);
        generator.addProfessionalTitleBlock('E-101', 'ELECTRICAL PLAN', '1/8" = 1\' - 0"');

        // Minimal electrical system mock from fixtures
        const electricalSystem = {
          panels: [
            {
              name: 'LP-1',
              specs: { voltage: 480, amperage: 225, circuits: 42 },
              location: 'ELEC ROOM',
              circuits: Array.from({ length: 12 }).map((_, i) => ({
                number: i + 1,
                name: `Lighting Circuit ${i + 1}`,
                breaker: { amperage: 20 },
                wire: { gauge: '12 AWG', length: 80 },
                conduit: { size: '3/4"' },
                fixtures: [],
                actualLoad: 12 + (i % 3),
              }))
        ...
        generator.drawElectricalPlan(cfg, electricalSystem);

        // M sheet - HVAC plan (placeholder schedule + zones)
        pdf.addPage('landscape', [36, 24]);
        generator.addProfessionalTitleBlock('M-101', 'HVAC PLAN', 'NTS');
        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(14);
        pdf.text('HVAC EQUIPMENT SCHEDULE', 18, 3, { align: 'center' });
        pdf.setFontSize(8);
        pdf.setFont('helvetica', 'normal');
        // Simple table
        pdf.rect(2, 3.5, 32, 1, 'S');
        pdf.text('TAG', 2.5, 4.1);
        pdf.text('DESCRIPTION', 5, 4.1);
        pdf.text('CAPACITY', 14, 4.1);
        pdf.text('CFM', 18, 4.1);
        pdf.text('POWER', 22, 4.1);
        pdf.text('LOCATION', 26, 4.1);
        const hvacRows = [
          ['AHU-1', 'Air Handler Unit', '10 TON', '4000', '9 kW', 'MECHANICAL'],
          ['DX-1', 'DX Condensing Unit', '10 TON', '-', '—', 'ROOF'],
          ['DEH-1', 'Desiccant Dehumidifier', '—', '1500', '2 kW', 'MECHANICAL'],
        ];
        let y = 4.8;
        hvacRows.forEach(row => {
          pdf.rect(2, y - 0.4, 32, 0.8, 'S');
          pdf.text(row[0], 2.5, y);
          pdf.text(row[1], 5, y);
          pdf.text(row[2], 14, y);
          pdf.text(row[3], 18, y);
          pdf.text(row[4], 22, y);
        ...
        // Zone labels
        pdf.setFont('helvetica', 'bold');
        pdf.text('ZONES:', 2, y + 1);
        pdf.setFont('helvetica', 'normal');
        pdf.text('Zone 1 - Cultivation A', 2, y + 1.6);
        pdf.text('Zone 2 - Cultivation B', 2, y + 2.1);
        pdf.text('Zone 3 - Drying', 2, y + 2.6);

        // P sheet - Plumbing riser (render nodes and connections)
        pdf.addPage('landscape', [36, 24]);
        generator.addProfessionalTitleBlock('P-101', 'PLUMBING RISER', 'NTS');
        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(12);
        pdf.text('IRRIGATION RISER DIAGRAM', 18, 3, { align: 'center' });
        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(8);
        let baseX = 8, baseY = 6;
        // Draw nodes
        const drawNode = (x: number, y: number, label: string) => {
          pdf.rect(x - 0.6, y - 0.3, 1.2, 0.6);
          pdf.text(label, x, y + 0.05, { align: 'center' });
        };
        drawNode(baseX, baseY, 'Nutrient Tank');
        drawNode(baseX + 6, baseY, 'Pump');
        drawNode(baseX + 12, baseY, 'Filter');
        // Connections
        pdf.line(baseX + 0.6, baseY, baseX + 5.4, baseY);
        pdf.line(baseX + 6.6, baseY, baseX + 11.4, baseY);
        // Zones
        const zones = 4;
        for (let i = 0; i < zones; i++) {
          const zx = baseX + 12 + (i + 1) * 3;
          pdf.circle(zx, baseY, 0.25);
          pdf.text(`Z${i + 1}`, zx, baseY + 0.7, { align: 'center' });
          pdf.line(baseX + 12.6, baseY, zx - 0.25, baseY);
        }

        // Export to data URL
        const dataUrl = pdf.output('datauristring');
        drawings.dataUrl = dataUrl;
      } catch (e) {
        logger.warn('api', 'Drawing generation skipped', e as Error);
      }

      // 5) Persist PDF to project as an attachment via review tasks (best effort)
      let reviewTaskIds: string[] = [];
      try {
        if (projectId && drawings.dataUrl) {
          const disciplines = [
            { title: 'Review Architectural Plan (A-101)', taskType: 'planning' },
            { title: 'Review Electrical Plan (E-101)', taskType: 'electrical' },
            { title: 'Review HVAC Plan (M-101)', taskType: 'HVAC' },
            { title: 'Review Plumbing Riser (P-101)', taskType: 'plumbing' }
          ];
            ...
            try {
              ...
              const t = await prisma.projectTask.create({
                data: {
                  projectId,
                  title: d.title,
                  description: 'Open the attached multi-sheet PDF and validate scope before permitting.',
                  taskType: d.taskType,
                  dependencies: [],
                  blockedBy: [],
                  assignedTo: [],
                  ...
                  status: 'pending',
                  progress: 0,
                  documents: [drawings.dataUrl]
                }
              });
              reviewTaskIds.push(t.id);
            } catch (e) {
              logger.warn('api', `Task create failed for ${d.title} (skipped)`, e as Error);
              break;
            }
          }
        }
      } catch (e) {
        logger.warn('api', 'Failed to attach drawings to project', e as Error);
      }
