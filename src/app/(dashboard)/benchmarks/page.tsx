...
export default function BenchmarksPage() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      // Fetch subscription via unified endpoint
  ...
  const fetchFacilityData = async () => {
    try {
      // Fetch reports list for facility
      const reportsRes = await fetch(`/api/benchmarks/reports?facilityId=${selectedFacility}`);
      if (reportsRes.ok) {
      ...
      logger.error('system', 'Error fetching facility data:', error );
    }
  };

  const handleGenerateReport = async (reportType: string) => {
    try {
      const response = await fetch('/api/market-data/analytics', {
        method: 'POST',
