...
const ReportCustomizer = dynamic(
  () => import('@/components/benchmarks/report-customizer').then(mod => mod.ReportCustomizer),
  { ssr: false }
);

const InteractiveAnalyticsDashboard = dynamic(
  () => import('@/components/benchmarks/interactive-analytics-dashboard').then(mod => mod.InteractiveAnalyticsDashboard),
  { ssr: false }
);
...
export default function AdvancedBenchmarksPage() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      logger.error('system', 'Error fetching user data:', error );
    }
  };
  ...
  const handleExportReport = async (format: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // ✅ Only run download logic in browser
