...
// Minimal stub for climate computer integrations to keep UI functional
const climateComputerManager = {
  getDevices: () => [],
  connectToDevice: async () => null,
  readData: async () => ({}),
  getAllEquipment: async (_facilityId?: string) => [],
  syncAll: async () => {},
  getIntegrationStatus: () => [],
  autoDiscover: async () => [],
  addIntegration: async (_config: any) => {}
};
...
export function HMIDashboard({ facilityId, roomId }: HMIDashboardProps) {
  ...
  const [bmsHours, setBmsHours] = useState(24);
  const [bmsRecent, setBmsRecent] = useState<Array<{ timestamp: Date; value: number; unit: string }>>([]);
  ...
  // Load recent BMS energy readings
  useEffect(() => {
    const loadBmsRecent = async () => {
      try {
        if (!facilityId) return;
        const res = await fetch(`/api/bms/recent?facilityId=${encodeURIComponent(facilityId)}&hours=${bmsHours}`).catch(() => null)
        if (res && res.ok) {
          const data = await res.json().catch(() => ({}))
          const rows = Array.isArray(data?.rows) ? data.rows : []
          const mapped = rows
            .filter((r: any) => r.readingType === 'electricity')
            .map((r: any) => ({ timestamp: new Date(r.timestamp), value: Number(r.value || 0), unit: r.unit || 'kWh' }))
            .reverse()
          setBmsRecent(mapped)
        }
      } catch {}
  ...
  const discoverEquipment = async () => {
    ...
    try {
      ...
      try {
        ...
        logger.warn('system', 'Climate computer discovery failed, using demo data:', error);
      }
      ...
      logger.error('system', 'Equipment discovery failed:', error );
    } finally {
      setIsScanning(false);
      setIsLoading(false);
    }
  ...
  const saveLayout = async (positions: Record<string, any>) => {
    try {
      await fetch('/api/hmi/layout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ facilityId, roomId, positions }),
      })
    } catch {}
  }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-gray-900 rounded-lg p-4">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-medium">Energy Usage (Recent)</h3>
                  <div className="flex items-center gap-2 text-sm">
                    <span className="text-gray-400">Hours</span>
                    <select
                      value={bmsHours}
                      onChange={e => setBmsHours(Math.max(1, Math.min(168, Number(e.target.value))))}
                ...
                <TelemetryChart
                  data={bmsRecent.map(p => ({ timestamp: p.timestamp, value: p.value }))}
                  label="Electricity"
                  unit={bmsRecent[0]?.unit || 'kWh'}
                  color="#22c55e"
                  min={0}
                  max={Math.max(1, Math.ceil((bmsRecent.reduce((m, p) => Math.max(m, p.value), 0)) * 1.2))}
                />
              </div>
            </div>
          </div>
        ...
        <EquipmentSetupWizard
          onComplete={(newEquipment) => {
            const equipmentWithIds = newEquipment.map(eq => ({
              ...eq,
              id: eq.id || `${eq.type}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`
            }));
...
function ClimateComputerIntegrationsModal({ onClose, onIntegrationAdded }: {
  ...
  const autoDiscover = async () => {
    ...
    try {
      ...
      logger.error('system', 'Auto-discovery failed:', error );
    } finally {
  ...
  const addIntegration = async (config: any) => {
    try {
      ...
      logger.error('system', 'Failed to add integration:', error );
    }
  };
