...
const MemoizedLineChart = React.memo(({ trendData }: { trendData: any[] }) => (
...
const MemoizedPieChart = React.memo(({ costBreakdown }: { costBreakdown: any[] }) => (
  <ResponsiveContainer width="100%" height="100%">
    <PieChart>
      <Pie
        data={costBreakdown}
...
export const OperationalInsights = React.memo(() => {
  const [timeRange, setTimeRange] = useState<'day' | 'week' | 'month' | 'cycle'>('week');
  const [selectedRoom, setSelectedRoom] = useState<string>('all');

  // Memoized simulated operational data
  ...
  // Memoized environmental correlations with yield
  const correlations: EnvironmentalCorrelation[] = useMemo(() => [
    {
  ...
  const trendData = useMemo(() => Array.from({ length: 7 }, (_, i) => ({
    ...
    yield: 62 + Math.random() * 8,
    energy: 44 + Math.random() * 4,
    quality: 90 + Math.random() * 5
  })), []);

  const getMetricStatus = useCallback((current: number, target: number, inverse: boolean = false) => {
  ...
  const handleTimeRangeChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    setTimeRange(e.target.value as 'day' | 'week' | 'month' | 'cycle');
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-6 gap-4">
        <MetricCard 
          icon={Leaf}
          value={metrics.yieldPerSqFt.toFixed(1)}
          unit="g/sq ft"
          label="Yield"
          target="Target: 70"
          status={getMetricStatus(metrics.yieldPerSqFt, 70)}
        />

        <MetricCard 
          icon={Zap}
          value={metrics.energyPerGram.toFixed(1)}
          unit="kWh/g"
          label="Energy"
          target="Target: <40"
          status={getMetricStatus(metrics.energyPerGram, 40, true)}
        />

        <MetricCard 
          icon={Droplets}
          value={metrics.waterPerGram.toFixed(1)}
          unit="L/g"
          label="Water"
          target="Target: <3.5"
          status={getMetricStatus(metrics.waterPerGram, 3.5, true)}
        />

        <MetricCard 
          icon={Users}
          value={metrics.laborHoursPerPound.toFixed(1)}
          unit="hrs/lb"
          label="Labor"
          target="Target: <2.0"
          status={getMetricStatus(metrics.laborHoursPerPound, 2.0, true)}
        />
        ...
        <MetricCard 
          ...
          label="Quality"
          target="Target: >90%"
          status={getMetricStatus(metrics.qualityScore, 90)}
        />

        <MetricCard 
          icon={DollarSign}
          value={`$${metrics.costPerGram.toFixed(2)}`}
          unit="per gram"
          label="Cost"
          target="Target: <$1.50"
          status={getMetricStatus(metrics.costPerGram, 1.50, true)}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
          ...
          <div className="h-64">
            <MemoizedPieChart costBreakdown={costBreakdown} />
          </div>
      ...
      <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
        ...
        <div className="h-64">
          <MemoizedLineChart trendData={trendData} />
        </div>
      </div>
...
OperationalInsights.displayName = 'OperationalInsights';