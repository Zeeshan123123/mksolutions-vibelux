...
export function PsychrometricCalculator() {
  ...
  const getLocationAndWeather = async () => {
    ...
    try {
      ...
      logger.error('system', 'Geolocation/Weather error:', error );
    } finally {
  ...
  const exportReport = () => {
    ...
    doc.text('Psychrometric Analysis Report', pageWidth / 2, yPosition, { align: 'center' });
    ...
    const inputData = [
      ...
      [inputs.inputType === 'relativeHumidity' ? 'Relative Humidity' : 
        ...
        inputs.inputType === 'dewPoint' ? 'Dew Point' : 'Absolute Humidity', 
        `${inputs.secondaryValue}${inputs.inputType === 'relativeHumidity' ? '%' : '°C'}`],
    ...
    doc.setFontSize(16);
    doc.setTextColor(40, 44, 52);
    doc.text('Calculated Properties', 20, yPosition);
    yPosition += 10;
    ...
    const calculatedData = [
      ...
      ['VPD', `${formatValue(results.vaporPressureDeficit, 'kPa')}`]
    ];
    ...
    doc.setFontSize(16);
    doc.setTextColor(40, 44, 52);
    doc.text('Plant Health Assessment', 20, yPosition);
    yPosition += 10;

    const healthData = [
      ['Stress Level', results.plantStressLevel ? results.plantStressLevel.charAt(0).toUpperCase() + results.plantStressLevel.slice(1) : 'Unknown'],
    ...
    if (weatherData) {
      ...
      doc.setFontSize(16);
      doc.setTextColor(40, 44, 52);
      doc.text('Outdoor Conditions', 20, yPosition);
      yPosition += 10;
      ...
      if (weatherData.solarRadiation) {
        weatherInfo.push(['Solar Radiation', `${weatherData.solarRadiation.toFixed(0)} W/m²`]);
      }
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      doc.setFontSize(8);
      doc.setTextColor(100, 100, 100);
      doc.text(`VibeLux Psychrometric Analysis Report - Page ${i} of ${totalPages}`, pageWidth / 2, pageHeight - 10, { align: 'center' });
    }

    // Save the PDF
    const fileName = `VibeLux_Psychrometric_Report_${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fileName);
  };
