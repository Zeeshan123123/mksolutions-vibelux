...
import { Bot, Send, X, Minimize2, Maximize2, Wand2, Sparkles, Loader2, RotateCcw, Zap, Coins, AlertCircle } from 'lucide-react';
import { useDesigner } from './designer/context/DesignerContext';
// useNotifications import removed - using showNotification from useDesigner instead
...
import { CREDIT_COSTS } from '@/lib/credits/credit-manager';
...
export function AIDesignAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      text: "👋 Hi! I'm your AI Design Assistant powered by Claude. I can help you create and optimize your lighting design.\n\n💰 **Credit System**: Each AI request uses credits (Simple: 10, Complex: 25, Full optimization: 50)",
      sender: 'assistant',
      timestamp: new Date()
    },
    {
      id: '2',
      text: "**Try simple commands:**\n" +
            "• 'Create a 40x60 room with 12 foot ceilings'\n" +
            "• 'Add 4x8 grid of fixtures for 800 PPFD'\n" +
            "• 'Add 3 rows of rolling benches'\n\n" +
            "**🚀 NEW! Complex multi-step requests:**\n" +
            "• 'Build a 20x40 room with 3 racks that are 5x20 feet, 2.5 feet apart, with fixtures mounted widthwise for 200 PPFD and 0.3 m/s airflow'\n" +
            "• 'Create a lettuce production room: 30x50 feet, 4 racks spaced 3 feet apart, optimize lighting for 180 PPFD, add fans for proper airflow'\n" +
            "• 'Set up a cannabis flower room with 800 PPFD on 4x8 rolling benches in 3 rows with 2 foot aisles'\n\n" +
            "**🌱 Advanced plant system with crop-specific intelligence:**\n" +
            "• 'Add butterhead lettuce with 8 inch spacing' (6\" tall crop)\n" +
            "• 'Place high-wire tomatoes with 18 inch spacing' (15ft tall - will suggest interlighting!)\n" +
            "• 'Add cannabis plants in flowering stage' (suggests under-canopy lighting)\n" +
            "• 'Fill racks with basil plants' (suggests blue spectrum for essential oils)\n" +
            "• 'Add strawberries' (suggests far-red for flowering)\n\n" +
            "**🔆 Intelligent lighting strategies:**\n" +
            "• Automatic interlighting for tall crops (tomatoes at 5ft, 10ft, 15ft)\n" +
            "• Under-canopy lighting for dense crops (cannabis lower buds)\n" +
            "• Spectrum optimization per crop type\n" +
            "• Growth stage-aware lighting (clone → vegetative → flowering)\n\n" +
            "**💡 Intelligent features:**\n" +
            "• Understands measurements: inches, feet, meters\n" +
            "• Knows crop requirements: spacing, PPFD, airflow\n" +
            "• Validates designs for feasibility\n" +
  ...
  // Listen for externally dispatched AI actions (e.g., one-click build)
  useEffect(() => {
    const handler = (e: Event) => {
      const custom = e as CustomEvent;
      const actions = custom.detail;
      if (Array.isArray(actions) && actions.length > 0) {
        executeActions(actions);
      }
    };
    window.addEventListener('designer.applyActions' as any, handler as any);
    return () => window.removeEventListener('designer.applyActions' as any, handler as any);
  }, [executeActions]);
  ...
  const getSuggestions = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logger.warn('system', 'Failed to get suggestions:', { data: response.status });
      }
    } catch (error) {
      logger.error('system', 'Error getting suggestions:', error );
    ...
    for (const action of actions) {
      try {
        switch (action.type) {
          ...
          case 'ADD_PLANT': {
            const { x, y, variety, growthStage = 'vegetative', width = 2, length = 2, targetDLI = 15, cropId } = action.params;
            ...
            addObject({
              ...
              width,
              length,
              height: variety === 'high-wire' || variety === 'high-wire-tomato' ? 6 : 1,
              enabled: true,
              variety,
              growthStage,
              targetDLI: targetDLI || dliTargets[variety] || 15,
              customName: `${variety} - ${growthStage}`,
              cropId: cropId || variety
            });
          ...
          case 'ADD_PLANTS_ARRAY': {
            ...
            // Use advanced crop database for accurate sizes
            const advancedCropId = cropId || mapVarietyToCropId(variety);
            const cropSpec = ADVANCED_CROP_DATABASE[advancedCropId];
            
            // Plant-specific properties with fallbacks
            const plantSizes: Record<string, { width: number; length: number; height: number }> = {
              'lettuce': { width: 0.8, length: 0.8, height: 0.5 },
              'butterhead-lettuce': { width: 0.8, length: 0.8, height: 0.5 },
              'hemp': { width: 4, length: 4, height: 4 },
              'cannabis-sativa': { width: 4, length: 4, height: 4 },
              'high-wire': { width: 2, length: 2, height: 15 },
              'high-wire-tomato': { width: 2, length: 2, height: 15 },
              'tomato': { width: 2, length: 2, height: 6 },
              'cannabis': { width: 4, length: 4, height: 4 },
              'herbs': { width: 1.5, length: 1.5, height: 2 },
              'genovese-basil': { width: 1.5, length: 1.5, height: 2 },
              'strawberry': { width: 1, length: 1, height: 0.7 },
              'microgreens': { width: 2, length: 1, height: 0.25 }
            };
            
            // Helper function to map variety to crop ID
            function mapVarietyToCropId(variety: string): string {
            ...
            // Use DLI from crop spec if available
            const dliTargets: Record<string, number> = {
              'lettuce': 14,
              'butterhead-lettuce': 14,
              'hemp': 45,
              'cannabis-sativa': 45,
              'high-wire': 30,
              'high-wire-tomato': 30,
            ...
            const plantSize = plantSizes[advancedCropId] || plantSizes[variety] || { width: 2, length: 2, height: 2 };
            const targetDLI = dliTargets[advancedCropId] || dliTargets[variety] || 15;
          ...
          case 'ADD_INTERLIGHTING': {
            const { plantRows, cropId, roomDimensions } = action.params;
            let interlightCount = 0;
            
            // Calculate interlighting positions based on crop
            const layerHeights = {
              'high-wire-tomato': [5, 10], // 5ft and 10ft for 15ft tall plants
              'tomato': [3, 6], // Standard tomatoes
              'cannabis-sativa': [2], // Single layer for bushy plants
            };
            
            const heights = layerHeights[cropId as keyof typeof layerHeights] || [3];
            
            plantRows.forEach((row: any) => {
              heights.forEach(height => {
                addObject({
                  ...
                  dimmingLevel: 100,
                  customName: `Interlight ${cropId} ${height}ft`
                });
                interlightCount++;
              });
            });
            
            showNotification('success', `Added ${interlightCount} interlighting fixtures for ${cropId}`);
            break;
          }
          
          case 'ADD_UNDER_CANOPY_LIGHTING': {
            const { plantPositions, cropId } = action.params;
            let underCanopyCount = 0;
            
            // Spectrum based on crop
            const spectrumMap = {
              'cannabis-sativa': 'red-heavy',
              'genovese-basil': 'blue-heavy',
              'strawberry': 'far-red'
            };
            
            const spectrum = spectrumMap[cropId as keyof typeof spectrumMap] || 'red-heavy';
            
            plantPositions.forEach((plant: any) => {
              // Place 4 small fixtures around each plant
              const angles = [0, 90, 180, 270];
              angles.forEach(angle => {
                const rad = (angle * Math.PI) / 180;
                const offset = plant.canopyRadius * 0.8;
                
                addObject({
                  type: 'fixture',
                  fixtureType: 'under-canopy',
                  x: plant.x + Math.cos(rad) * offset,
                  y: plant.y + Math.sin(rad) * offset,
                  ...
                  dimmingLevel: 100,
                  customName: `UC-${spectrum}`
                });
                underCanopyCount++;
              });
            });
            
            showNotification('success', `Added ${underCanopyCount} under-canopy ${spectrum} fixtures for ${cropId}`);
            break;
          }
        ...
        logger.error('system', 'Error executing action:', error );
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          setMessages(prev => [...prev, {
            ...
            text: `🔐 **Authentication Required**\n\n${errorData.message || 'Please sign in to use the AI Designer.'}\n\nPlease refresh the page and sign in to continue.`,
            sender: 'assistant',
            timestamp: new Date()
          }]);
          setIsLoading(false);
          return;
        } else if (response.status === 429 && errorData.upgradeRequired) {
        ...
        throw new Error(`API Error: ${response.status} - ${errorData.message || 'Failed to process request'}`);
      }
      ...
      setMessages(prev => [...prev, {
        ...
        text: 'Sorry, I encountered an error processing your request with Claude. Please try again.',
        sender: 'assistant',
        timestamp: new Date()
      }]);
    } finally {
...
**Status:** ${data.analysis.status.replace('-', ' ')}
      ...
      logger.error('system', 'Error analyzing design:', error );
...
*Recommendations verified against live research databases including ResearchGate, ArXiv, and peer-reviewed journals.*
      ...
      logger.error('system', 'Error getting fact-based recommendations:', error );
                ...
                <div className="flex items-center gap-2">
                  ...
                  {creditBalance && (
                    <div className="flex items-center gap-1 text-xs text-gray-400">
                      ...
                      <span>{creditBalance.available} credits</span>
                    </div>
                  )}
          ...
          {!isMinimized && (
            <>
              ...
              <div className="p-4 border-t border-gray-700">
                {creditBalance && creditBalance.available < 10 && (
                  <div className="mb-2 flex items-center gap-2 p-2 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                    <AlertCircle className="w-4 h-4 text-yellow-400 flex-shrink-0" />
                    <span className="text-xs text-yellow-400">Low credits! {creditBalance.available} remaining.</span>
                    <Link href="/pricing#credits" className="text-xs text-yellow-400 underline hover:text-yellow-300">
                      Get more
                    </Link>
                  </div>
                )}
