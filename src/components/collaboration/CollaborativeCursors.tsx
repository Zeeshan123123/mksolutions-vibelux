...
const sanitizeColor = (color: string): string => {
  ...
  const validColorRegex = /^(#[0-9a-fA-F]{3,6}|[a-zA-Z]+)$/;
  if (!validColorRegex.test(color)) {
    return '#888888'; // Default gray color for invalid inputs
  }
  return color;
};
...
export function CollaborativeSelections({ 
  ...
  useEffect(() => {
    ...
    selections.forEach((selection, userId) => {
      ...
      // Safely escape all text content and validate user data
      const escapedBefore = escapeHtml(before);
      const escapedSelected = escapeHtml(selected);
      const escapedAfter = escapeHtml(after);
      const sanitizedColor = sanitizeColor(user.user.color);
      const escapedUserName = escapeHtml(user.user.name || 'Unknown User');

      // Create highlighted version with sanitized content
      const highlightedHTML = `
        ${escapedBefore}
        <span 
          class="collaborative-selection" 
          style="background-color: ${sanitizedColor}33; border-bottom: 2px solid ${sanitizedColor};"
      ...
      // Use DOMPurify as an additional safety layer
      element.innerHTML = DOMPurify.sanitize(highlightedHTML);
    })
