...
import { EnhancedChartRenderer } from './EnhancedChartRenderer.stub'
...
function EnhancedCustomReportingBuilderContent() {
  ...
  const handleProfessionalExport = async (sections: any[], format: string) => {
    ...
    try {
      if (format === 'pdf') {
        ...
        addNotification({ type: 'success', title: 'Professional PDF report generated successfully!' })
      ...
      logger.error('system', 'Error generating professional export:', error )
      addNotification({ type: 'error', title: 'Error generating report', message: error instanceof Error ? error.message : 'Unknown error' })
    } finally {
  ...
  const exportReportAsWord = async (sections: any[]) => {
    ...
    addNotification({ type: 'info', title: 'Word export feature coming soon!' })
  }
  ...
  const exportReport = async (format: string) => {
    try {
      ...
      try {
        ...
        logger.error('system', 'Error generating PDF:', error )
      } finally {
        setGeneratingPDF(false)
      }
    }
    } catch (error) {
      logger.error('system', 'Error exporting report:', error )
      addNotification({ type: 'error', title: 'Error exporting report', message: error instanceof Error ? error.message : 'Unknown error' })
      if (format === 'pdf') {
        setGeneratingPDF(false)
      }
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={async () => {
                ...
                addNotification({ type: 'success', title: `Report "${reportName}" saved successfully!` })
