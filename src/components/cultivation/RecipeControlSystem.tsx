...
export function RecipeControlSystem() {
  ...
  const [newDuration, setNewDuration] = useState(activeRecipe.duration);
  const [showDurationPresets, setShowDurationPresets] = useState(false);

  const durationPresets = [
    { label: 'Microgreens', days: 7 },
    { label: 'Herbs (Basil)', days: 21 },
    { label: 'Leafy Greens', days: 35 },
    { label: 'Lettuce', days: 45 },
    { label: 'Tomatoes', days: 90 },
    { label: 'Cannabis Veg', days: 30 },
    { label: 'Cannabis Flower', days: 60 },
    { label: 'Strawberries', days: 120 },
    { label: 'Peppers', days: 75 },
    { label: 'Custom', days: 0 }
  ];

  const updateDuration = () => {
    if (newDuration === activeRecipe.duration) {
      setEditingDuration(false);
      return;
    }

    const newSetpoints: SetpointData[] = Array.from({ length: newDuration }, (_, i) => {
      // If we have existing data for this day, use it
      if (i < activeRecipe.setpoints.length) {
        return activeRecipe.setpoints[i];
      }
      
      // Otherwise, generate new data based on the pattern
      const baseTemp = 72 + Math.sin(i * 0.2) * 2;
      const baseHumidity = 65 + Math.cos(i * 0.15) * 5;
      const baseCO2 = 800 + (i / newDuration) * 400;
      const basePH = 5.8 + Math.sin(i * 0.1) * 0.2;
      const baseEC = 1.2 + (i / newDuration) * 0.8;
      ...
      return {
        ...
        humidity: {
          day: baseHumidity,
          night: baseHumidity + 8
        },
        co2: {
          day: baseCO2,
          night: Math.max(400, baseCO2 - 300)
        },
        lightIntensity: {
          day: Math.min(100, 40 + (i / newDuration) * 60),
          night: 0
        },
        dosingA: {
          day: 100,
    ...
    // Adjust current day if it's beyond the new duration
            ...
            <div className="relative">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1 px-2 py-1 bg-gray-700 hover:bg-gray-600 text-white rounded border border-gray-600"
                >
                ...
                <input
                  type="number"
                  value={newDuration}
                  onChange={(e) => setNewDuration(Math.max(1, Math.min(365, parseInt(e.target.value) || 1)))}
                  className="w-20 px-2 py-1 bg-gray-700 text-white rounded border border-gray-600 focus:border-purple-500 focus:outline-none"
                  min="1"
                  max="365"
                />
                <button
                ...
                <button
                  onClick={() => {
                    setNewDuration(activeRecipe.duration);
                    setEditingDuration(false);
                ...
                <div className="absolute top-full left-0 mt-1 w-48 bg-gray-800 border border-gray-700 rounded-lg shadow-lg z-10">
                  {durationPresets.map((preset) => (
                    <button
                      ...
                      onClick={() => {
                        if (preset.days > 0) {
                          setNewDuration(preset.days);
                        }
                        setShowDurationPresets(false);
                      }}
                      className="w-full text-left px-3 py-2 text-sm hover:bg-gray-700 text-gray-300 hover:text-white transition-colors first:rounded-t-lg last:rounded-b-lg"
                    >
                      <div className="flex justify-between items-center">
                        <span>{preset.label}</span>
                        {preset.days > 0 && (
                          <span className="text-gray-500">{preset.days} days</span>
                        )}
                      </div>
                    </button>
                  ))}
