...
export interface RoomObject {
  id: string;
  type: 'fixture' | 'plant' | 'bench' | 'rack' | 'underCanopy' | 'window' | 'greenhouse' | 'emergencyFixture' | 'exitDoor' | 'egressPath' | 'obstacle' | 'wall' | 'hvacFan' | 'equipment' | 'rectangle' | 'circle' | 'line' | 'unistrut' | 'calculation_surface' | 'tank' | 'pipe' | 'filter' | 'pump';
  x: number;
  y: number;
  z: number;
  rotation: number;
...
export interface CogenerationComponent {
  id: string;
...
export interface DesignerState {
  ...
  objects: RoomObject[];
  cogenerationComponents?: CogenerationComponent[];
  ui: UIState;
...
export type DesignerAction =
  ...
  | { type: 'ADD_COGENERATION_COMPONENT'; payload: CogenerationComponent }
  | { type: 'UPDATE_COGENERATION_COMPONENT'; payload: { id: string; updates: Partial<CogenerationComponent> } }
  | { type: 'DELETE_COGENERATION_COMPONENT'; payload: string }
  | { type: 'UNDO' }
  | { type: 'REDO' }
  | { type: 'RESET' }
