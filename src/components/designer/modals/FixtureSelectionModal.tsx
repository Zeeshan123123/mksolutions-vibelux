...
export function FixtureSelectionModal({
  ...
  const loadFixtures = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/fixtures?limit=50'); // Load first 50 fixtures
      if (response.ok) {
        const data = await response.json();
        logger.info('system', 'API Response:', { data: data }); // Debug log
        ...
        if (Array.isArray(data)) {
          ...
          const transformedFixtures = data.fixtures.map((f: any) => {
            logger.info('system', 'Processing fixture:', { data: f }); // Debug log
            return {
              id: f.id || f.productId || `fixture_${Math.random().toString(36).substr(2, 9)}`,
              brand: f.brand || f.manufacturer || 'Unknown',
              model: f.model || f.modelNumber || f.productName || 'Unknown Model',
              wattage: f.wattage || f.reportedWattage || 400,
              ppf: f.ppf || f.reportedPPF || 1000,
              efficacy: f.efficacy || f.reportedPPE || 2.5,
              spectrum: f.spectrum || 'Full Spectrum',
              spectrumData: {
                blue: f.blueFlux ? (f.blueFlux / (f.reportedPPF || 1000)) * 100 : 25,
                green: f.greenFlux ? (f.greenFlux / (f.reportedPPF || 1000)) * 100 : 15,
                red: f.redFlux ? (f.redFlux / (f.reportedPPF || 1000)) * 100 : 45,
                farRed: f.farRedFlux ? (f.farRedFlux / (f.reportedPPF || 1000)) * 100 : 8
              },
              category: f.category || 'LED',
              coverage: 16, // Default 4x4 ft coverage
              price: f.price || Math.round((f.reportedWattage || 400) * 1.5), // Estimate $1.50 per watt
              beamAngle: f.beamAngle || 120,
              voltage: f.voltage || '120-277V',
              dimmable: f.dimmable !== false,
              warranty: f.warranty || 5,
              dlcData: {
                qualified: f.dlcQualified !== false,
                premium: f.dlcPremium || false
              }
            };
          });
          logger.info('system', 'Transformed fixtures:', { data: transformedFixtures.length }); // Debug log
          setFixtures(transformedFixtures);
        } else {
          logger.error('system', 'Invalid fixture data format:', data);
          setFixtures([]);
        }
      } else {
        logger.error('system', 'Failed to fetch fixtures:', response.status);
        setFixtures([]);
      }
    } catch (error) {
      logger.error('system', 'Failed to load fixtures:', error );
      setFixtures([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const filteredFixtures = useMemo(() => {
    ...
    if (!Array.isArray(fixtures)) {
      logger.error('system', 'Fixtures is not an array:', fixtures);
      return [];
    }
