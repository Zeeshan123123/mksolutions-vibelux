...
import { renderAllCogenerationComponents } from '../utils/cogenerationRenderer';
...
export function useCanvas2D(
  ...
  const redraw = useCallback(() => {
    const canvas = canvasRef.current;
    if (!canvas) {
      logger.warn('api', 'useCanvas2D: No canvas ref');
      return;
    }

    const ctx = canvas.getContext('2d');
    if (!ctx) {
      logger.warn('api', 'useCanvas2D: No canvas context');
      return;
    }
    
    // Skip if canvas has no size
    if (canvas.width === 0 || canvas.height === 0) {
      logger.warn('api', 'useCanvas2D: Canvas has no size', { data: { width: canvas.width, height: canvas.height } });
      return;
    }
    ...
    // Draw cogeneration components
    const currentCogenerationComponents = (state as any).cogenerationComponents;
    if (currentCogenerationComponents && currentCogenerationComponents.length > 0) {
      renderAllCogenerationComponents(ctx, currentCogenerationComponents, {
        scale,
        offsetX,
        offsetY,
        showClearances: true,
        ...
        showLabels: true,
        selectedComponentId: currentUI.selectedObjectId
      });
    }
