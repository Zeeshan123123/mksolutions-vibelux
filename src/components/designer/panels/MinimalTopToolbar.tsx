...
import { 
  ...
  Home, Tent, Droplets, Wind, DollarSign
} from 'lucide-react';
...
const modeConfigs = {
  ...
  simulate: {
    ...
    actions: [
      ...
      { icon: Sun, label: 'Solar Analysis', action: 'solar-analysis' },
  ...
  output: {
    ...
    actions: [
      ...
      { icon: Zap, label: 'Electrical Design', action: 'electrical-design' },
      { icon: Droplets, label: 'Irrigation Design', action: 'irrigation-design' },
      { icon: Wind, label: 'HVAC Design', action: 'hvac-design' },
      { icon: Building, label: 'Structural Design', action: 'structural-design' },
      { icon: Activity, label: 'Environmental Controls', action: 'environmental-controls' },
      { icon: Eye, label: '3D Visualization', action: '3d-visualization' },
      { icon: DollarSign, label: 'Cost Analysis', action: 'cost-analysis' },
...
export function MinimalTopToolbar({
  ...
  const handleSave = () => {
    try {
      if (!state) {
        logger.error('system', 'State is undefined');
      ...
      if (!state.room || !state.objects) {
        logger.error('system', 'State is missing required properties:', state);
  ...
  const handleModeAction = (action: string) => {
    ...
    switch (action) {
      ...
      case 'light-sim':
        ...
        break;
      case 'solar-analysis':
        window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'solarAnalysis' }));
        break;
      case 'growth':
      ...
      case 'batch-validation':
        window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'systemValidation' }));
        break;
      case 'alert-system':
      ...
      case 'electrical-design':
        window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'electricalDesignOutput' }));
        break;
      case 'irrigation-design':
        window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'irrigationDesign' }));
        break;
      case 'hvac-design':
        window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'hvacSystemDesign' }));
        break;
      case 'structural-design':
        window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'structuralDesign' }));
        break;
      case 'environmental-controls':
        window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'environmentalControls' }));
        break;
      case '3d-visualization':
        window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'advanced3DVisualization' }));
        break;
      case 'cost-analysis':
        window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'costAnalysis' }));
        break;
      case 'reports':
        window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'facilityDashboard' }));
        break;
      case 'commissioning':
      ...
      default:
        logger.warn('system', `Unknown action: ${action}`);
    ...
    <div className="h-14 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-b border-gray-200 dark:border-gray-800 flex items-center justify-between px-4 relative z-50">
      ...
      <div className="flex items-center gap-2 flex-1 justify-center max-w-2xl overflow-hidden">
        ...
        <button
          onClick={() => {
            window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'solarAnalysis' }));
            showNotification('info', 'Solar Analysis & Shadow Tracking opened');
          }}
          className="px-2 py-1 bg-amber-100 dark:bg-amber-900/30 hover:bg-amber-200 dark:hover:bg-amber-900/50 rounded-md transition-colors flex items-center gap-1.5"
          title="Solar Analysis & Shadow Tracking"
        >
          <Sun className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" />
          <span className="text-xs font-medium text-amber-700 dark:text-amber-300">Sun Path</span>
        </button>

        {/* Electrical Design Button */}
        <button
          onClick={() => {
            window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'electricalDesignOutput' }));
            showNotification('info', 'Electrical Design Output opened');
          }}
          className="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-200 dark:hover:bg-blue-900/50 rounded-md transition-colors flex items-center gap-1.5"
          title="Electrical Design Output"
        >
          <Zap className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
          <span className="text-xs font-medium text-blue-700 dark:text-blue-300">Electrical</span>
        </button>

        {/* Irrigation Design Button */}
        <button
          onClick={() => {
            window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'irrigationDesign' }));
            showNotification('info', 'Irrigation Design opened');
          }}
          className="px-2 py-1 bg-cyan-100 dark:bg-cyan-900/30 hover:bg-cyan-200 dark:hover:bg-cyan-900/50 rounded-md transition-colors flex items-center gap-1.5"
          title="Irrigation System Design"
        >
          <Droplets className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400" />
          <span className="text-xs font-medium text-cyan-700 dark:text-cyan-300">Irrigation</span>
        </button>

        {/* HVAC System Design Button */}
        <button
          onClick={() => {
            window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'hvacSystemDesign' }));
            showNotification('info', 'HVAC System Design opened');
          }}
          className="px-2 py-1 bg-orange-100 dark:bg-orange-900/30 hover:bg-orange-200 dark:hover:bg-orange-900/50 rounded-md transition-colors flex items-center gap-1.5"
          title="HVAC System Design"
        >
          <Wind className="w-3.5 h-3.5 text-orange-600 dark:text-orange-400" />
          <span className="text-xs font-medium text-orange-700 dark:text-orange-300">HVAC</span>
        </button>

        {/* Structural Design Button */}
        <button
          onClick={() => {
            window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'structuralDesign' }));
            showNotification('info', 'Structural Design opened');
          }}
          className="px-2 py-1 bg-purple-100 dark:bg-purple-900/30 hover:bg-purple-200 dark:hover:bg-purple-900/50 rounded-md transition-colors flex items-center gap-1.5"
          title="Structural Design"
        >
          <Building className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" />
          <span className="text-xs font-medium text-purple-700 dark:text-purple-300">Structural</span>
        </button>
        ...
        <button
          onClick={() => {
            window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'environmentalControls' }));
            showNotification('info', 'Environmental Controls opened');
          }}
          className="px-2 py-1 bg-teal-100 dark:bg-teal-900/30 hover:bg-teal-200 dark:hover:bg-teal-900/50 rounded-md transition-colors flex items-center gap-1.5"
          title="Environmental Controls"
        >
          <Activity className="w-3.5 h-3.5 text-teal-600 dark:text-teal-400" />
          <span className="text-xs font-medium text-teal-700 dark:text-teal-300">Environment</span>
        </button>
        ...
        <button
          onClick={() => {
            window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'advanced3DVisualization' }));
            showNotification('info', '3D Visualization opened');
          }}
          className="px-2 py-1 bg-pink-100 dark:bg-pink-900/30 hover:bg-pink-200 dark:hover:bg-pink-900/50 rounded-md transition-colors flex items-center gap-1.5"
          title="3D Facility Visualization"
        >
          <Eye className="w-3.5 h-3.5 text-pink-600 dark:text-pink-400" />
          <span className="text-xs font-medium text-pink-700 dark:text-pink-300">3D View</span>
        </button>

        {/* Cost Analysis & ROI Button */}
        <button
          onClick={() => {
            window.dispatchEvent(new CustomEvent('togglePanel', { detail: 'predictiveROI' }));
            showNotification('info', 'Cost Analysis & ROI opened');
          }}
          className="px-2 py-1 bg-emerald-100 dark:bg-emerald-900/30 hover:bg-emerald-200 dark:hover:bg-emerald-900/50 rounded-md transition-colors flex items-center gap-1.5"
          title="Cost Analysis & ROI Calculator"
        >
          <DollarSign className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
          <span className="text-xs font-medium text-emerald-700 dark:text-emerald-300">ROI</span>
        </button>
      </div>
