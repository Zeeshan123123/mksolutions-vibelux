...
export function HVACSystemPanel({ isOpen = true, onClose }: HVACSystemPanelProps) {
  ...
  const handleAddSystem = (system: HVACSystem) => {
    ...
    const newEquipment = {
      ...
      // Store electrical specifications for load calculations
      voltage: system.power.voltage,
      phase: system.power.phase,
      watts: system.power.watts || (system.power.mca * system.power.voltage * (system.power.phase === 3 ? Math.sqrt(3) : 1)),
      mca: system.power.mca, // Minimum circuit ampacity
      mop: system.power.mop, // Maximum overcurrent protection
      price: system.price,
