...
import { Droplets, Search, X, ChevronRight, Gauge, Zap, DollarSign, Info, Plus, AlertTriangle, Leaf, Calculator, Activity, Waves, Wrench } from 'lucide-react';
...
import { DripperAnalysisPanel } from './DripperAnalysisPanel';
import { PumpAnalysisPanel } from './PumpAnalysisPanel';
import { HydroponicAnalysisPanel } from './HydroponicAnalysisPanel';
import { ProfessionalIrrigationPanel } from './ProfessionalIrrigationPanel';
...
export function IrrigationSystemPanel({ isOpen = true, onClose }: IrrigationSystemPanelProps) {
  ...
  const [showDripperAnalysis, setShowDripperAnalysis] = useState(false);
  const [showPumpAnalysis, setShowPumpAnalysis] = useState(false);
  const [showHydroponicAnalysis, setShowHydroponicAnalysis] = useState(false);
  ...
  return (
    <div className="absolute top-0 right-0 w-96 h-full bg-gray-900 border-l border-gray-700 shadow-2xl flex flex-col z-50 overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-700">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div className="grid grid-cols-2 gap-1">
            <button
              onClick={() => setShowDripperAnalysis(true)}
              className="flex items-center justify-center gap-1 px-2 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-medium transition-colors"
            >
              <Calculator className="w-3 h-3" />
              Dripper
            </button>
            <button
              onClick={() => setShowPumpAnalysis(true)}
              className="flex items-center justify-center gap-1 px-2 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-medium transition-colors"
            >
          ...
          <div className="grid grid-cols-2 gap-1">
            <button
              onClick={() => setShowHydroponicAnalysis(true)}
              className="flex items-center justify-center gap-1 px-2 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-xs font-medium transition-colors"
            >
              <Waves className="w-3 h-3" />
              Hydro
            </button>
            <button
              onClick={() => setShowProfessionalAnalysis(true)}
              className="flex items-center justify-center gap-1 px-2 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-xs font-medium transition-colors"
            >
              ...
              >
                ...
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span className={`text-xs px-2 py-1 rounded ${
                    ...
                    system.category === 'Aeroponic' ? 'bg-pink-900/50 text-pink-400' :
                    system.category === 'Dosing' ? 'bg-yellow-900/50 text-yellow-400' :
                    system.category === 'Controller' ? 'bg-indigo-900/50 text-indigo-400' :
                    system.category === 'WaterTreatment' ? 'bg-teal-900/50 text-teal-400' :
                    system.category === 'Boom' ? 'bg-orange-900/50 text-orange-400' :
                    system.category === 'Spray' ? 'bg-sky-900/50 text-sky-400' :
                    system.category === 'Overhead' ? 'bg-slate-900/50 text-slate-400' :
                    system.category === 'Valves' ? 'bg-amber-900/50 text-amber-400' :
                    system.category === 'Filtration' ? 'bg-emerald-900/50 text-emerald-400' :
                    system.category === 'Monitoring' ? 'bg-violet-900/50 text-violet-400' :
                    system.category === 'Safety' ? 'bg-red-900/50 text-red-400' :
      ...
      {/* Dripper Analysis Panel */}
      <DripperAnalysisPanel
        isOpen={showDripperAnalysis}
        onClose={() => setShowDripperAnalysis(false)}
      />

      {/* Pump Analysis Panel */}
      <PumpAnalysisPanel
        isOpen={showPumpAnalysis}
        onClose={() => setShowPumpAnalysis(false)}
      />

      {/* Hydroponic Analysis Panel */}
      <HydroponicAnalysisPanel
        isOpen={showHydroponicAnalysis}
        onClose={() => setShowHydroponicAnalysis(false)}
      />

      {/* Professional Irrigation Engineering Panel */}
      <ProfessionalIrrigationPanel
        isOpen={showProfessionalAnalysis}
        onClose={() => setShowProfessionalAnalysis(false)}
      />
    </div>
  );
}