...
import { PlantPropertiesSection } from './PlantPropertiesSection';
...
export function PropertiesPanel({ isOpen, onClose }: PropertiesPanelProps) {
  ...
  const renderSpecificProperties = () => {
    switch (selectedObject.type) {
      ...
      case 'plant':
        return (
          <PlantPropertiesSection 
            selectedObject={selectedObject}
            onUpdate={(property, value) => updateObject(selectedObject.id, { [property]: value })}
          />
        );
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-sm"
                      step="0.1"
                      title={selectedObject.type === 'plant' ? 'Height is auto-set based on plant variety and growth stage' : ''}
                    />
                  </div>
                </div>
              </div>
