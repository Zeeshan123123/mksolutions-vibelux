...
export function WebGLRayTracingVisualization({ 
  ...
  useEffect(() => {
    const initializeRaytracer = async () => {
      try {
        ...
        logger.info('system', 'WebGL raytracer initialized successfully');
      } catch (error) {
        logger.error('system', 'Failed to initialize WebGL raytracer:', error );
  ...
  const runSimulation = async () => {
    ...
    try {
      ...
      logger.info('system', `Simulation completed in ${simulationTime.toFixed(2)}s with ${results.length} points`);
      
    } catch (error) {
      logger.error('system', 'Simulation failed:', error );
    } finally {
