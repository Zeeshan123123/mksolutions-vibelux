...
export function ResponsiveLayoutWrapper({ children }: { children: React.ReactNode }) {
  ...
  // Marketing/informational pages that should show the footer
  const footerPages = [
    '/pricing',
    '/about',
    '/how-it-works',
  ...
  const showFooter = footerPages.some(page => pathname.startsWith(page));
  ...
  if (isTablet) {
    return (
      <div className="relative min-h-screen">
        ...
        {/* Main Content */}
        <div className="min-h-screen pl-0 flex flex-col">
          {/* Toggle button */}
          <button
