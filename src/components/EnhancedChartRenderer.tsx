...
import { SankeyChart, SpiderChart, NetworkChart } from './advanced-charts/AdvancedVisualizationSuite';
import { GanttChart } from './project-management/GanttChart';

interface EnhancedChartProps {
  type: 'line' | 'bar' | 'pie' | 'area' | 'heatmap' | 'vector' | 'gauge' | 'scatter' | 'radar' | 'sankey' | 'spider' | 'network' | 'gantt';
  title: string;
  config: any;
}
...
export function EnhancedChartRenderer({ type, title, config }: EnhancedChartProps) {
  const renderChart = () => {
    ...
    if (chartTypesNeedingData.includes(type) && !Array.isArray(config.data)) {
      logger.error('system', 'Chart data must be an array for type:', { type, received: typeof config.data });
      return (
        <div className="flex items-center justify-center h-full text-gray-400">
    ...
    switch (type) {
      ...
      case 'sankey':
        return <SankeyChart data={config.data} config={config} />;
        
      case 'spider':
        return <SpiderChart data={config.data} config={config} />;
        
      case 'network':
        return <NetworkChart data={config.data} config={config} />;
