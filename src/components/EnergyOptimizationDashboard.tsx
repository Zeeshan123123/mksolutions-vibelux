...
export function EnergyOptimizationDashboard({ facilityId, location }: Props) {
  ...
  useEffect(() => {
    if (location?.lat && location?.lon) {
      const fetchWeather = async () => {
        try {
          const weather = await noaaClient.getWeather(location);
          const metrics = noaaClient.calculateAgriculturalMetrics(weather);
          setWeatherData(weather);
          setWeatherMetrics(metrics);
        } catch (error) {
          logger.error('system', 'Failed to fetch weather:', error );
        }
      };
      fetchWeather();
  ...
  // Weather-based energy optimization recommendations
  const getWeatherOptimizationStrategy = () => {
    if (!weatherData || !weatherMetrics) return null;

    const temp = weatherData.current.temperature;
    const humidity = weatherData.current.humidity;
    const vpd = weatherMetrics.vpd;
    ...
    if (temp > 85) {
      recommendations.push({
        type: 'cooling',
        message: `High outdoor temp (${temp}°F) - Optimize HVAC scheduling during off-peak hours`,
        severity: 'high',
        savings: '15-20%'
      });
    }

    // VPD-based dehumidification optimization
    if (vpd < 0.8 && humidity > 70) {
      recommendations.push({
        type: 'dehumidification',
        message: `Low VPD (${vpd.toFixed(2)} kPa) - Schedule dehumidification during low-rate periods`,
        severity: 'medium',
        savings: '8-12%'
      });
    }

    // Cold weather heating optimization
    if (temp < 50) {
      recommendations.push({
        type: 'heating',
        message: `Cold conditions (${temp}°F) - Pre-heat during off-peak hours, reduce during peak`,
        severity: 'medium',
        savings: '10-15%'
      });
    }
  ...
  const weatherOptimizations = getWeatherOptimizationStrategy();

  return (
    <div className="space-y-6">
      {/* Weather-Based Optimization Alert */}
      {weatherOptimizations && weatherOptimizations.length > 0 && (
        <Alert className="border-blue-500/50 bg-blue-500/10">
          <Cloud className="h-4 w-4 text-blue-500" />
          <AlertDescription>
            <div className="space-y-2">
              <p className="font-medium text-blue-400">Weather-Based Energy Optimization Available</p>
              {weatherOptimizations.map((opt, idx) => (
        ...
        <TabsContent value="analytics" className="space-y-4">
          ...
          {location && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <Card className="bg-gradient-to-br from-blue-500/10 to-blue-600/10 border-blue-500/20">
                <CardHeader>
                  <CardTitle>Weather Impact on Energy</CardTitle>
                  <CardDescription>Real-time optimization adjustments</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {weatherMetrics && (
                    <>
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-400">HVAC Load Adjustment</span>
                          <span className="text-sm font-medium text-yellow-400">
                            {weatherData.current.temperature > 80 ? '+25%' : weatherData.current.temperature < 60 ? '+15%' : 'Normal'}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-400">Dehumidification Need</span>
                          <span className="text-sm font-medium text-blue-400">
                            {weatherMetrics.vpd < 0.8 ? 'High' : weatherMetrics.vpd > 1.2 ? 'Low' : 'Moderate'}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-400">Natural Ventilation</span>
                          <span className="text-sm font-medium text-green-400">
                            {weatherData.current.temperature >= 65 && weatherData.current.temperature <= 75 && weatherData.current.humidity < 60 ? 'Available' : 'Not Recommended'}
                          </span>
                        </div>
                      </div>
                      <div className="pt-4 border-t border-gray-700">
                        <p className="text-xs text-gray-400 mb-2">Estimated Weather Impact</p>
                        <div className="flex items-baseline gap-2">
                          <span className="text-2xl font-bold text-yellow-400">
                            {weatherData.current.temperature > 85 || weatherData.current.temperature < 50 ? '+$45' : '+$12'}
                          </span>
                          <span className="text-sm text-gray-400">/day in energy costs</span>
                        </div>
                      </div>
                    </>
                  )}
