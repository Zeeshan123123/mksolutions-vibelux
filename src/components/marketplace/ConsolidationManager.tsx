...
export function ConsolidationManager({
  ...
  const fetchCenters = async () => {
    ...
    try {
      ...
      logger.error('system', 'Error fetching centers:', error );
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateConsolidation = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        logger.error('system', 'Failed to create consolidation order');
      }
    } catch (error) {
      logger.error('system', 'Error creating consolidation order:', error );
    } finally {
      setCreating(false);
    }
  };
