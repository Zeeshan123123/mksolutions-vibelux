...
interface ElectricalEstimatorProps {
  initialHVACEquipment?: Array<{
    id: string;
    manufacturer: string;
    model: string;
...
export function ElectricalEstimator({ initialHVACEquipment = [] }: ElectricalEstimatorProps) {
  const [inputs, setInputs] = useState<EstimatorInputs>({
    ...
    hvacEquipment: initialHVACEquipment,
    systemVoltage: 277,
    phases: 3,
    wireRunDistance: 100,
    laborRate: 85,
  ...
  const calculateEstimate = (): EstimateResults => {
    const fixtureLoad = inputs.fixtures
      .filter(f => f.enabled)
      .reduce((sum, f) => sum + (f.wattage * f.quantity), 0);

    const hvacLoad = inputs.hvacEquipment
      .filter(h => h.enabled)
      .reduce((sum, h) => {
        // Calculate power consumption from electrical specs
        const voltage = h.electricalSpecs.voltage;
        const amps = h.electricalSpecs.minCircuitAmpacity;
        const phases = h.electricalSpecs.phase;
        const power = phases === 3 
          ? voltage * amps * Math.sqrt(3) * 0.8 // 3-phase power with 0.8 power factor
          : voltage * amps; // Single phase
        return sum + (power * h.quantity);
      }, 0);

    const totalLoad = fixtureLoad + hvacLoad;

    const totalFixtures = inputs.fixtures
      .filter(f => f.enabled)
      .reduce((sum, f) => sum + f.quantity, 0);

    const totalHVACUnits = inputs.hvacEquipment
      .filter(h => h.enabled)
    ...
    // Material costs
    const materials = calculateMaterialCosts(circuits, totalFixtures + totalHVACUnits);
    
    // Labor costs
    const labor = calculateLaborCosts(circuits, totalFixtures + totalHVACUnits);
  ...
  const calculateCircuits = () => {
    ...
    // Calculate circuits for fixtures
    ...
    // Calculate circuits for HVAC equipment
    const enabledHVAC = inputs.hvacEquipment.filter(h => h.enabled);
    
    enabledHVAC.forEach(hvacUnit => {
      for (let i = 0; i < hvacUnit.quantity; i++) {
        const voltage = hvacUnit.electricalSpecs.voltage;
        const current = hvacUnit.electricalSpecs.minCircuitAmpacity;
        const breakerSize = hvacUnit.electricalSpecs.maxOvercurrentProtection;
        const wireSize = selectWireSize(current, inputs.wireRunDistance);
        const conduitSize = selectConduitSize(wireSize);

        circuits.push({
          id: `HVAC-${circuitIndex}`,
          breaker: `${breakerSize}A`,
          wire: wireSize,
          conduit: conduitSize,
          cost: 0 // Will be calculated in material costs
  ...
  const updateHVACUnit = (index: number, field: keyof Omit<typeof inputs.hvacEquipment[0], 'electricalSpecs'>, value: string | number | boolean) => {
    const updatedUnits = [...inputs.hvacEquipment];
    updatedUnits[index] = { ...updatedUnits[index], [field]: value };
    setInputs({ ...inputs, hvacEquipment: updatedUnits });
  };

  const updateHVACUnitElectrical = (index: number, field: keyof typeof inputs.hvacEquipment[0]['electricalSpecs'], value: number) => {
    const updatedUnits = [...inputs.hvacEquipment];
    updatedUnits[index] = {
      ...updatedUnits[index],
      electricalSpecs: {
        ...updatedUnits[index].electricalSpecs,
        [field]: value
      }
    };
    setInputs({ ...inputs, hvacEquipment: updatedUnits });
  ...
  const addHVACUnit = () => {
    ...
    setInputs({ ...inputs, hvacEquipment: [...inputs.hvacEquipment, newUnit] });
  };

  const removeHVACUnit = (index: number) => {
    const updatedUnits = inputs.hvacEquipment.filter((_, i) => i !== index);
    setInputs({ ...inputs, hvacEquipment: updatedUnits });
  };
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="bg-gray-900/60 backdrop-blur-xl border-gray-800">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-white">HVAC Equipment</CardTitle>
                  <button
                    onClick={addHVACUnit}
                    className="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg text-white font-medium transition-colors"
                  >
              ...
              <CardContent>
                <div className="space-y-4">
                  {inputs.hvacEquipment.map((unit, index) => (
                    <div key={unit.id} className="p-4 bg-gray-800/50 rounded-lg">
                      <div className="flex items-center gap-4 mb-3">
                        <input
                          type="checkbox"
                          checked={unit.enabled}
                          onChange={(e) => updateHVACUnit(index, 'enabled', e.target.checked)}
                          className="w-4 h-4"
                        />
                        <input
                          type="text"
                          value={unit.manufacturer}
                          onChange={(e) => updateHVACUnit(index, 'manufacturer', e.target.value)}
                          className="flex-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white"
                        ...
                        <input
                          type="text"
                          value={unit.model}
                          onChange={(e) => updateHVACUnit(index, 'model', e.target.value)}
                          className="flex-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white"
                          placeholder="Model"
                        />
                        <input
                          type="number"
                          value={unit.quantity}
                          onChange={(e) => updateHVACUnit(index, 'quantity', Number(e.target.value))}
                          className="w-20 px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white"
                          placeholder="Qty"
                        />
                        <button
                      ...
                      <div className="grid grid-cols-4 gap-3 text-sm">
                        <div>
                          <label className="text-xs text-gray-400">Cooling (tons)</label>
                          <input
                            type="number"
                            step="0.1"
                            value={unit.coolingCapacity}
                            onChange={(e) => updateHVACUnit(index, 'coolingCapacity', Number(e.target.value))}
                        ...
                        <div>
                          ...
                          <select
                            value={unit.electricalSpecs.voltage}
                            onChange={(e) => updateHVACUnitElectrical(index, 'voltage', Number(e.target.value))}
                            className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-white"
                          >
                            <option value={208}>208V</option>
                            <option value={240}>240V</option>
                        ...
                        <div>
                          <label className="text-xs text-gray-400">MCA (A)</label>
                          <input
                            type="number"
                            value={unit.electricalSpecs.minCircuitAmpacity}
                            onChange={(e) => updateHVACUnitElectrical(index, 'minCircuitAmpacity', Number(e.target.value))}
                            className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-white"
                          />
                        </div>
                        <div>
                          <label className="text-xs text-gray-400">MOCP (A)</label>
                          <input
                            type="number"
                            value={unit.electricalSpecs.maxOvercurrentProtection}
                            onChange={(e) => updateHVACUnitElectrical(index, 'maxOvercurrentProtection', Number(e.target.value))}
                            className="w-full px-2 py-1 bg-gray-700 border border-gray-600 rounded text-white"
                          />
                        </div>
                      </div>
