...
interface EnhancedCFDAnalysisPanelProps {
  initialHVACUnits?: HVACUnit[];
}

export function EnhancedCFDAnalysisPanel({ initialHVACUnits = [] }: EnhancedCFDAnalysisPanelProps) {
  ...
  // Function to generate boundary conditions from HVAC units
  const generateHVACBoundaryConditions = (hvacUnits: HVACUnit[]) => {
    const hvacBoundaries: BoundaryCondition[] = [];
    
    hvacUnits.forEach((unit, index) => {
      // Create supply air inlet
      const supplyInlet: BoundaryCondition = {
        id: `hvac-supply-${unit.id}`,
        name: `${unit.name} - Supply`,
        type: 'inlet',
        position: { 
          x: unit.position.x, 
          y: unit.position.y, 
        ...
        properties: {
          velocity: { 
            x: 0, 
            y: 0, 
            z: -Math.sqrt(unit.properties.airflow / 0.36) // Convert to velocity based on airflow
          },
          temperature: unit.properties.supplyTemperature,
          turbulenceIntensity: 5
        }
      };

      // Create return air outlet (offset from supply)
      const returnOutlet: BoundaryCondition = {
        id: `hvac-return-${unit.id}`,
        name: `${unit.name} - Return`,
        type: 'outlet',
        position: { 
          x: unit.position.x + 10, // Offset return location
          y: unit.position.y + 5, 
          z: 0.5 // Near floor
        },
        dimensions: { width: 1.0, height: 0.8, depth: 0.1 },
      ...
      hvacBoundaries.push(supplyInlet, returnOutlet);
    });

    return hvacBoundaries;
  };

  // Update boundary conditions when HVAC units change
  useEffect(() => {
    if (initialHVACUnits.length > 0) {
      const hvacBoundaries = generateHVACBoundaryConditions(initialHVACUnits);
      
      // Keep non-HVAC boundaries and add HVAC boundaries
      const nonHVACBoundaries = boundaries.filter(b => 
        !b.id.startsWith('hvac-supply-') && !b.id.startsWith('hvac-return-')
      );
      
      setBoundaries([...nonHVACBoundaries, ...hvacBoundaries]);
    }
  }, [initialHVACUnits]);
