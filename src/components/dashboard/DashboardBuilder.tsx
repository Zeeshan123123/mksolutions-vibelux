...
export function DashboardBuilder({ 
  ...
  const handleSaveDashboard = () => {
    ...
    try {
      ...
      logger.error('system', 'Failed to parse saved dashboards:', error );
      savedDashboards = [];
    }
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        logger.error('system', 'Failed to save dashboard to localStorage:', error );
      }
    }
  };
  ...
  const handleImportDashboard = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        logger.error('system', 'Failed to import dashboard:', error );
      }
    };
