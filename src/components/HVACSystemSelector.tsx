...
interface HVACSystemSelectorProps {
  ...
  onCustomUnitSelect?: (customUnit: CustomHVACUnit) => void;
}

export interface CustomHVACUnit {
  manufacturer: string;
  model: string;
  coolingCapacity: number; // tons
  coolingEfficiency: number; // EER or COP
  coolingEfficiencyUnit: 'EER' | 'SEER' | 'COP' | 'kW/ton';
  heatingCapacity: number; // kW or BTU/hr
  heatingEfficiency: number; // % or COP
  heatingEfficiencyUnit: '%' | 'COP' | 'HSPF';
  electricalSpecs: {
    voltage: number;
    phase: 1 | 3;
    minCircuitAmpacity: number;
    maxOvercurrentProtection: number;
    fanPowerConsumption: number; // Watts
  };
  airflow: {
    nominal: number; // CFM
    externalStaticPressure: number; // inches W.C.
  };
  dehumidification?: {
    capacity: number; // pints/day or L/day
    unit: 'pints/day' | 'L/day';
  };
  physicalDimensions: {
    length: number;
    width: number;
...
export function HVACSystemSelector({ 
  ...
  const [customUnit, setCustomUnit] = useState<CustomHVACUnit>({
    manufacturer: '',
    model: '',
    coolingCapacity: 10,
    coolingEfficiency: 14,
    coolingEfficiencyUnit: 'EER',
    heatingCapacity: 30,
    heatingEfficiency: 95,
    heatingEfficiencyUnit: '%',
    electricalSpecs: {
      voltage: 460,
      phase: 3,
      minCircuitAmpacity: 50,
      maxOvercurrentProtection: 60,
      fanPowerConsumption: 2000
    },
    airflow: {
      nominal: 4000,
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-sm font-medium text-gray-400">Filter by System Type</h4>
          <button
          ...
          >
            ...
            {showCustomUnit ? 'Hide Custom Unit' : 'Add Custom Unit'}
          </button>
        </div>
      ...
      {/* Custom Unit Form */}
      {showCustomUnit && (
        <div className="bg-gray-800 rounded-lg p-6 border-2 border-green-500/30">
          <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
            <Settings className="w-5 h-5 text-green-400" />
            Custom HVAC Unit Configuration
          </h3>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="text"
                    value={customUnit.manufacturer}
                    onChange={(e) => setCustomUnit({...customUnit, manufacturer: e.target.value})}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                    placeholder="e.g., 50TC-D08"
                  />
                </div>
              </div>
              
              <h4 className="text-sm font-medium text-gray-300 pt-2">Cooling Specifications</h4>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="text-xs text-gray-400">Capacity (tons)</label>
                  <input
                    type="number"
                    step="0.1"
                    value={customUnit.coolingCapacity}
                    onChange={(e) => setCustomUnit({...customUnit, coolingCapacity: Number(e.target.value)})}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  />
                </div>
                <div>
                  <label className="text-xs text-gray-400">Efficiency</label>
                  <input
                    type="number"
                    step="0.1"
                    value={customUnit.coolingEfficiency}
                    onChange={(e) => setCustomUnit({...customUnit, coolingEfficiency: Number(e.target.value)})}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  />
                </div>
                <div>
                  ...
                  <select
                    value={customUnit.coolingEfficiencyUnit}
                    onChange={(e) => setCustomUnit({...customUnit, coolingEfficiencyUnit: e.target.value as any})}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  >
              ...
              <h4 className="text-sm font-medium text-gray-300 pt-2">Heating Specifications</h4>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="text-xs text-gray-400">Capacity (kW)</label>
                  <input
                    type="number"
                    step="0.1"
                    value={customUnit.heatingCapacity}
                    onChange={(e) => setCustomUnit({...customUnit, heatingCapacity: Number(e.target.value)})}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    step="0.1"
                    value={customUnit.heatingEfficiency}
                    onChange={(e) => setCustomUnit({...customUnit, heatingEfficiency: Number(e.target.value)})}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  />
                </div>
                <div>
                  <label className="text-xs text-gray-400">Unit</label>
                  <select
                    value={customUnit.heatingEfficiencyUnit}
                    onChange={(e) => setCustomUnit({...customUnit, heatingEfficiencyUnit: e.target.value as any})}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  >
                    <option value="%">%</option>
                    <option value="COP">COP</option>
            ...
            {/* Electrical & Airflow */}
            <div className="space-y-4">
              <h4 className="text-sm font-medium text-gray-300">Electrical Specifications</h4>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-400">Voltage</label>
                  <input
                    type="number"
                    value={customUnit.electricalSpecs.voltage}
                    onChange={(e) => setCustomUnit({
                      ...customUnit, 
                      electricalSpecs: {...customUnit.electricalSpecs, voltage: Number(e.target.value)}
                    })}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  />
                </div>
                <div>
                  ...
                  <select
                    value={customUnit.electricalSpecs.phase}
                    onChange={(e) => setCustomUnit({
                      ...customUnit, 
                      electricalSpecs: {...customUnit.electricalSpecs, phase: Number(e.target.value) as any}
                    })}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  >
                ...
                <div>
                  <label className="text-xs text-gray-400">Min Circuit Amps</label>
                  <input
                    type="number"
                    value={customUnit.electricalSpecs.minCircuitAmpacity}
                    onChange={(e) => setCustomUnit({
                      ...customUnit, 
                      electricalSpecs: {...customUnit.electricalSpecs, minCircuitAmpacity: Number(e.target.value)}
                    })}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  />
                </div>
                <div>
                  <label className="text-xs text-gray-400">Max Breaker (A)</label>
                  <input
                    type="number"
                    value={customUnit.electricalSpecs.maxOvercurrentProtection}
                    onChange={(e) => setCustomUnit({
                      ...customUnit, 
                      electricalSpecs: {...customUnit.electricalSpecs, maxOvercurrentProtection: Number(e.target.value)}
                    })}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  />
                </div>
              ...
              <h4 className="text-sm font-medium text-gray-300 pt-2">Airflow</h4>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-400">Nominal CFM</label>
                  <input
                    type="number"
                    value={customUnit.airflow.nominal}
                    onChange={(e) => setCustomUnit({
                      ...customUnit, 
                      airflow: {...customUnit.airflow, nominal: Number(e.target.value)}
                    })}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  />
                </div>
                <div>
                  <label className="text-xs text-gray-400">ESP (in. W.C.)</label>
                  <input
                    type="number"
                    step="0.1"
                    value={customUnit.airflow.externalStaticPressure}
                    onChange={(e) => setCustomUnit({
                      ...customUnit, 
                      airflow: {...customUnit.airflow, externalStaticPressure: Number(e.target.value)}
                    })}
                    className="w-full mt-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm"
                  />
                </div>
              ...
              <div className="pt-4">
                <button
                  onClick={() => {
                    // Call the custom unit callback
                    onCustomUnitSelect(customUnit);
                    
                    // Also convert to standard format for display
                    const customSystem: HVACSystemType = {
                      id: 'custom-' + Date.now(),
                      name: `${customUnit.manufacturer} ${customUnit.model}`,
                      category: 'packaged',
                      description: 'Custom configured HVAC unit',
                      features: customUnit.features,
                      efficiency: {
                        cooling: { 
                          min: customUnit.coolingEfficiency, 
                          max: customUnit.coolingEfficiency, 
                          unit: customUnit.coolingEfficiencyUnit 
                        },
                        heating: { 
                          min: customUnit.heatingEfficiency, 
                          max: customUnit.heatingEfficiency, 
                          unit: customUnit.heatingEfficiencyUnit 
                        }
                      },
                      capacityRange: { 
                        min: customUnit.coolingCapacity, 
                        max: customUnit.coolingCapacity, 
                        unit: 'tons' 
                      },
                      costRange: { min: 0, max: 0 },
                      pros: ['Custom specifications', 'Exact match for requirements'],
                      cons: ['Manual configuration required'],
                      bestFor: ['Specific project requirements'],
                      components: ['Custom configuration']
                    };
                    handleSystemSelect(customSystem);
                    setShowCustomUnit(false);
                  }}
                  disabled={!customUnit.manufacturer || !customUnit.model}
                  className="w-full py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-all"
                >
                  Use This Custom Unit
                </button>
              </div>
            </div>
          </div>
