...
export function LICORMonitor() {
  ...
  const [selectedRadio, setSelectedSensor] = useState<string>('');
  const [readings, setReadings] = useState<RadioReading[]>([]);
  ...
  const fetchSensorStatus = async () => {
    try {
      ...
      logger.error('system', 'Failed to fetch sensor status:', error );
      setIsLoading(false);
    }
  };
  ...
  const fetchReadings = async (sensorId: string) => {
    try {
      ...
      logger.error('system', 'Failed to fetch readings:', error );
    }
  };
  ...
  const fetchDLI = async (sensorId: string) => {
    try {
      ...
      logger.error('system', 'Failed to fetch DLI:', error );
    }
  };
  ...
  const simulateReading = async () => {
    try {
      ...
      logger.error('system', 'Failed to simulate reading:', error );
    }
  };
