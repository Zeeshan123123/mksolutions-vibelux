...
interface PIDControlPanelProps {
  ...
  deepRedPercentage?: number;
}

export function PIDControlPanel({ 
  allowHighIntensity = false, 
  deepRedPercentage = 60 
}: PIDControlPanelProps) {
    ...
    const interval = setInterval(() => {
      setZones(prevZones => 
        prevZones.map(zone => {
          ...
          // Update controller
          let output = controller.update(simulatedPPFD)
          
          // Apply deep red safety dimming if high intensity not allowed
          if (!allowHighIntensity && deepRedPercentage > 80) {
            // Limit intensity based on deep red percentage to prevent white tip
            const maxAllowedIntensity = 100 - (deepRedPercentage - 80) * 0.75 // Reduce by 0.75% per 1% over 80%
            output = Math.min(output, maxAllowedIntensity)
          }
