...
interface HeatLoadInputs {
  ...
  roomsOperatingSimultaneously: number;
...
export function EnhancedHeatLoadCalculator() {
  const [inputs, setInputs] = useState<HeatLoadInputs>({
    ...
    height: 12,
    numberOfRooms: 1,
    roomsOperatingSimultaneously: 1,
    ...
    <div className="bg-gray-800 rounded-lg p-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Multi-Room Configuration */}
          <div className="bg-gray-700 rounded-lg p-4 border-2 border-orange-500/30">
            <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-sm text-gray-400">Total Number of Rooms</label>
                <input
                  type="number"
                  min="1"
                  value={inputs.numberOfRooms}
                  onChange={(e) => setInputs({...inputs, numberOfRooms: Number(e.target.value)})}
                  className="w-full mt-1 px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white"
                />
              </div>
              <div>
                <label className="text-sm text-gray-400">Rooms Operating Simultaneously</label>
                <input
                  type="number"
                  min="1"
                  max={inputs.numberOfRooms}
                  value={inputs.roomsOperatingSimultaneously}
                  onChange={(e) => setInputs({...inputs, roomsOperatingSimultaneously: Math.min(Number(e.target.value), inputs.numberOfRooms)})}
                  className="w-full mt-1 px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white"
                />
              </div>
            </div>
            {inputs.numberOfRooms > 1 && (
              <div className="mt-4 p-3 bg-orange-500/10 rounded-lg border border-orange-500/30">
                <p className="text-sm text-orange-400">
                  <strong>Facility Summary:</strong> {inputs.numberOfRooms} total rooms with {inputs.roomsOperatingSimultaneously} operating at peak load
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  Peak load calculations will be based on {inputs.roomsOperatingSimultaneously} room{inputs.roomsOperatingSimultaneously > 1 ? 's' : ''} running simultaneously
                </p>
              </div>
            )}
          </div>
              ...
              <div className="bg-gray-700 rounded-lg p-4">
                <div className="text-center">
                  ...
                  <p className="text-gray-400 mb-2">
                    {inputs.numberOfRooms > 1 ? 'Per Room Cooling Required' : 'Total Cooling Required'}
                  </p>
                  ...
                  {/* Multi-room facility totals */}
                  {inputs.numberOfRooms > 1 && (
                    <div className="mt-4 pt-4 border-t border-gray-600">
                      <div className="bg-orange-500/10 rounded-lg p-4 border border-orange-500/30">
                        <h4 className="text-lg font-semibold text-orange-400 mb-3">Facility Totals</h4>
                        <div className="space-y-2">
                          <div className="flex justify-between">
                            <span className="text-gray-300">Peak Load ({inputs.roomsOperatingSimultaneously} rooms)</span>
                            <span className="text-xl font-bold text-white">
                              {(results.coolingTonsRequired * inputs.roomsOperatingSimultaneously).toFixed(1)} tons
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-300">Total Capacity (all {inputs.numberOfRooms} rooms)</span>
                            <span className="text-xl font-bold text-white">
                              {(results.coolingTonsRequired * inputs.numberOfRooms).toFixed(1)} tons
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-300">Recommended Central System</span>
                            <span className="text-xl font-bold text-orange-400">
                              {Math.ceil(results.recommendedUnitSize * inputs.roomsOperatingSimultaneously / 5) * 5} tons
                            </span>
                          </div>
                        </div>
                      </div>
