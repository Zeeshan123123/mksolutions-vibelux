...
export function EquipmentDashboard() {
  ...
  const handleAddEquipment = async (newEquipment: Omit<Equipment, 'id' | 'usage' | 'maintenance'> & { initialHours?: number }) => {
    try {
      ...
      logger.error('system', 'Error adding equipment:', error );
                        ...
                        <button 
                          onClick={async () => {
                            try {
                              ...
                              logger.error('system', 'Error stopping tracking:', error );
                            }
                          }}
                        ...
                        <button 
                          onClick={async () => {
                            try {
                              ...
                              logger.error('system', 'Error starting tracking:', error );
                            }
                          }}
                      ...
                      <button 
                        onClick={() => {
                          ...
                          const newWorkOrder = {
                            ...
                            description: `Scheduled maintenance for ${eq.name}`,
                            equipment: eq.name,
                            location: eq.location || 'Equipment Location',
                    ...
                    <button 
                      onClick={() => {
                        ...
                        setSelectedEquipment({ 
                          ...
                          manufacturer: 'Work Order System',
                          model: wo.title,
                          serialNumber: wo.id,
                          installDate: wo.dateCreated,
                          lastMaintenance: wo.dateCreated
                        });
                        setShowDetailsModal(true);
                      }}
                      className="px-3 py-1 bg-gray-800 hover:bg-gray-700 rounded text-sm transition-all"
              ...
              {spareParts.map(part => (
                <div key={part.id} className="bg-gray-900 rounded-lg p-6">
                  ...
                  <div className="flex gap-2">
                    <button 
                      onClick={() => {
                        // Open inventory part details
                        setSelectedEquipment({ 
                          ...part, 
                          type: 'Spare Part',
                          status: part.currentStock <= part.minimumStock ? 'Low Stock' : 'In Stock',
                    ...
                    <button 
                      onClick={() => {
                        // Order more parts functionality
                        const orderQuantity = part.reorderQuantity || 10;
                        const newOrder = {
                          id: Date.now().toString(),
                          title: `Reorder: ${part.name}`,
                          description: `Order ${orderQuantity} units of ${part.name} (P/N: ${part.partNumber})`,
                          equipment: part.name,
                          location: part.location,
                          priority: part.currentStock <= part.minimumStock ? 'high' as const : 'medium' as const,
                          status: 'open' as const,
                          assignee: 'Procurement Team',
                          dateCreated: new Date().toISOString().split('T')[0],
                          dueDate: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 3 days from now
                          checklist: [
                            `Order ${orderQuantity} units of ${part.name}`,
                        ...
                        alert(`Order created for ${orderQuantity} units of ${part.name}`);
                      }}
                      className="px-3 py-1 bg-purple-600 hover:bg-purple-700 rounded text-sm transition-all"
                    >
                      Order More
