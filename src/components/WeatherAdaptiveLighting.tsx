...
// Stub imports
const ControlsAnalyzer = {
  analyze: () => ({ analysis: {}, recommendations: [] })
}
type ControlAnalysisResult = any
  ...
  const fetchEnergyData = async () => {
    ...
    try {
      ...
      if ('error' in incentives) {
        logger.warn('system', 'Incentive data error:', { data: incentives.error  })
      } else {
        setIncentiveData(incentives)
      }

      if ('error' in rates) {
        logger.warn('system', 'Utility rates error:', { data: rates.error  })
      } else {
        setUtilityRates(rates)
      }
    } catch (err) {
      logger.warn('system', 'Energy data fetch error:', { data: err })
    }
  }
