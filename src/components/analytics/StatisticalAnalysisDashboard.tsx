...
export function StatisticalAnalysisDashboard({ 
  ...
  const [realDataStatus, setRealDataStatus] = useState<'loading' | 'loaded' | 'error' | 'no-data'>('loading')
  
  // Fetch real data from database
  const fetchRealData = async (): Promise<DataPoint[]> => {
    setIsLoadingRealData(true)
    try {
      // Fetch from cultivation cycles and lab results
      const where = batchId ? { batchId } : {}
      
      // Fetch harvest data with lab results
      ...
      // Fetch correlation analysis results
      const correlationAnalyses = await prisma.correlationAnalysis.findMany({
        where,
        orderBy: { timestamp: 'desc' },
        take: 100
      })
      
      // Fetch data points from pipeline
      ...
      harvests.forEach(harvest => {
        const envConditions = harvest.environmentalConditions as any
        const labResults = harvest.labTestResults?.[0]
        
        if (envConditions) {
          realData.push({
            timestamp: harvest.harvestDate,
            variables: {
              dli: envConditions.avgDLI || 0,
              ppfd: envConditions.avgPPFD || 0,
              vpd: envConditions.avgVPD || calculateVPD(envConditions.avgTemperature, envConditions.avgHumidity),
              temperature: envConditions.avgTemperature || 0,
              humidity: envConditions.avgHumidity || 0,
              co2: envConditions.avgCO2 || 0,
              photoperiod: 12, // Default flowering photoperiod
              yield: harvest.dryWeight / 10, // g/m²
              thc: labResults?.cannabinoids?.thc || 0,
              cbd: labResults?.cannabinoids?.cbd || 0,
              terpenes: labResults?.terpenes?.totalTerpenes || 0,
              energy_usage: 0 // Will be calculated from sensor data
            }
          })
        }
      })
      
      // Process pipeline data points
      dataPoints.forEach(dp => {
        const point = dp as any
        if (point.environmental) {
          realData.push({
            timestamp: point.timestamp,
            variables: {
              dli: point.environmental.dli || 0,
              ppfd: point.environmental.ppfd || 0,
              vpd: point.environmental.vpd || calculateVPD(point.environmental.temperature, point.environmental.humidity),
              temperature: point.environmental.temperature || 0,
              humidity: point.environmental.humidity || 0,
              co2: point.environmental.co2 || 0,
              photoperiod: 12,
              yield: 0, // Will be filled from harvest data
              thc: 0,
              cbd: 0,
              terpenes: 0,
              energy_usage: 0
      ...
      if (realData.length === 0) {
        setRealDataStatus('no-data')
        toast({
          title: 'No Real Data Available',
          description: 'Using simulated data for demonstration. Start collecting real cultivation data to see actual correlations.',
          variant: 'warning'
        })
        return generateSampleData() // Fall back to sample data
      }
      
      setRealDataStatus('loaded')
      toast({
        title: 'Real Data Loaded',
        description: `Analyzing ${realData.length} real data points from your cultivation cycles.`,
      })
      ...
      toast({
        ...
        description: 'Failed to load real cultivation data. Using simulated data instead.',
        variant: 'destructive'
      })
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (useRealData && !data) {
        sampleData = await fetchRealData()
      } else {
    ...
    loadData()
  }, [data, batchId, useRealData, minCorrelation, showOnlySignificant])
              ...
              <div className="flex items-center gap-2 mt-2">
                {isLoadingRealData ? (
                  ...
                  <div className="flex items-center gap-2 text-sm text-green-400">
                    ...
                    Using real cultivation data
                  </div>
                ) : realDataStatus === 'no-data' ? (
                  <div className="flex items-center gap-2 text-sm text-yellow-400">
                    <AlertCircle className="h-4 w-4" />
                    No real data available - using simulation
                  </div>
                ) : realDataStatus === 'error' ? (
                  <div className="flex items-center gap-2 text-sm text-red-400">
                    <AlertCircle className="h-4 w-4" />
                    Error loading data - using simulation
                  </div>
                ) : null}
              </div>
            </div>
