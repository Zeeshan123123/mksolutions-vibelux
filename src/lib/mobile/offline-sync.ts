...
class SyncQueue {
  ...
  private async persist(): Promise<void> {
    try {
      ...
      logger.error('api', 'Failed to persist sync queue:', error );
    }
  }
  ...
  async load(): Promise<void> {
    try {
      ...
      logger.error('api', 'Failed to load sync queue:', error );
      this.queue = [];
    }
  }
}
...
export async function fetchAndCache(type: string): Promise<any[]> {
  try {
    ...
    logger.error('api', `Failed to fetch and cache ${type}:`, error);
    return [];
  }
}
...
export async function forceSync(item: PendingSync): Promise<void> {
  ...
  try {
    ...
    logger.error('api', 'Force sync failed:', error );
    throw error;
  }
}
...
export async function handleSyncError(item: PendingSync, error: any): Promise<void> {
  ...
  logger.error('api', `Sync error for ${item.type}:${item.data.id}:`, error);
