...
class WebRTCSignalingServer {
  ...
  private setupSocketHandlers() {
    this.io.on('connection', async (socket: Socket) => {
      logger.info('api', 'Client connected:', { data: socket.id });
      ...
      socket.on('join-consultation', async (data: { consultationId: string }) => {
        try {
          ...
          logger.error('api', 'Error joining consultation:', error );
  ...
  private async authenticateSocket(socket: Socket): Promise<string | null> {
    try {
      ...
      logger.error('api', 'Socket authentication error:', error );
      return null;
    }
  }
  ...
  private async handleEndSession(socket: Socket, userId: string, data: { consultationId: string }) {
    ...
    if (consultation) {
      ...
      if (originalAmount > actualAmount) {
        ...
        logger.info('api', `Processing refund of $${refundAmount} for consultation ${data.consultationId}`);
      }
    }
  ...
  private handleDisconnect(socket: Socket, userId: string) {
    logger.info('api', 'Client disconnected:', { data: socket.id });
