...
export class SAPConnector {
  ...
  async disconnect(): Promise<void> {
    ...
    try {
      ...
      logger.error('api', 'Failed to logout from SAP:', error );
    } finally {
  ...
  async setupWebhook(webhookUrl: string): Promise<void> {
    ...
    logger.warn('api', 'SAP B1 webhook setup not implemented. Consider using polling instead.');
  }
  ...
  async handleError(error: Error, context: string): Promise<void> {
    logger.error('api', `SAP Connector Error in ${context}:`, error);
