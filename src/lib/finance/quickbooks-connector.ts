...
export class QuickBooksConnector {
  ...
  private validateCustomerId(customerId: string): void {
    ...
    // Check length (QuickBooks IDs are typically 1-21 characters)
    if (customerId.length < 1 || customerId.length > 50) {
      throw new Error('Customer ID length invalid');
    }
    
    // Allow only alphanumeric characters, hyphens, and underscores
    const validIdRegex = /^[a-zA-Z0-9_-]+$/;
    if (!validIdRegex.test(customerId)) {
  ...
  async disconnect(): Promise<void> {
    ...
    if (this.refreshToken) {
      try {
        ...
        logger.error('api', 'Failed to revoke tokens:', error );
      }
    }
