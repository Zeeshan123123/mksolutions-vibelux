...
export class VideoConferenceManager {
  ...
  static async handleParticipantJoin(
    ...
    logger.info('api', `Participant ${userId} (${role}) joined room ${roomId}`);
  }
  ...
  static async handleParticipantLeave(
    ...
    logger.info('api', `Participant ${userId} left room ${roomId}`);
  }
  ...
  private static async processRefund(
    ...
    logger.info('api', `Processing refund of $${refundAmount} for payment ${paymentIntentId}`);
  ...
  static async forceEndSession(
    ...
    if (session) {
      ...
      logger.info('api', `Force ended session ${roomId}: ${reason}`);
    }
  }
}
