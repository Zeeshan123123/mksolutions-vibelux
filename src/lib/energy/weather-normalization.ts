...
export class WeatherNormalizationService {
  ...
  constructor(
    private noaaApiKey: string = (process.env.NOAA_API_TOKEN || process.env.NOAA_API_KEY || '')
  ) {}
  ...
  async normalizeEnergyUsage(
    ...
    // Facility weather binding (provider/station overrides)
    const binding = await prisma.facilityWeather.findUnique({ where: { facilityId } });

    // Get weather data for current period
    const currentWeather = await this.getWeatherData(
      binding?.latitude ?? facility.latitude,
      binding?.longitude ?? facility.longitude,
      startDate,
      endDate,
      binding?.stationId
    );
  ...
  private async getWeatherData(
    ...
    const station = stationOverride ? { id: stationOverride } : await this.findNearestStation(latitude, longitude);
    ...
    // Try to enrich with daily average humidity via Open‑Meteo
    try {
      const omUrl = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&daily=relative_humidity_2m_mean&start_date=${format(startDate,'yyyy-MM-dd')}&end_date=${format(endDate,'yyyy-MM-dd')}&timezone=UTC`;
      const omRes = await axios.get(omUrl);
      const times: string[] = omRes.data?.daily?.time || [];
      const rh: number[] = omRes.data?.daily?.relative_humidity_2m_mean || [];
      const rhMap = new Map(times.map((t, i) => [t, rh[i]]));
      weatherData.forEach(d => {
        const key = format(d.date, 'yyyy-MM-dd');
        const val = rhMap.get(key);
        if (typeof val === 'number') d.humidity = val;
      });
    } catch {}
