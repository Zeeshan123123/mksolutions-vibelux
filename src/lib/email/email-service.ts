...
// Note: Use dynamic import for handlebars to avoid bundler warnings in serverless builds
...
export class EmailService {
  ...
  private async renderTemplate(template: string, data: Record<string, any>): Promise<string> {
    const hb = await import('handlebars');
  ...
  private async trackEmail(data: {
    ...
    try {
      ...
      logger.error('api', 'Failed to track email:', error );
    }
  }
  ...
  async trackOpen(messageId: string, recipient: string): Promise<void> {
    try {
      ...
      logger.error('api', 'Failed to track email open:', error );
    }
  }
  ...
  async trackClick(messageId: string, url: string): Promise<void> {
    try {
      ...
      logger.error('api', 'Failed to track email click:', error );
    }
  }
