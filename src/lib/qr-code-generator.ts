...
export interface QRCodeData {
  id: string;
  type: 'container' | 'inventory' | 'asset' | 'location' | 'cultivation-task';
  facilityId: string;
  metadata: {
    name: string;
    description?: string;
...
export class QRCodeGenerator {
  ...
  /**
   * Generate a QR code for cultivation tasks
   */
  async generateCultivationTaskQR(
    taskId: string,
    facilityId: string,
    metadata: Partial<QRCodeData['metadata']> & {
      plantId?: string;
    ...
    const data: QRCodeData = {
      ...
      facilityId,
      metadata: {
        name: metadata.name || `Task ${taskId}`,
        ...metadata
      },
      tracking: {
        createdAt: new Date(),
    ...
    for (let i = 0; i < count; i++) {
      ...
      switch (type) {
        ...
        case 'location':
          ...
          break;
        case 'cultivation-task':
          result = await this.generateCultivationTaskQR(id, facilityId, metadata, options);
          break;
      }
...
// Cultivation-specific QR code functions
export interface CultivationQRData {
  taskId: string;
  plantId: string;
  locationId: string;
  technique: string;
...
export async function generateCultivationQRCodes(
  tasks: Array<{
    taskId: string;
    plantId: string;
    locationId: string;
  ...
  options?: QRCodeOptions
): Promise<Array<{ qrCode: string; data: QRCodeData; cultivationData: CultivationQRData }>> {
  const results = [];

  for (const task of tasks) {
    const cultivationData: CultivationQRData = {
      taskId: task.taskId,
    ...
    const qrResult = await generator.generateCultivationTaskQR(
      task.taskId,
      task.facilityId,
      {
        name: `${task.technique} - Plant ${task.plantId}`,
...
export function generateCultivationTaskBatch(
  technique: string,
  plantIds: string[],
  locationId: string,
  facilityId: string
