...
function calculatePPFDAtPoint(
  ...
  // PPFD calculation for grow lights using empirical formula
  // This formula is calibrated to match real-world measurements
  
  // Convert distance from feet to meters
  const distanceInMeters = distance * 0.3048;
  
  // For grow lights, the effective coverage area is typically smaller than theoretical
  // because of reflector design and light concentration
  // A 600W LED typically covers about 4x4 ft (1.5 m²) at 3 ft height
  ...
  // PPFD calculation with real-world calibration
  const ppfd = (ppf * opticalEfficiency / Math.max(effectiveArea, 0.1)) * angleFactor;
...
export function calculateRecommendedFixtures(
  ...
  const totalPPFNeeded = targetPPFD * roomArea * 0.092903; // Convert ft² to m² to get μmol/s
