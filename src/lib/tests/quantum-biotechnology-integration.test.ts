...
describe('Quantum-Biotechnology Integration', () => {
  ...
  test('Integration: Quantum-optimized spectrum affects gene prediction', async () => {
    ...
    logger.info('api', '- Quantum advantage:', { data: quantumResult.quantumAdvantage });
    logger.info('api', '- Optimal red ratio:', { data: quantumResult.optimalValues.red_ratio });
    logger.info('api', '- Gene predictions count:', { data: predictions.length });
    logger.info('api', '- First gene fold change:', { data: predictions[0]?.foldChange });
  });
});
