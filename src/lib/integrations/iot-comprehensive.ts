...
export class ComprehensiveIoTManager extends EventEmitter {
  ...
  async discoverDevices(networkType: 'wifi' | 'zigbee' | 'modbus' = 'wifi'): Promise<Array<{
    ...
    try {
      ...
      logger.error('api', `❌ Device discovery failed for ${networkType}:`, error);
      return [];
    }
  }
  ...
  async bulkRegisterSensors(discoveredDevices: Array<{
    ...
    for (const discovered of discoveredDevices) {
      try {
        ...
        logger.error('api', `Failed to register ${discovered.name}:`, error);
        failed++;
      }
    }
  ...
  async registerDevice(device: Omit<SensorDevice, 'status' | 'lastReading'>): Promise<boolean> {
    try {
      ...
      if (!validation.valid) {
        logger.error('api', 'Device validation failed:', validation.errors );
        return false;
      }
      ...
      logger.error('api', 'Failed to register IoT device:', error );
      return false;
    }
  }
  ...
  private startPolling(deviceId: string): void {
    ...
    const pollingInterval = setInterval(async () => {
      try {
        ...
        logger.error('api', `Polling error for ${device.name}:`, error);
  ...
  private async storeReading(reading: SensorReading): Promise<void> {
    ...
    try {
      ...
      logger.error('api', 'Failed to store sensor reading:', error );
    }
  }
