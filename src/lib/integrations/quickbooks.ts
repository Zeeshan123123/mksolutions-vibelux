...
export class QuickBooksIntegration {
  ...
  private async refreshAccessToken() {
    ...
    if (process.env.NODE_ENV !== 'test') {
      try {
        ...
        logger.error('api', 'Failed to save updated QuickBooks tokens:', error )
      }
    }
  }
  ...
  async getInvoices(startDate?: Date, endDate?: Date) {
    ...
    if (startDate && endDate) {
      // Validate dates are actually Date objects and not strings that could contain SQL
      if (!(startDate instanceof Date) || !(endDate instanceof Date)) {
        throw new Error('Invalid date parameters');
      }
      // Safely format dates using ISO string and validate format
      const startDateStr = startDate.toISOString().split('T')[0];
      const endDateStr = endDate.toISOString().split('T')[0];
      
      // Additional validation - ensure dates are in expected YYYY-MM-DD format
      const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
      if (!dateRegex.test(startDateStr) || !dateRegex.test(endDateStr)) {
        throw new Error('Invalid date format');
      }
      
      query += ` WHERE TxnDate >= '${startDateStr}' AND TxnDate <= '${endDateStr}'`
    }
  ...
  async syncHarvestBatch(batch: {
    ...
    try {
      ...
      logger.error('api', 'Failed to sync harvest batch to QuickBooks:', error )
