...
export class ClimateComputerIntegrationManager {
  ...
  async addIntegration(config: ClimateComputerConfig): Promise<boolean> {
    try {
      ...
      logger.error('api', `Failed to add integration ${config.id}:`, error);
      return false;
    }
  }
  ...
  async removeIntegration(integrationId: string): Promise<boolean> {
    try {
      ...
      logger.error('api', `Failed to remove integration ${integrationId}:`, error);
      return false;
    }
  }
    ...
    try {
      if (typeof window !== 'undefined') {
        ...
        try {
          ...
          logger.error('api', 'Server-side discovery failed:', error );
        }
      }
    } catch (error) {
      logger.error('api', 'Auto-discovery failed:', error );
    }
  ...
  async syncIntegration(integrationId: string): Promise<SyncResult> {
    ...
    try {
      ...
      logger.error('api', `Sync failed for integration ${integrationId}:`, error);
    }
    ...
    for (const [integrationId, adapter] of this.integrations.entries()) {
      ...
      if (config && config.enabled && (!facilityId || config.facilityId === facilityId)) {
        try {
          ...
          logger.error('api', `Failed to write setpoint via ${integrationId}:`, error);
        }
      }
    }
    ...
    for (const [integrationId, adapter] of this.integrations.entries()) {
      ...
      if (config && config.enabled && (!facilityId || config.facilityId === facilityId)) {
        try {
          ...
          logger.error('api', `Failed to get equipment from ${integrationId}:`, error);
        }
      }
    }
  ...
  private startPeriodicSync(intervalMs: number = 300000): void { // 5 minutes
    this.syncInterval = setInterval(() => {
      this.syncAll().catch(error => {
        logger.error('api', 'Periodic sync failed:', error );
      });
    }, intervalMs);
  }
