...
export class HortimaxAdapter {
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      logger.error('api', 'Failed to connect to Hortimax system:', error );
      return false;
    }
  }
  ...
  async writeSetpoint(pointId: string, value: number): Promise<boolean> {
    ...
    try {
      ...
      logger.error('api', `Failed to write setpoint ${pointId}:`, error);
      return false;
    }
  }
  ...
  private async connectToSynopta(): Promise<boolean> {
    try {
      ...
      logger.error('api', 'Synopta connection failed:', error );
      return false;
    }
  }
  ...
  private async connectToManager(): Promise<boolean> {
    try {
      ...
      logger.error('api', 'Manager connection failed:', error );
      return false;
    }
  }
  ...
  private async connectToModbus(): Promise<boolean> {
    try {
      ...
      // Dynamic import for server-side only - use eval to prevent webpack bundling
      ...
      logger.error('api', 'Hortimax Modbus connection failed:', error );
      return false;
    }
  }
  ...
  private async readDataPoint(pointId: string): Promise<HortimaxDataPoint | null> {
    ...
    try {
      ...
      logger.error('api', `Failed to read data point ${pointId}:`, error);
      return null;
    }
  }
...
export async function discoverHortimaxSystems(networkRange: string = '192.168.1'): Promise<HortimaxConfig[]> {
  ...
  if (typeof window !== 'undefined') {
    logger.warn('api', 'Modbus discovery not available in browser environment');
    return discovered;
  }
  ...
  const modbusSerial = await import('modbus-serial').catch(() => null);
  if (!modbusSerial) {
    logger.warn('api', 'modbus-serial module not available');
    return discovered;
  }
