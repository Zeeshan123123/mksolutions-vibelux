...
export class OnsetHOBOIntegration extends IoTSensorIntegration {
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      logger.error('api', 'HOBO connection error:', error );
      return false;
    }
  }
  ...
  async discoverDevices(): Promise<SensorDevice[]> {
    try {
      ...
      logger.error('api', 'HOBO device discovery error:', error );
      return [];
    }
  }
...
export class TrolmasterIntegration extends IoTSensorIntegration {
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      logger.error('api', 'Trolmaster connection error:', error );
      return false;
    }
  }
  ...
  async discoverDevices(): Promise<SensorDevice[]> {
    try {
      ...
      logger.error('api', 'Trolmaster device discovery error:', error );
      return [];
    }
  }
...
export class ModbusSensorIntegration extends IoTSensorIntegration {
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      logger.error('api', 'Modbus connection error:', error );
      return false;
    }
  }
  ...
  async getBulkReadings(deviceIds: string[]): Promise<Map<string, SensorReading>> {
    ...
    for (const deviceId of deviceIds) {
      try {
        ...
        logger.error('api', `Failed to read ${deviceId}:`, error);
      }
    }
