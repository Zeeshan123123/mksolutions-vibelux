...
export class ModbusProtocol {
  ...
  async connect(): Promise<void> {
    ...
    logger.info('api', `Connecting to Modbus device at ${this.config.host}:${this.config.port}`);
  ...
  async writeCoil(address: number, value: boolean): Promise<void> {
    ...
    logger.info('api', `Writing coil ${address}: ${value}`);
  }
  ...
  async writeRegister(address: number, value: number): Promise<void> {
    ...
    logger.info('api', `Writing register ${address}: ${value}`);
  }
  ...
  async writeMultipleRegisters(address: number, values: number[]): Promise<void> {
    ...
    logger.info('api', `Writing ${values.length} registers starting at ${address}`);
  }
...
export class BACnetProtocol {
  ...
  async discover(): Promise<BACnetDevice[]> {
    ...
    logger.info('api', 'Discovering BACnet devices...');
  ...
  async writeProperty(
    ...
    logger.info('api', `Writing BACnet property: Device ${deviceId}`, { data: { objectType, objectInstance, propertyId, value } });
  }
...
export class OPCUAProtocol {
  ...
  async connect(): Promise<void> {
    logger.info('api', `Connecting to OPC UA server at ${this.config.endpoint}`);
  ...
  async write(nodeId: string, value: any): Promise<void> {
    ...
    node.value = value;
    logger.info('api', `Wrote OPC UA node ${nodeId}: ${value}`);
  }
...
export class MQTTProtocol {
  ...
  async connect(): Promise<void> {
    logger.info('api', `Connecting to MQTT broker at ${this.config.broker}`);
  ...
  async subscribe(topic: string, callback: (message: any) => void): Promise<void> {
    ...
    logger.info('api', `Subscribed to MQTT topic: ${topic}`);
  ...
  async publish(topic: string, message: any): Promise<void> {
    ...
    logger.info('api', `Publishing to MQTT topic ${topic}:`, { data: message });
  ...
  async unsubscribe(topic: string): Promise<void> {
    ...
    logger.info('api', `Unsubscribed from MQTT topic: ${topic}`);
  }
