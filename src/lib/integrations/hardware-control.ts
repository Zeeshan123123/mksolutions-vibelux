...
export class HardwareController extends EventEmitter {
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      logger.error('api', '❌ Hardware connection failed:', error );
  ...
  async controlLighting(command: {
    ...
    try {
      ...
      logger.error('api', 'Lighting control error:', error );
  ...
  async controlEnvironment(command: {
    ...
    try {
      ...
      logger.error('api', 'Environmental control error:', error );
  ...
  async controlIrrigation(command: {
    ...
    try {
      ...
      logger.error('api', 'Irrigation control error:', error );
  ...
  private async testModbusConnections(): Promise<void> {
    ...
    for (const [deviceId, device] of this.modbusDevices) {
      try {
        ...
        logger.error('api', `❌ Modbus device ${device.name} failed:`, error);
        this.deviceStatus.set(deviceId, {
          deviceId,
          online: false,
          lastSeen: new Date(),
  ...
  private async testBACnetConnections(): Promise<void> {
    ...
    for (const [deviceId, device] of this.bacnetDevices) {
      try {
        ...
        logger.error('api', `❌ BACnet device ${device.name} failed:`, error);
        this.deviceStatus.set(deviceId, {
          deviceId,
          online: false,
          lastSeen: new Date(),
  ...
  private async pollDevice(deviceId: string): Promise<void> {
    ...
    try {
      ...
      logger.error('api', `Polling error for device ${deviceId}:`, error);
