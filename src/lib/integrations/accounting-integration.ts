...
export class AccountingIntegrationService {
  ...
  /**
   * Safely validate and format date for SQL queries
   */
  private validateAndFormatDate(date: Date): string {
  ...
  private async fetchQuickBooksRevenue(startDate: Date, endDate: Date): Promise<RevenueData[]> {
    ...
    try {
      // Validate and safely format dates
      const startDateStr = this.validateAndFormatDate(startDate);
      const endDateStr = this.validateAndFormatDate(endDate);
      
      // Fetch invoices
      const invoicesResponse = await fetch(
        `${baseUrl}/company/${this.config.companyId}/query?query=SELECT * FROM Invoice WHERE TxnDate >= '${startDateStr}' AND TxnDate <= '${endDateStr}'`,
        {
          headers: {
      ...
      const salesResponse = await fetch(
        `${baseUrl}/company/${this.config.companyId}/query?query=SELECT * FROM SalesReceipt WHERE TxnDate >= '${startDateStr}' AND TxnDate <= '${endDateStr}'`,
        {
          headers: {
            'Authorization': `Bearer ${this.config.accessToken}`,
            'Accept': 'application/json',
      ...
      logger.error('api', 'QuickBooks API error:', error );
  ...
  private async fetchXeroRevenue(startDate: Date, endDate: Date): Promise<RevenueData[]> {
    ...
    try {
      ...
      logger.error('api', 'Xero API error:', error );
  ...
  private async fetchFreshBooksRevenue(startDate: Date, endDate: Date): Promise<RevenueData[]> {
    ...
    try {
      ...
      logger.error('api', 'FreshBooks API error:', error );
  ...
  private async fetchQuickBooksExpenses(startDate: Date, endDate: Date): Promise<ExpenseData[]> {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/company/${this.config.companyId}/query?query=SELECT * FROM Purchase WHERE TxnDate >= '${startDateStr}' AND TxnDate <= '${endDateStr}'`,
        {
          headers: {
            'Authorization': `Bearer ${this.config.accessToken}`,
            'Accept': 'application/json',
      ...
      logger.error('api', 'QuickBooks expense error:', error );
      return [];
    }
  }
  ...
  private async fetchXeroExpenses(startDate: Date, endDate: Date): Promise<ExpenseData[]> {
    ...
    try {
      ...
      logger.error('api', 'Xero expense error:', error );
      return [];
    }
  }
