...
export class CO2EnrichmentController {
  ...
  private async setInjectionRate(rate: number): Promise<void> {
    ...
    logger.info('api', `CO2 injection rate set to: ${rate.toFixed(2)} L/min`);
  }
  ...
  public startController(): void {
    this.isActive = true;
    logger.info('api', `CO2 Enrichment Controller started for ${this.config.facilityId}/${this.config.zoneId}`);
  }

  public stopController(): void {
    this.isActive = false;
    this.currentInjectionRate = 0;
    logger.info('api', `CO2 Enrichment Controller stopped for ${this.config.facilityId}/${this.config.zoneId}`);
  }
}