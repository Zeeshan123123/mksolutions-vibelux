...
// Export design data
export function exportDesign(options: {
  room: any,
  fixtures: any[],
  metrics?: any,
  heatmap?: any,
  spectrum?: any,
  format?: 'json' | 'pdf' | 'dwg' | 'excel' | 'html'
}): any {
  const { room, fixtures, metrics, heatmap, spectrum, format = 'json' } = options;
  
  const design = {
    room,
    fixtures: fixtures.map(f => ({
      id: f.id,
      position: f.position || { x: f.x, y: f.y, z: f.z },
      rotation: f.rotation,
      model: f.model,
      specifications: f.model?.specifications || {
        ppf: f.ppf,
        wattage: f.wattage,
        coverage: f.coverage
      },
  ...
  // Return the design data - actual format conversion happens in the UI layer
  return design;
}