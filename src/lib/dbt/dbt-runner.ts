...
export class DbtRunner {
  ...
  async parse(): Promise<LineageGraph> {
    try {
      ...
      logger.error('api', 'Failed to parse dbt project:', error );
      throw error;
    }
  }
  ...
  async generateDocs(): Promise<void> {
    try {
      ...
      logger.error('api', 'Failed to generate dbt docs:', error );
      throw error;
    }
  }
  ...
  async serveDocs(port: number = 8080): Promise<ChildProcess> {
    try {
      ...
      logger.error('api', 'Failed to serve dbt docs:', error );
      throw error;
    }
  }
  ...
  async clean(): Promise<void> {
    try {
      ...
      logger.error('api', 'Failed to clean dbt artifacts:', error );
      throw error;
    }
  }
  ...
  async getModelLineage(modelName: string): Promise<{
    ...
    try {
      ...
      logger.error('api', `Failed to get lineage for model ${modelName}:`, error);
  ...
  async listResources(resourceType?: 'models' | 'tests' | 'sources' | 'snapshots'): Promise<string[]> {
    try {
      ...
      logger.error('api', 'Failed to list dbt resources:', error );
      return [];
    }
  }
