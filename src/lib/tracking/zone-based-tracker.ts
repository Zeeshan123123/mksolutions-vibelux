...
export class ZoneBasedTracker {
  ...
  public async connectAppleWatch(userId: string): Promise<boolean> {
    ...
    try {
      ...
      logger.info('api', 'Requesting Apple HealthKit permissions:', { data: permissions });
      
      return true;
    } catch (error) {
      logger.error('api', 'Failed to connect Apple Watch:', error );
      return false;
    }
  }
  ...
  private async collectBiometricData(
    ...
    try {
      ...
      logger.error('api', 'Failed to collect biometric data:', error );
      return null;
    }
  }
  ...
  private sendHealthAlerts(userId: string, alerts: string[]): void {
    ...
    logger.info('api', `Health alerts for user ${userId}:`, { data: alerts });
  }
