...
class InfluxDBClient {
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      if (!url || !token || !org || !bucket) {
        logger.warn('api', 'InfluxDB configuration incomplete, using PostgreSQL for time-series data')
        return false
      }
      ...
      logger.error('api', '❌ InfluxDB connection failed:', error )
  ...
  private async testConnection(): Promise<void> {
    ...
    try {
      ...
      logger.info('api', '✅ InfluxDB connection established')
    } catch (error) {
  ...
  async writeSensorData(readings: SensorReading[]): Promise<boolean> {
    if (!this.isConnected || !this.writeApi) {
      logger.warn('api', 'InfluxDB not connected, skipping write')
      return false
    }
    ...
    try {
      ...
      logger.error('api', 'Error writing to InfluxDB:', error )
      return false
    }
  }
  ...
  async querySensorData(
    ...
    if (!this.isConnected || !this.queryApi) {
      logger.warn('api', 'InfluxDB not connected, returning empty data')
      return []
    }
    ...
    try {
      ...
      logger.error('api', 'Error querying InfluxDB:', error )
      return []
    }
  }
