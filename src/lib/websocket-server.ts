...
class VibeLuxWebSocketServer {
  ...
  async start(port: number = 3002): Promise<boolean> {
    try {
      ...
      logger.error('api', '❌ Failed to start WebSocket server:', error )
      return false
    }
  }
  ...
  private async handleConnection(ws: WebSocket, request: any) {
    ...
    if (token) {
      try {
        ...
        logger.warn('api', `Authentication failed for client ${clientId}:`, { data: error })
      }
    }
  ...
  private async handleMessage(clientId: string, data: any) {
    ...
    try {
      ...
      switch (message.type) {
        ...
        default:
          logger.warn('api', `Unknown message type from ${clientId}: ${message.type}`)
      }
    } catch (error) {
      logger.error('api', `Error handling message from ${clientId}:`, error)
      this.sendToClient(clientId, {
        type: 'error',
  ...
  private async handleControl(clientId: string, controlData: any) {
    ...
    try {
      ...
      logger.error('api', `Control error for client ${clientId}:`, error)
      this.sendToClient(clientId, {
        type: 'error',
  ...
  private handleError(clientId: string, error: Error) {
    logger.error('api', `WebSocket error for client ${clientId}:`, error)
  }
  ...
  private async streamSensorData() {
    try {
      ...
      logger.error('api', 'Error streaming sensor data:', error )
    }
  }
  ...
  private async streamLightingStatus() {
    try {
      ...
      logger.error('api', 'Error streaming lighting data:', error )
    }
  }
  ...
  private sendToClient(clientId: string, message: any) {
    ...
    if (client && client.ws.readyState === WebSocket.OPEN) {
      try {
        ...
        logger.error('api', `Error sending to client ${clientId}:`, error)
      }
    }
  }
  ...
  private async sendRecentData(clientId: string, channel: string) {
    try {
      ...
      logger.error('api', `Error sending recent data for ${channel}:`, error)
    }
  }
...
if (process.env.NODE_ENV === 'development') {
  wsServer.start().catch((error) => logger.error('api', 'Async operation failed', error))
}
