...
export class GridPricingAPI {
  ...
  async initializeRealTimeConnection(): Promise<void> {
    try {
      ...
      this.websocket.onerror = (error) => {
        logger.error('api', 'Grid pricing WebSocket error:', error )
      }
    } catch (error) {
      logger.error('api', 'Failed to initialize grid pricing connection:', error )
      throw error
    }
  }
  ...
  private handleRealTimePriceUpdate(data: any): void {
    try {
      ...
      logger.error('api', 'Error processing real-time price update:', error )
    }
  }
  ...
  async getCurrentPricing(): Promise<GridPricingData> {
    try {
      ...
      logger.error('api', 'Failed to fetch current pricing:', error )
      throw error
    }
  }
  ...
  async getDayAheadPricing(): Promise<EnergyMarketData> {
    try {
      ...
      logger.error('api', 'Failed to fetch day-ahead pricing:', error )
      throw error
    }
  }
  ...
  async getUtilitySignals(): Promise<UtilitySignal[]> {
    try {
      ...
      logger.error('api', 'Failed to fetch utility signals:', error )
      throw error
    }
  }
  ...
  async submitDemandResponseBid(
    ...
    try {
      ...
      logger.error('api', 'Failed to submit demand response bid:', error )
      return false
    }
  }
