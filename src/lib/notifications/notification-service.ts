...
export class NotificationService extends EventEmitter {
  ...
  private async startProcessing(): Promise<void> {
    setInterval(async () => {
      ...
      try {
        ...
        logger.error('api', 'Error processing alerts:', error );
      } finally {
  ...
  private async sendPushNotification(user: any, alert: Alert): Promise<void> {
    ...
    const notification = {
      title: alert.title,
      body: alert.message,
      icon: '/icons/vibelux-notification-icon.png',
      badge: '/icons/vibelux-badge.png',
      tag: `vibelux-${alert.type}-${alert.projectId}`,
      ...
      data: {
        ...
        roomId: alert.roomId,
        url: `/facility/${alert.projectId}${alert.roomId ? `/room/${alert.roomId}` : ''}`
      }
    };
    ...
    try {
      ...
      if (typeof window !== 'undefined' && 'serviceWorker' in navigator && 'Notification' in window) {
        ...
        if (permission === 'granted') {
          ...
          logger.info('api', 'Push notification sent:', {
            userId: user.id,
            alertId: alert.id,
            title: notification.title
          });
        } else {
          logger.warn('api', 'Push notification permission denied for user:', user.id);
        }
      }
      
      // FCM for mobile apps (if configured)
      if (this.config.push.provider === 'fcm' && this.config.push.config?.serverKey) {
        await this.sendFCMNotification(user.pushToken, notification);
      }
      
    } catch (error) {
      logger.error('api', 'Push notification error:', error);
    }
  }

  // Send FCM notification for mobile apps
  private async sendFCMNotification(token: string, notification: any): Promise<void> {
    try {
      const response = await fetch('https://fcm.googleapis.com/fcm/send', {
        method: 'POST',
        headers: {
          'Authorization': `key=${this.config.push?.config?.serverKey}`,
        ...
        body: JSON.stringify({
          ...
          notification: {
            ...
            body: notification.body,
            icon: notification.icon,
            click_action: `${process.env.NEXT_PUBLIC_APP_URL}${notification.data.url}`
          },
          data: notification.data,
          android: {
            priority: 'high',
            notification: {
              channel_id: 'vibelux_alerts',
              sound: 'default',
              vibrate_timings: ['1s', '2s', '1s']
            }
          },
          apns: {
            headers: {
      ...
      logger.info('api', 'FCM notification sent:', result);
    } catch (error) {
      logger.error('api', 'FCM notification error:', error);
    }
  }
