...
export class GeneExpressionPredictor {
  ...
  public async predictGeneExpression(
    ...
    logger.info('api', `Predicting gene expression for ${cropSpecies}...`);
  ...
  public async optimizeForMetabolites(
    ...
    logger.info('api', `Optimizing lighting for metabolites: ${targetMetabolites.join(', ')}`);
  ...
  public async optimizeMicrobiome(
    ...
    logger.info('api', `Optimizing microbiome for functions: ${targetFunctions.join(', ')}`);
  ...
  private calculateDLI(spectrum: SpectrumProfile, intensity: number, photoperiod: number): number { return intensity * photoperiod * 0.0036; }
