...
export class BackupManager {
    ...
    try {
      ...
      for (const table of tablesToBackup) {
        try {
          ...
          metadata.rowCounts[table] = 0 // Simplified for now
        } catch (error) {
          logger.warn('system', `Could not get row count for table ${table}:`, error)
  ...
  async restoreBackup(backupId: string, options: {
    ...
    try {
      ...
      logger.error('system', `Restore failed: ${backupId}`, error)
  ...
  async deleteBackup(backupId: string): Promise<void> {
    ...
    try {
      ...
      logger.error('system', `Failed to delete backup: ${backupId}`, error)
      throw error
    }
  }
  ...
  async cleanupOldBackups(): Promise<void> {
    ...
    for (const backup of oldBackups) {
      try {
        ...
        logger.error('system', `Failed to cleanup backup ${backup.id}:`, error)
      }
    }
  }
  ...
  private async getTableList(options: {
    ...
    // Hardcoded for now - in production would query database
    const allTables = ['User', 'Project', 'Fixture', 'ProjectFixture', 'LightRecipe', 'Report', 'Subscription']
  ...
  private async cleanupTempFiles(files: string[]): Promise<void> {
    for (const file of files) {
      try {
        ...
        logger.warn('system', `Failed to cleanup temp file ${file}:`, error)
      }
    }
  }
...
export class BackupScheduler {
  ...
  private async checkAndRunScheduledBackups(): Promise<void> {
    try {
      ...
      logger.error('system', 'Scheduled backup failed:', error)
    }
  }
