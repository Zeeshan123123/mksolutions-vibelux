...
class EnvironmentValidator {
  ...
  validate(): EnvVars {
    ...
    try {
      ...
      logger.error('api', '❌ Environment validation failed:', error )
      ...
      if (isProduction) {
        ...
        logger.warn('api', '⚠️ Development mode: continuing with invalid environment')
  ...
  private performSecurityChecks(env: EnvVars): void {
    ...
    // Log all warnings
    warnings.forEach(warning => logger.warn('api', warning))
