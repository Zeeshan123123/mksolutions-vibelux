...
export async function storeSensorDataInPostgres(readings: TimeSeriesReading[]): Promise<boolean> {
  try {
    ...
    logger.error('api', 'Error storing sensor data in PostgreSQL:', error )
    return false
  }
}
...
export async function querySensorDataFromPostgres(
  ...
  try {
    ...
    logger.error('api', 'Error querying sensor data from PostgreSQL:', error )
    return []
  }
}
...
export async function storeHybridSensorData(readings: TimeSeriesReading[]): Promise<boolean> {
  ...
  try {
    ...
    logger.warn('api', 'InfluxDB not available, falling back to PostgreSQL');
  }
...
export async function getRecentEnvironmentalData(
  ...
  try {
    ...
    logger.warn('api', 'InfluxDB not available', { data: 'using PostgreSQL' })
  }
...
export async function checkDatabaseHealth(): Promise<{
  ...
  try {
    ...
    logger.error('api', 'PostgreSQL health check failed:', error )
  }
  ...
  try {
    ...
    logger.error('api', 'InfluxDB health check failed:', error )
  }
