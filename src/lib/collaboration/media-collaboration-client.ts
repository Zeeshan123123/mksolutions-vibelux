...
export class MediaCollaborationClient extends EventEmitter {
  ...
  async initializeDevices() {
    try {
      ...
      logger.error('api', 'Failed to enumerate devices:', error )
    }
  }
  ...
  async startCall(participants: string[] = []): Promise<string> {
    ...
    try {
      ...
      logger.error('api', 'Failed to start call:', error )
      this.callState = null
      throw error
    }
  }
  ...
  async joinCall(callId: string): Promise<void> {
    ...
    try {
      ...
      logger.error('api', 'Failed to join call:', error )
      this.callState = null
      throw error
    }
  }
  ...
  async startScreenShare(): Promise<void> {
    ...
    try {
      ...
      logger.error('api', 'Failed to start screen sharing:', error )
      throw error
    }
  }
  ...
  async stopScreenShare(): Promise<void> {
    ...
    try {
      ...
      logger.error('api', 'Failed to stop screen sharing:', error )
    }
  }
  ...
  private async initializeLocalStream(): Promise<void> {
    try {
      ...
      logger.error('api', 'Failed to get local stream:', error )
      throw error
    }
  }
  ...
  private createPeerConnection(userId: string, user: User, initiator: boolean): CallParticipant {
    ...
    peer.on('error', (error) => {
      logger.error('api', `Peer error for user ${userId}:`, error)
