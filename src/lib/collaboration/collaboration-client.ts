...
export class CollaborationClient extends EventEmitter {
  ...
  connect() {
    ...
    try {
      ...
      logger.error('api', 'Failed to connect to collaboration server:', error )
  ...
  private handleMessage(event: MessageEvent) {
    try {
      ...
      switch (data.type) {
        ...
        default:
          logger.warn('api', 'Unknown message type:', { data: data.type })
      }
    } catch (error) {
  ...
  private handleServerError(data: any) {
    logger.error('api', 'Server error:', data.error )
...
export class OperationalTransform {
  ...
  static apply(doc: string, op: any): string {
    switch (op.type) {
      ...
      default:
        logger.warn('api', 'Unknown operation type:', { data: op.type })
        return doc
    }
  }
}