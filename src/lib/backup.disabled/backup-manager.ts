...
export class BackupManager {
    ...
    try {
      ...
      for (const table of tablesToBackup) {
        try {
          ...
          logger.warn('api', `Could not get row count for table ${table}:`, { data: error })
      ...
      logger.error('api', `Backup failed: ${backupId}`, error)
  ...
  async restoreBackup(backupId: string, options: {
    ...
    try {
      ...
      logger.error('api', `Restore failed: ${backupId}`, error)
  ...
  async deleteBackup(backupId: string): Promise<void> {
    ...
    try {
      ...
      logger.error('api', `Failed to delete backup: ${backupId}`, error)
      throw error
    }
  }
  ...
  async cleanupOldBackups(): Promise<void> {
    ...
    for (const backup of oldBackups) {
      try {
        ...
        logger.error('api', `Failed to cleanup backup ${backup.id}:`, error)
      }
    }
  ...
  private async cleanupTempFiles(files: string[]): Promise<void> {
    for (const file of files) {
      try {
        ...
        logger.warn('api', `Failed to cleanup temp file ${file}:`, { data: error })
      }
    }
  }
...
export class BackupScheduler {
  ...
  private async checkAndRunScheduledBackups(): Promise<void> {
    try {
      ...
      logger.error('api', 'Scheduled backup failed:', error )
    }
  }
