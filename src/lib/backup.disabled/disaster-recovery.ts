...
export class DisasterRecoveryManager {
  ...
  private async encryptBackup(backupPath: string): Promise<string> {
    ...
    if (!password) {
      throw new Error('BACKUP_ENCRYPTION_KEY environment variable is required for backup encryption');
    }
  ...
  private async verifyBackupIntegrity(recoveryPoint: RecoveryPoint): Promise<boolean> {
    try {
      ...
      logger.error('api', 'Backup integrity check failed:', error );
      return false;
    }
  }
  ...
  private async getLocalBackups(): Promise<RecoveryPoint[]> {
    ...
    try {
      ...
      logger.error('api', 'Error getting local backups:', error );
    }
  ...
  private async getRemoteBackups(): Promise<RecoveryPoint[]> {
    ...
    try {
      ...
      logger.error('api', 'Error getting S3 backups:', error );
    }
  ...
  private async deleteBackup(point: RecoveryPoint): Promise<void> {
    try {
      ...
      logger.error('api', 'Error deleting backup:', error );
    }
  }
