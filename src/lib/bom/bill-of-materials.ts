...
// Export the generator function with multiple signatures for compatibility
export async function generateBillOfMaterials(
  input: any
): Promise<ProfessionalBOM> {
  // Handle different input formats
  if (input.fixtures && input.roomDimensions) {
    // New format
    const fixtures = input.fixtures as Fixture[];
    const roomDimensions = input.roomDimensions;
    const room = { 
    ...
    return BillOfMaterialsGenerator.generate(room, fixtures, projectInfo);
  } else if (input.room && input.fixtures) {
    // Expected format from AI modules
    const projectInfo = {
      name: input.designType === 'professional' ? 'Professional Grow Facility' : 'Grow Facility',
      number: `PRJ-${Date.now()}`,
      client: 'VibeLux Client',
      location: 'TBD',
      preparedBy: 'VibeLux Autonomous BIM Engine'
    };
    
    return BillOfMaterialsGenerator.generate(input.room, input.fixtures, projectInfo);
  } else {
    // Fallback for other formats
    const room = input.room || { width: 20, length: 40, height: 4 };
