...
import { createClaudeClient, selectModel } from './claude-config'
...
export class AIRouter {
  ...
  async routeRequest(
    ...
    // No token limits - use maximum available context window
    ...
    return claudeQueue.add(async () => {
      ...
      try {
        ...
        logger.error('api', 'AI Router error:', error )
        throw error
      }
    })
  }
  ...
  private selectOptimalModel(complexity: 'simple' | 'medium' | 'complex', userTier: string): string {
    ...
    switch (complexity) {
      case 'simple':
        return userTier === 'free' ? 'claude-3-haiku-20240307' : 'claude-4-sonnet-20250514'
      case 'medium':
        return 'claude-4-sonnet-20250514'
      case 'complex':
        return 'claude-4-sonnet-20250514'
  ...
  private calculateCost(inputTokens: number, outputTokens: number, model: string): number {
    ...
    const modelCosts = costs[model as keyof typeof costs] || costs['claude-4-sonnet-20250514']
