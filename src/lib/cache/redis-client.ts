...
class RedisClient {
  ...
  private async initialize() {
    ...
    try {
      ...
      this.client.on('error', (error) => {
        logger.error('api', '❌ Redis connection error:', error )
      ...
      logger.error('api', 'Failed to initialize Redis client:', error )
      this.client = null
    }
  }
  ...
  async expire(key: string, ttl: number, options: CacheOptions = {}): Promise<boolean> {
    ...
    try {
      ...
      logger.error('api', 'Cache expire error:', error )
      this.stats.errors++
      return false
    }
  }
  ...
  async mget<T = any>(keys: string[], options: CacheOptions = {}): Promise<(T | null)[]> {
    ...
    try {
      ...
      logger.error('api', 'Cache mget error:', error )
      this.stats.errors++
  ...
  async mset(keyValuePairs: Record<string, any>, options: CacheOptions = {}): Promise<boolean> {
    ...
    try {
      ...
      logger.error('api', 'Cache mset error:', error )
      this.stats.errors++
      return false
    }
  }
  ...
  async incr(key: string, options: CacheOptions = {}): Promise<number> {
    ...
    try {
      ...
      logger.error('api', 'Cache incr error:', error )
      this.stats.errors++
      return 0
    }
  }
  ...
  async decr(key: string, options: CacheOptions = {}): Promise<number> {
    ...
    try {
      ...
      logger.error('api', 'Cache decr error:', error )
      this.stats.errors++
      return 0
    }
  }
  ...
  async flushAll(): Promise<boolean> {
    ...
    try {
      ...
      logger.error('api', 'Cache flush error:', error )
      this.stats.errors++
      return false
    }
  }
