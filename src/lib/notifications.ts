...
export const NotificationTemplates = {
  ...
  maintenanceAlert: (equipmentName: string, alertType: string, message: string) => ({
    type: 'maintenance_alert' as NotificationType,
    title: `Maintenance Alert: ${alertType}`,
    message: `${equipmentName}: ${message}`,
  }),

  predictiveMaintenance: (equipmentName: string, prediction: string) => ({
    type: 'predictive_maintenance' as NotificationType,
    title: 'Predictive Maintenance Alert',
    message: `${equipmentName}: ${prediction}`,
  }),

  equipmentFailure: (equipmentName: string, errorCode?: string) => ({
    type: 'equipment_failure' as NotificationType,
    title: 'Equipment Failure',
    message: `${equipmentName} has failed${errorCode ? ` (Error: ${errorCode})` : ''}`,
  }),

  performanceWarning: (equipmentName: string, metric: string, value: number, threshold: number) => ({
    type: 'performance_warning' as NotificationType,
    title: 'Performance Warning',
    message: `${equipmentName}: ${metric} is ${value} (threshold: ${threshold})`,
  }),

  maintenanceDue: (equipmentName: string, dueDate: string) => ({
    type: 'maintenance_alert' as NotificationType,
    title: 'Maintenance Due',
    message: `${equipmentName} maintenance is due on ${dueDate}`,
  }),

  maintenanceOverdue: (equipmentName: string, daysPastDue: number) => ({
    type: 'maintenance_alert' as NotificationType,
    title: 'Maintenance Overdue',
    message: `${equipmentName} maintenance is ${daysPastDue} days overdue`,
  }),
};
...
async function sendEmailNotificationIfEnabled(
  ...
  const shouldSendEmail = {
    ...
    maintenance_alert: preferences.maintenanceAlerts !== false,
    predictive_maintenance: preferences.maintenanceAlerts !== false,
    equipment_failure: preferences.maintenanceAlerts !== false,
    performance_warning: preferences.maintenanceAlerts !== false,
    success: false,
    error: false,
    warning: false,
    info: false,
