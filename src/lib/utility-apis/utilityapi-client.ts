...
export class UtilityAPIClient {
  ...
  constructor() {
    ...
    this.client.interceptors.response.use(
      (response) => response,
      (error) => {
        logger.error('api', 'UtilityAPI Error:', error.response?.data || error.message );
        throw error;
      }
    );
  }
  ...
  async syncCustomerData(customerId: string): Promise<void> {
    ...
    for (const connection of connections) {
      try {
        ...
        logger.error('api', `Failed to sync connection ${connection.id}:`, error);
  ...
  async getUtilitiesForAddress(address: string): Promise<any[]> {
    try {
      ...
      logger.error('api', 'Failed to get utilities for address:', error );
      return [];
    }
  }
  ...
  async triggerSync(customerId: string): Promise<{ success: boolean; synced: number; errors: number }> {
    ...
    for (const connection of connections) {
      try {
        ...
        errors++;
        logger.error('api', `Sync failed for connection ${connection.id}:`, error);
      }
    }
  ...
  async bulkSync(): Promise<{ processed: number; successful: number; failed: number }> {
    ...
    for (const connection of activeConnections) {
      try {
        ...
        failed++;
        logger.error('api', `Bulk sync failed for ${connection.id}:`, error);
        await this.markConnectionError(connection.id, error);
      }
