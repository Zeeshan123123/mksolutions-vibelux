...
export class ArcadiaClient {
  ...
  constructor() {
    ...
    this.client.interceptors.response.use(
      (response) => response,
      (error) => {
        logger.error('api', 'Arcadia API Error:', error.response?.data || error.message );
        throw error;
      }
    );
  }
  ...
  async syncCustomerData(customerId: string): Promise<void> {
    ...
    for (const connection of connections) {
      try {
        ...
        logger.error('api', `Failed to sync Arcadia connection ${connection.id}:`, error);
  ...
  async getUtilitiesByLocation(zipCode: string): Promise<any[]> {
    ...
    try {
      ...
      logger.error('api', 'Failed to get utilities by location:', error );
      return [];
    }
  }
  ...
  async disconnectAccount(connectionId: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error('api', 'Failed to disconnect Arcadia account:', error );
      return false;
    }
  }
