...
export class DebugLogger {
  ...
  static error(message: string, error?: any) {
    ...
    this.logs.push(entry);
    logger.error('api', `[DEBUG ERROR] ${message}`, error instanceof Error ? error : undefined, error instanceof Error ? undefined : { error });
    this.notifyListeners();
  }
  ...
  static warn(message: string, details?: any) {
    ...
    this.logs.push(entry);
    logger.warn('api', `[DEBUG WARN] ${message}`, details);
    this.notifyListeners();
  }
