...
export class RealtimeTracker {
  ...
  async sendSOSAlert(message?: string): Promise<void> {
    ...
    try {
      ...
      logger.error('api', 'Failed to send SOS alert:', error );
      throw error;
    }
  }
  ...
  async createGeofence(zone: GeofenceZone): Promise<void> {
    ...
    try {
      ...
      logger.error('api', 'Failed to create geofence:', error );
      throw error;
    }
  }
  ...
  async shareLiveLocation(
    ...
    try {
      ...
      logger.error('api', 'Failed to share location:', error );
      throw error;
    }
  ...
  async stopSharingLocation(userIds?: string[]): Promise<void> {
    ...
    try {
      ...
      logger.error('api', 'Failed to stop sharing location:', error );
      throw error;
    }
  }
  ...
  private initializePusher(): void {
    ...
    if (!this.pusher) {
      logger.warn('api', 'Pusher not available, using mock service');
      return;
    }
  ...
  private handleLocationError(error: GeolocationPositionError): void {
    logger.error('api', 'Location error:', error );
  ...
  private async sendLocationUpdate(update: LocationUpdate): Promise<void> {
    try {
      ...
      logger.error('api', 'Failed to send location update:', error );
      // Error is already handled by TrackingErrorHandler (queued for offline sync)
    }
  }
  ...
  private async sendAlert(alert: TrackingAlert): Promise<void> {
    try {
      ...
      logger.error('api', 'Failed to send alert:', error );
      // Error is already handled by TrackingErrorHandler (queued for offline sync)
    }
  }
