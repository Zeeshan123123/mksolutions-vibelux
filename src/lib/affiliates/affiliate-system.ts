...
class AffiliateSystem {
  ...
  async processClick(
    ...
    const fraudCheck = await fraudDetection.checkClickFraud({
      ...
      clickId: visitorId // Use visitor ID temporarily
    });

    if (!fraudCheck.passed) {
      // Log fraud attempt
      await fraudDetection.logFraudAttempt({
        affiliateId: link.affiliateId,
        type: 'click',
      ...
      if (fraudCheck.action === 'block' || fraudCheck.action === 'suspend') {
  ...
  async trackConversion(
    ...
    const fraudCheck = await fraudDetection.checkConversionFraud({
    ...
    if (!fraudCheck.passed) {
      ...
      if (fraudCheck.action === 'block' || fraudCheck.action === 'suspend') {
        throw new Error('Conversion flagged for review');
      }
    }
