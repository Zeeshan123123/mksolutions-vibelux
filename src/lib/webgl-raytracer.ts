...
export class WebGLMonteCarloRaytracer extends MonteCarloRaytracer {
  ...
  private async initializeWebGL(): Promise<void> {
    try {
      ...
      if (!gl) {
        logger.warn('api', 'WebGL2 not available, falling back to CPU ray tracing');
        return;
      }
      ...
      for (const ext of requiredExtensions) {
        if (!gl.getExtension(ext)) {
          logger.warn('api', `Required WebGL extension ${ext} not available`);
          return;
        }
      }
      ...
      logger.info('api', 'WebGL ray tracing initialized successfully');

    } catch (error) {
      logger.warn('api', 'Failed to initialize WebGL ray tracing:', { data: error  });
  ...
  public async runSimulation(
    ...
    if (!this.isWebGLEnabled || !this.webglContext) {
      ...
      logger.info('api', 'Using CPU ray tracing fallback');
      return super.runSimulation(measurementPoints, parameters);
    }

    logger.info('api', 'Using GPU-accelerated ray tracing');
    ...
    try {
      ...
      logger.warn('api', 'GPU ray tracing failed, falling back to CPU', { data: { error } });
      return super.runSimulation(measurementPoints, parameters);
    }
  }
  ...
  private async runGPUSimulation(
    ...
    for (let frame = 0; frame < frameCount; frame++) {
      ...
      if (frame % 10 === 0) {
        ...
        if (convergence) {
          logger.info('api', `Converged after ${frame} frames`);
          break;
        }
      }
    }
