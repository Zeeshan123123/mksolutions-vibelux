...
export class CrossRefClient {
  ...
  async searchWorks(query: string, filters?: {
    ...
    try {
      ...
      logger.error('api', 'CrossRef search error:', error );
      return [];
    }
  }
  ...
  async getWorkByDOI(doi: string): Promise<CrossRefWork | null> {
    try {
      ...
      logger.error('api', 'CrossRef DOI lookup error:', error );
      return null;
    }
  }
  ...
  async enhancePapers(papers: ResearchPaper[]): Promise<EnhancedResearchPaper[]> {
    ...
    for (const paper of papers) {
      try {
        ...
        logger.error('api', `Error enhancing paper ${paper.title}:`, error);
        enhanced.push({
          ...paper,
          citedByCount: 0,
          referencesCount: 0,
  ...
  async searchAgriculturalLighting(): Promise<CrossRefWork[]> {
    ...
    for (const query of queries) {
      try {
        ...
        logger.error('api', `Error searching for query "${query}":`, error);
      }
    }
  ...
  async getCitationMetrics(doi: string): Promise<{
    ...
    try {
      ...
      logger.error('api', 'Error getting citation metrics:', error );
      return { citedByCount: 0, referencesCount: 0, citingWorks: [] };
    }
  }
