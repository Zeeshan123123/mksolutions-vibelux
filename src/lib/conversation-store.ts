...
class ConversationStore {
  ...
  getConversations(userId: string): Conversation[] {
    try {
      ...
      logger.error('api', 'Error loading conversations:', error )
      return []
    }
  }
  ...
  addMessage(conversationId: string, userId: string, message: ConversationMessage): void {
    ...
    if (!conversation) {
      logger.error('api', 'Conversation not found:', conversationId)
      return
    }
  ...
  private getAllConversations(): Conversation[] {
    try {
      ...
      logger.error('api', 'Error loading all conversations:', error )
      return []
    }
  }
  ...
  private saveAllConversations(conversations: Conversation[]): void {
    try {
      ...
      logger.error('api', 'Error saving conversations:', error )
