...
export class EquipmentRuntimeMonitor {
  ...
  startRuntime(equipmentId: string): void {
    if (this.activeSessions.has(equipmentId)) {
      logger.warn('api', `Runtime already being tracked for equipment ${equipmentId}`);
      return;
    }
  ...
  stopRuntime(equipmentId: string): number {
    ...
    if (!session) {
      logger.warn('api', `No active runtime session for equipment ${equipmentId}`);
      return 0;
    }
    ...
    this.updateInterval = setInterval(() => {
      ...
      this.runtimeData.forEach((data, equipmentId) => {
        if (data.status === 'running' && 
          ...
          logger.warn('api', `Runtime data stale for equipment ${equipmentId}, marking as stopped`);
...
export function handleRuntimeWebhook(req: Request): Response {
  try {
    ...
    logger.error('api', 'Runtime webhook error:', error );
