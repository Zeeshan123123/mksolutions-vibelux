...
export async function validateDeviceFingerprint(
  ...
  try {
    ...
    logger.error('api', 'Device fingerprint validation error:', error );
    return false;
  }
}
...
export async function createSecureMobileSession(
  ...
  try {
    ...
    logger.error('api', 'Failed to create secure mobile session:', error );
    return null;
  }
}
...
export async function verifyMobileTokenWithSession(
  ...
  try {
    ...
    if (!user) {
      logger.warn('api', `User not found in database for Clerk ID: ${payload.sub}`);
      return null;
    }
    ...
    logger.error('api', 'Mobile token verification failed:', error );
    return null;
  }
}
...
export async function revokeDeviceSessions(userId: string, deviceId: string): Promise<void> {
  try {
    ...
    logger.error('api', 'Failed to revoke device sessions:', error );
  }
}
...
export async function getUserActiveSessions(userId: string): Promise<MobileSession[]> {
  try {
    ...
    logger.error('api', 'Failed to get user sessions:', error );
    return [];
  }
}
...
export async function validateMobileApiKey(apiKey: string): Promise<boolean> {
  try {
    ...
    logger.error('api', 'API key validation failed:', error );
    return false;
  }
}