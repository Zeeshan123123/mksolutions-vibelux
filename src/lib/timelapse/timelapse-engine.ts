...
export class TimeLapseEngine {
  ...
  public startCapture(projectId: string): void {
    ...
    logger.info('api', `Started time-lapse capture for project: ${project.name}`);
  }
  ...
  private async captureFrame(projectId: string): Promise<TimeLapseFrame | null> {
    ...
    try {
      ...
      logger.error('api', `Failed to capture frame for project ${projectId}:`, error);
    ...
    project.milestones.forEach(milestone => {
      if (!milestone.completed && dayNumber >= milestone.targetDay) {
        ...
        if (criteriasMet) {
          ...
          logger.info('api', `Milestone achieved: ${milestone.name} on day ${dayNumber}`);
        }
      }
    });
  }
  ...
  private async processVideoCompilation(project: TimeLapseProject, video: CompiledVideo, settings: any): Promise<void> {
    ...
    logger.info('api', `Compiling video: ${video.name}`);
    logger.info('api', `Processing ${settings.frameRange.end - settings.frameRange.start} frames...`);
