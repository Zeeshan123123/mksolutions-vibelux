...
export class OfflineSyncService {
  ...
  private async processQueue(): Promise<void> {
      ...
      try {
        ...
        if (success) {
          ...
          if (operation.retryCount >= operation.maxRetries) {
            logger.error('api', `Max retries exceeded for operation ${operation.id}`);
            processedIds.push(operation.id); // Remove from queue
            failedOperations.push(operation);
          }
        }
      } catch (error) {
        logger.error('api', `Failed to execute operation ${operation.id}:`, error);
        operation.retryCount++;
        if (operation.retryCount >= operation.maxRetries) {
          processedIds.push(operation.id); // Remove from queue
          failedOperations.push(operation);
    ...
    if (failedOperations.length > 0) {
      logger.warn('api', `${failedOperations.length} operations failed permanently`);
    }
  ...
  private async executeOperation(operation: QueuedOperation): Promise<boolean> {
    try {
      ...
      if (response.ok) {
        ...
        logger.error('api', `Client error for operation ${operation.id}: ${response.status}`);
  ...
  private saveQueueToStorage(): void {
    try {
      ...
      logger.error('api', 'Failed to save queue to storage:', error );
    }
  }
  ...
  private loadQueueFromStorage(): void {
    try {
      ...
      logger.error('api', 'Failed to load queue from storage:', error );
      this.queue = [];
    }
  }
  ...
  private notifyStatusChange(): void {
    ...
    this.statusCallbacks.forEach(callback => {
      try {
        ...
        logger.error('api', 'Status callback error:', error );
      }
    });
  }
