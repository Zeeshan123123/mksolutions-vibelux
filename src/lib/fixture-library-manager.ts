...
export class FixtureLibraryManager {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (stored) {
        try {
          ...
          logger.error('api', 'Error loading fixture library state:', e)
        }
      }
    }
  ...
  async importFromIES(file: File): Promise<ImportResult> {
    ...
    try {
      ...
      // Derive horticulture PPF using electrical power and assumed PPE when spectral data is unavailable.
      // Do NOT convert lumens to PPF – that depends on spectrum and is not reliable from IES alone.
      const inputWatts = Math.round((parsedIES as any).metadata?.inputWatts || 0)
      const assumedPPE = 2.5 // μmol/J default until user overrides
      const estimatedPPF = inputWatts > 0 ? Math.round(inputWatts * assumedPPE) : undefined
      ...
      const fixture: Partial<CustomFixture> = {
        ...
        category: 'IES Import',
        wattage: inputWatts || 100,
        ppf: estimatedPPF || 250, // placeholder if watts unknown; user should update
        efficacy: estimatedPPF && inputWatts > 0 ? Number((estimatedPPF / inputWatts).toFixed(2)) : assumedPPE,
        spectrum: 'Custom IES',
        spectrumData: {
          blue: 20,
          green: 10,
      ...
      result.warnings.push('PPF estimated using wattage × PPE. Update PPF/PPE for accurate horticulture calculations.')
    } catch (e) {
