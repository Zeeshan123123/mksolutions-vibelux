...
export function encrypt(text: string): string {
  try {
    ...
    logger.error('api', 'Encryption failed:', error );
...
export function decrypt(encryptedData: string): string {
  try {
    ...
    logger.error('api', 'Decryption failed:', error );
...
export function verifyHash(data: string, hashedData: string): boolean {
  ...
  return crypto.timingSafeEqual(Buffer.from(computedHash, 'hex'), Buffer.from(hashedData, 'hex'));
}
