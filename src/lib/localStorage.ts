...
export function saveToLocalStorage<T>(key: string, data: T): void {
  try {
    ...
    logger.error('api', `Error saving to localStorage:`, error )
  }
}
...
export function loadFromLocalStorage<T>(key: string, defaultValue: T): T {
  try {
    ...
    logger.error('api', `Error loading from localStorage:`, error )
  }
  return defaultValue
}
...
export function removeFromLocalStorage(key: string): void {
  try {
    ...
    logger.error('api', `Error removing from localStorage:`, error )
  }
}
...
export function clearAllLocalStorage(): void {
  try {
    ...
    logger.error('api', `Error clearing localStorage:`, error )
  }
}
...
export function useLocalStorage<T>(key: string, defaultValue: T): [T, (value: T) => void] {
  ...
  const setValue = (value: T) => {
    try {
      ...
      logger.error('api', `Error setting localStorage value:`, error )
    }
  }
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === key && e.newValue) {
        try {
          ...
          logger.error('api', `Error parsing storage event:`, error )
        }
      }
    }
