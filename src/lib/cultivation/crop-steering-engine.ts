...
export class CropSteeringEngine extends EventEmitter {
  ...
  private evaluateTrigger(trigger: SteeringTrigger, data: any, metrics: SteeringMetrics): boolean {
    try {
      switch (trigger.type) {
        case 'sensor':
          // Combine data and metrics for variable lookup
          const sensorVariables = { ...data, ...metrics };
          return evaluateExpression(trigger.condition, sensorVariables);
        case 'morphology':
          return evaluateExpression(trigger.condition, metrics);
        default:
          return false;
      }
    } catch (error) {
      logger.error('api', 'Error evaluating trigger condition:', error );
      return false;
    }
  }
  ...
  private applySteeringStrategy(roomId: string, strategy: SteeringStrategy): void {
    ...
    if (safetyWarnings.length > 0) {
      ...
      if (criticalWarnings.length > 0) {
        logger.error('api', 'CRITICAL SAFETY VIOLATION: Strategy blocked', criticalWarnings);
        return;
      }
    }
