    ...
    try {
      Object.entries(cropDatabase).forEach(([category, crops]) => {
        ...
        if (!Array.isArray(crops)) {
          logger.warn('api', `Skipping category ${category}: crops is not an array`)
          return
        }
        
        crops.forEach(crop => {
          if (!crop || !crop.name) {
            logger.warn('api', `Skipping invalid crop in category ${category}`)
            return
          }
      ...
      logger.error('api', 'Error loading crop requirements:', error )
...
if (typeof window === 'undefined') {
  fixtureSearchEngine.initialize().catch((error) => logger.error('api', 'Async operation failed', error))
}
