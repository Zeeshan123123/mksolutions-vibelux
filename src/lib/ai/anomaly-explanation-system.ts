...
export class AnomalyExplanationSystem {
  ...
  async explainAnomaly(
    ...
    logger.info('api', `Generating explanation for ${anomaly.type} anomaly: ${anomaly.metric}`);
  ...
  private async generateClaudeExplanation(
    ...
    try {
      ...
      logger.error('api', 'Error generating Claude explanation:', error );
  ...
  private async getScientificBacking(anomaly: Anomaly, cropType: string): Promise<any> {
    try {
      ...
      logger.error('api', 'Error getting scientific backing:', error );
      return undefined;
    }
  }
}
