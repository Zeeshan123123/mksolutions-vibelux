...
export class FactBasedDesignAdvisor {
  ...
  private async verifyRecommendations(
    ...
    for (const rec of recommendations) {
      try {
        logger.info('api', `Verifying recommendation: ${rec.recommendation.substring(0, { data: 100 })}...`);
        ...
        logger.error('api', 'Error verifying recommendation:', error );
  ...
  private async getRelevantStudies(cropType: string, topic: string): Promise<string[]> {
    try {
      ...
      logger.error('api', 'Error fetching research studies:', error );
  ...
  async explainDesignDecision(
    ...
    try {
      ...
      logger.error('api', 'Error generating explanation:', error );
