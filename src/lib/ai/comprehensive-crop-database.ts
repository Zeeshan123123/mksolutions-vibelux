...
export class ComprehensiveCropDatabase {
  ...
  public async initializeDatabase(): Promise<void> {
    logger.info('api', 'Initializing comprehensive crop database...');
    ...
    logger.info('api', `Database initialized with ${this.database.varieties.size} crop varieties`);
  }
  ...
  private async updateFromResearch(): Promise<void> {
    try {
      ...
      logger.info('api', `Processing ${recentPapers.length} recent research papers...`);
      
    } catch (error) {
      logger.error('api', 'Error updating from research:', error );
    }
  }
  ...
  /**
   * Load flowers and ornamentals
   */
  private async loadFlowersOrnamentals(): Promise<void> {
   ...
   * Load microgreens
   */
  private async loadMicrogreens(): Promise<void> {
    // Placeholder implementation
    return Promise.resolve();
  }
   ...
   * Load mushrooms
   */
  private async loadMushrooms(): Promise<void> {
    // Placeholder implementation
    return Promise.resolve();
  }
   ...
   * Load tree fruits
   */
  private async loadTreeFruits(): Promise<void> {
    // Placeholder implementation
    return Promise.resolve();
  }
   ...
   * Load berries
   */
  private async loadBerries(): Promise<void> {
    // Placeholder implementation
    return Promise.resolve();
  }

  /**
   * Load grains and cereals
   */
  private async loadGrainsCereals(): Promise<void> {
    // Placeholder implementation
    return Promise.resolve();
  }
...
export function getCropDatabase(): ComprehensiveCropDatabase {
  // During build, return a stub that does nothing
  ...
  // Only initialize in browser or after build
  if (!isInitialized && typeof window !== 'undefined') {
    isInitialized = true;
    cropDatabaseInstance.initializeDatabase().catch((error) => logger.error('api', 'Async operation failed', error));
  }
  
  return cropDatabaseInstance;
}

// For backward compatibility - return stub during build
export const cropDatabase = process.env.NODE_ENV === 'production' && !global.window ? {
  getCrop: () => null,
  getCropByCategory: () => [],
  searchCrops: () => [],
  getAllCrops: () => [],
  ...
  getCrop: (name: string) => getCropDatabase().getCrop(name),
  getCropByCategory: (category: string) => getCropDatabase().getCropByCategory(category),
  searchCrops: (query: string) => getCropDatabase().searchCrops(query),
  getAllCrops: () => getCropDatabase().getAllCrops(),
  getCategories: () => getCropDatabase().getCategories(),
  updateFromResearch: () => getCropDatabase().updateFromResearch()
};