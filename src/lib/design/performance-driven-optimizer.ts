...
export class PerformanceDrivenOptimizer {
  ...
  private createImplementationPlan(recommendations: DesignRecommendation[]): ImplementationStep[] {
    ...
    phases.forEach((phase, index) => {
      const totalCost = phase && phase.length > 0 
        ? phase.reduce((sum, rec) => sum + (rec?.investmentRequired || 0), 0)
        : 0;
      const totalImpact = phase && phase.length > 0 
        ? phase.reduce((sum, rec) => sum + (rec?.expectedROI || 0), 0) / phase.length
        : 0;
  ...
  private getPhaseTitle(phase: DesignRecommendation[]): string {
    if (!phase || phase.length === 0) return 'Empty Phase';
    
    const categories = phase
      .map(r => r?.category)
      .filter(Boolean)
      .filter((value, index, self) => self.indexOf(value) === index); // unique values
      
    return categories.join(' & ').replace(/^\w/, c => c.toUpperCase()) || 'Miscellaneous';
  }

  private getPhaseDescription(phase: DesignRecommendation[]): string {
    if (!phase || phase.length === 0) return 'No optimizations in this phase';
    
    const types = phase
      .map(r => r?.type)
      .filter(Boolean);
      
    return `Implement ${phase.length} optimization${phase.length > 1 ? 's' : ''} focusing on ${types.join(', ')}`;
  }

  private calculatePhaseDuration(phase: DesignRecommendation[]): string {
    if (!phase || phase.length === 0) return '0 weeks';
    
    let maxWeeks = 1;
    
    for (const recommendation of phase) {
      try {
        const timeframe = recommendation?.implementation?.timeframe || '1 week';
        const timeframeParts = timeframe.split('-');
        const weeks = parseInt(timeframeParts[timeframeParts.length - 1]) || 1;
        if (weeks > maxWeeks) {
          maxWeeks = weeks;
        }
      } catch (error) {
        // Fallback to 1 week if parsing fails
        logger.warn('api', 'Error parsing timeframe:', { data: error  });
      }
    }
