...
export class KafkaStreams {
  ...
  async connect(): Promise<void> {
    try {
      ...
      logger.error('api', 'Failed to connect to Kafka:', error );
      throw error;
    }
  }
  ...
  async registerSchemas(): Promise<void> {
    ...
    for (const schema of schemas) {
      try {
        ...
        logger.error('api', `Failed to register schema ${schema.subject}:`, error);
      }
    }
  }
  ...
  async createTopics(topics: Array<{
    ...
    try {
      ...
      logger.error('api', 'Failed to create topics:', error );
      throw error;
    }
  }
  ...
  async publishTimeSeriesEvents(data: TimeSeriesData[]): Promise<void> {
    try {
      ...
      logger.error('api', 'Failed to publish time-series events:', error );
      throw error;
    }
  }
  ...
  async publishLogEvents(logs: Array<{
    ...
    try {
      ...
      logger.error('api', 'Failed to publish log events:', error );
      throw error;
    }
  }
  ...
  async setupConnectors(connectors: ConnectorConfig[]): Promise<void> {
    ...
    for (const connector of connectors) {
      try {
        ...
        logger.error('api', `Failed to register connector ${connector.name}:`, error);
      }
    }
  }
  ...
  async getTopicMetrics(topic: string): Promise<any> {
    try {
      ...
      logger.error('api', `Failed to get metrics for topic ${topic}:`, error);
      throw error;
    }
  }
  ...
  async getConsumerLag(groupId: string): Promise<any> {
    try {
      ...
      logger.error('api', `Failed to get consumer lag for group ${groupId}:`, error);
      throw error;
    }
  }
}
