...
export class UpdateManager {
  ...
  static async hasFeatureAccess(userId: string, featureFlag: string): Promise<boolean> {
    try {
      ...
      logger.error('api', 'Error checking feature access:', error );
      return false;
    }
  }
  ...
  static async handlePlanUpdate(
    ...
    try {
      ...
      logger.error('api', 'Error handling plan update:', error );
      throw error;
    }
  }
  ...
  static async deployUpdate(update: Update): Promise<void> {
    try {
      ...
      logger.info('api', `Update ${update.version} deployed with ${update.rolloutPercentage}% rollout`);
    } catch (error) {
      logger.error('api', 'Error deploying update:', error );
      throw error;
    }
  }
  ...
  static async increaseRollout(featureName: string, newPercentage: number): Promise<void> {
    ...
    logger.info('api', `Increased rollout for ${featureName} to ${newPercentage}%`);
  }
  ...
  static async emergencyDisable(featureName: string, reason: string): Promise<void> {
    ...
    logger.info('api', `Emergency disabled feature: ${featureName} - ${reason}`);
  }
