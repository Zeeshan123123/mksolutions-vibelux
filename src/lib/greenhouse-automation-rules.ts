...
export interface Actuator {
  id: string;
  type: 'heating' | 'cooling' | 'ventilation' | 'lighting' | 'irrigation' | 'co2' | 'shade' | 'blackout' | 'thermal_screen' | 'energy_screen';
  state: 'on' | 'off' | 'auto';
  value?: number; // 0-100 for variable control
  lastChange: Date;
  screenLayer?: 'upper' | 'middle' | 'lower' | 'sidewall';
}
...
export const RULE_TEMPLATES = {
  ...
  blackout_control: {
    photoperiod_enforcement: {
      id: 'blackout_photoperiod',
      name: 'Photoperiod Blackout Control',
      description: 'Deploy blackout curtains for dark period enforcement',
      enabled: true,
      priority: 15,
      conditions: [
        {
          ...
          value: ['18:00', '06:00'] // 12-hour dark period
        },
        {
          type: 'stage' as const,
      ...
      actions: [
        {
          actuatorId: 'blackout_screen_main',
          command: 'set' as const,
          value: 100,
          rampTime: 30 // Gradual 30-minute deployment
        }
      ],
      cooldownMinutes: 60
    },
    ...
    light_deprivation: {
      id: 'light_deprivation_schedule',
      name: 'Light Deprivation Schedule',
      description: 'Scheduled light deprivation for flowering induction',
      enabled: false,
      priority: 14,
      conditions: [
        {
          ...
          parameter: 'current',
          operator: 'between' as const,
          value: ['14:00', '02:00'] // Extended dark period
        }
      ],
      actions: [
        {
          actuatorId: 'blackout_screen_all',
          command: 'set' as const,
          value: 100
        },
        {
          actuatorId: 'sidewall_blackout',
          command: 'set' as const,
          value: 100
        }
      ]
    ...
    dark_period_protection: {
      id: 'dark_period_breach',
      name: 'Dark Period Protection',
      description: 'Protect dark period from light contamination',
      enabled: true,
      priority: 18,
      conditions: [
        {
      ...
      actions: [
        {
          actuatorId: 'blackout_screen_emergency',
          command: 'set' as const,
          value: 100
        },
        {
          actuatorId: 'alert_system',
          command: 'on' as const
        }
      ],
      cooldownMinutes: 5
  ...
  thermal_screen_control: {
    energy_saving_night: {
      id: 'thermal_screen_night',
      name: 'Night Energy Conservation',
      description: 'Deploy thermal screens at night for energy savings',
      enabled: true,
      priority: 8,
      conditions: [
        {
      ...
      actions: [
        {
          actuatorId: 'thermal_screen_upper',
          command: 'set' as const,
          value: 85,
          rampTime: 15
        },
        {
          actuatorId: 'energy_screen_middle',
          command: 'set' as const,
          value: 75,
          rampTime: 15
        }
    ...
    extreme_weather_protection: {
      id: 'weather_screen_deploy',
      name: 'Extreme Weather Screen Deployment',
      description: 'Deploy screens based on weather conditions',
      enabled: true,
      priority: 16,
      conditions: [
        {
      ...
      actions: [
        {
          actuatorId: 'all_screens',
          command: 'set' as const,
          value: 0 // Retract for safety
        }
      ],
      cooldownMinutes: 10
    }
  ...
  multi_layer_coordination: {
    summer_heat_management: {
      id: 'multi_layer_summer',
      name: 'Summer Multi-Layer Shading',
      description: 'Coordinate multiple screens for optimal cooling',
      enabled: true,
      priority: 12,
      conditions: [
        {
      ...
      actions: [
        {
          ...
          command: 'set' as const,
          value: 30 // First layer - 30% shade
        },
        {
          actuatorId: 'shade_screen_middle',
          command: 'set' as const,
          value: 30, // Second layer - additional 30%
          rampTime: 5
        }
      ],
      cooldownMinutes: 20
    ...
    winter_insulation: {
      id: 'multi_layer_winter',
      name: 'Winter Multi-Layer Insulation',
      description: 'Maximize heat retention with multiple screens',
      enabled: true,
      priority: 10,
      conditions: [
        {
      ...
      actions: [
        ...
        {
          actuatorId: 'thermal_screen_lower',
          command: 'set' as const,
          value: 85,
          rampTime: 15
        }
...
export class GreenhouseAutomationEngine {
  ...
  executeAction(action: Action) {
    ...
    if (!actuator) {
      logger.warn('api', `Actuator ${action.actuatorId} not found`);
      return;
    }
  ...
  importRules(rulesJson: string) {
    try {
      ...
      logger.error('api', 'Failed to import rules:', error );
