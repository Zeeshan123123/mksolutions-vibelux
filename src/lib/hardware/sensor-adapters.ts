...
export abstract class SensorAdapter extends EventEmitter {
  ...
  protected emitReading(reading: SensorReading): void {
    ...
    recordSensorReading(
      ...
      this.roomId,
      reading.metadata
    ).catch(err => logger.error('api', 'Failed to record reading:', err));
  }
}
  ...
  protected async pollSensors(): Promise<void> {
    for (const [sensorId, config] of this.sensors) {
      try {
        ...
        logger.error('api', `Error reading sensor ${sensorId}:`, error);
  ...
  private simulateSerialData(): void {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        logger.error('api', 'Error parsing serial data:', error );
      }
