...
export class UtilityRateService {
  ...
  async getRealTimeRate(facilityId: string): Promise<RealTimeRate> {
    try {
      ...
      logger.error('api', 'Failed to get real-time rate:', error );
  ...
  private async getUtilityRate(facilityId: string): Promise<UtilityRate> {
    ...
    try {
      ...
      try {
        ...
        logger.warn('api', 'Database table utility_rates not found', { data: 'using fallback rates' });
        storedRate = null;
      }
      ...
      logger.error('api', 'Failed to get utility rate:', error );
      throw error;
    }
  }
  ...
  private async fetchFromOpenEI(facilityId: string): Promise<UtilityRate> {
    try {
      ...
      logger.error('api', 'OpenEI API error:', error );
