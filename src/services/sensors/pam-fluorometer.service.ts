...
export interface PAMProtocolSettings {
  name: string;
  actinicLight: number;
  saturatingPulse: number;
  darkAdaptationTime: number;
  measurementInterval: number;
  totalMeasurementTime: number;
}
...
export class PAMFluorometerService {
  ...
  static protocols: Record<string, PAMProtocolSettings> = {
    'Standard PAM': {
      name: 'Standard PAM',
      actinicLight: 200,
      saturatingPulse: 3000,
    ...
    'Rapid Light Curve': {
      ...
      totalMeasurementTime: 480
    },
    'Quenching Analysis': {
      name: 'Quenching Analysis',
      actinicLight: 500,
    ...
    'Dark Recovery': {
      ...
      actinicLight: 0,
      saturatingPulse: 3000,
      darkAdaptationTime: 60,
      measurementInterval: 300,
      totalMeasurementTime: 1800
    }
  };
  ...
  async connectDevice(deviceId: string): Promise<boolean> {
    try {
      ...
      logger.info('api', `Connected to PAM device: ${deviceId}`);
      return true;
    } catch (error) {
      logger.error('api', 'Failed to connect to PAM device:', error );
      return false;
    }
  }
