...
export default function CalculatorsPage() {
  ...
  const getCropStatus = () => {
    if (Number(currentDLI) < currentCropData.min) return { status: 'Critical', color: 'red', icon: 'AlertTriangle' }
    if (Number(currentDLI) < currentCropData.optimal) return { status: 'Deficient', color: 'yellow', icon: 'TrendingDown' }
    if (Number(currentDLI) > currentCropData.max) return { status: 'Excessive', color: 'orange', icon: 'TrendingUp' }
    return { status: 'Optimal', color: 'green', icon: 'Check' }
  }
